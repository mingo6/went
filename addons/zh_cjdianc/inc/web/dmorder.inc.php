...
if(checksubmit('export_submit', true)) {
  ...
  $time="'%$time%'";
   $start=$_GPC['time']['start'];
  $end=$_GPC['time']['end'];
  $count = pdo_fetchcolumn("SELECT COUNT(*) FROM". tablename("cjdc_order")." WHERE type=4 and dm_state=2 and  time >='{$start}' and time<='{$end}'");
  ...
  for ($j = 1; $j <= $pagesize; $j++) {
    $sql = "select a.*,b.name as md_name from " . tablename("cjdc_order")."  a"  . " inner join " . tablename("cjdc_store")." b on a.store_id=b.id  WHERE a.type=4 and a.time >='{$start}' and a.dm_state=2 and a.time<='{$end}' limit " . ($j - 1) * 5000 . ",5000 ";
