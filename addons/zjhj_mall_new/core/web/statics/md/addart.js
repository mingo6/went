...
myyapp.controller('ctr',['$scope',function($scope){
	...
	$scope.modules = [
		{
			...
			params : {
				...
				data : [
					{
						id : '00000001',
						img: window.sysinfo.siteroot+'/addons/zjhj_mall/core/web/statics/images/stemp.png',
						type : 'url',
					}
				]
			}
				...
				data : [
					{
						id : '00000001',
						title : '导航名称',
						img: window.sysinfo.siteroot+'/addons/zjhj_mall/core/web/statics/images/thank.png',
						type : 'url',
						url : ''
					},
					{
	...
	$scope.otherurl = [
		{title:'查看表单数据',url:'/zjhj_mall/pages/form/form'},
		{title:'文章列表',url:'/zjhj_mall/pages/artlist/artlist'}
	];
	...
	$scope.saveData = function(){
        ...
        $.ajax({
            type: 'POST',
            dataType : 'json',
            url: window.sysinfo.siteroot + '/addons/zjhj_mall/core/web/index.php?r=mch%2Fsite%2Fmodule%2Fsavepage',
            data: {_csrf:_csrf, data : items, basic:basic, tid:tempid, id : page && page.id ? page.id : 0 },
            success: function(data){
                if(data.status == 200){
                    alert("已保存！");
					if( op == 'add' ) location.href = window.sysinfo.siteroot + '/addons/zjhj_mall/core/web/index.php?r=mch%2Fsite%2Fmodule%2Fpagelist'+'&id='+tempid;
            	}else{
                    alert("保存失败");
                }
            },
