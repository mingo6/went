...
{
    // 简单自定义错误方法(赶时间版)
    protected function reError($msg)
    {
        $return = [
            'status' => 1,
            'msg' => '此模板不能删除'
    ...
    public function actionIndex()
    {
        ...
        $SitetempListForm->store_id = $this->store_id;
        $module = $SitetempListForm->serach();
    ...
    //系统模板
    public function actionSysmodule()
    {
        ...
        $SitetempListForm->user_id = $admin->id;
        $SitetempListForm->store_id = $this->store_id;
        $module = $SitetempListForm->sysmodule();
        return $this->render('sysmodule', [
            'module' => $module
        ]);
    ...
    public function actionAddTemp()
    {
        ...
        if ($id) {
            $SitetempTemp = SitetempTemp::findOne($id);
        } else {
            $SitetempTemp = new SitetempTemp();
            $SitetempTemp->uniacid = $this->store->acid;
            $SitetempTemp->user_id = $admin->id;
            $SitetempTemp->store_id = $this->store_id;
            $SitetempTemp->createtime = time();
        }

        $SitetempTemp->name = \Yii::$app->request->post('name');
        $SitetempTemp->number = \Yii::$app->request->post('number');
        $SitetempTemp->img = \Yii::$app->request->post('img');

        if ($SitetempTemp->save()) {
            return [
                'code' => 0,
                'msg' => '添加成功！'
            ];
    ...
    public function actionDelete()
    {
        ...
        $sitetempTemp = SitetempTemp::findOne($id);
        if (!$sitetempTemp) {
            return $this->reError('此模板不存在');
        }

        if ($sitetempTemp->issystem == 1) {
            return $this->reError('此模板不能删除');
        }

        if ($sitetempTemp->isact == 1) {
            return $this->reError('使用中的模板不能删除');
        }

        if ($sitetempTemp->delete()) {
            $return = [
                'status' => 200,
                'msg' => Url::toRoute(['index'])
            ];
    ...
    //复制为系统模板
    public function actionTosystem()
    {
        ...
        $temp = SitetempTemp::findOne($id);
        if (!$temp) {
            return $this->reError('没有找到模板');
        }

        $new_temp = new SitetempTemp();
        $new_temp->uniacid = $this->store->acid;
        $new_temp->createtime = time();
        $new_temp->name = $temp['name'];
        $new_temp->number = 1;
        $new_temp->img = $temp['img'];
        $new_temp->isact = 0;
        $new_temp->issystem = 1;
        $new_temp->issetsystem = 1;

        if ($new_temp->save()) {

            $bar = SitetempBar::findOne(['tempid' => $temp['id']]);

            $new_bar = new SitetempBar();
            //$new_bar -> scenario ='edit';
            $new_bar->uniacid = $this->store->acid;
            $new_bar->data = $bar->data;
            $new_bar->createtime = time();
            $new_bar->tempid = $new_temp->id;

            $new_bar->save();

            $page = SitetempPage::find(['tempid' => $temp['id']])->asArray()->all();

            foreach ($page as $k => $v) {

                $SitetempPage = new SitetempPage();
                //$SitetempPage -> scenario = 'edit';
                $SitetempPage->uniacid = $this->store->acid;
                $SitetempPage->params = $v['params'];
                $SitetempPage->createtime = time();
                $SitetempPage->name = $v['name'];
                $SitetempPage->tempid = $new_temp->id;
                $SitetempPage->save();
            }
    ...
    public function actionTopage()
    {
        ...
        if ($temp->name == '电镀厂系统模板' && $temp->issystem == 1) {
            $this->insertddPage($tid);
        } elseif ($temp->name == '网络公司系统模板' && $temp->issystem == 1) {
            $this->insertwlPage($tid);
        } else {
            ...
            if ($tid) {
                ...
                $SitetempBar = new SitetempBar();
                $SitetempBar->attributes = $data;
                $SitetempBar->save();
                $page = SitetempPage::find()->where([
                    'tempid' => $temp->id,
                ])->asArray()->all();
                ...
                if ($page) {
                    ...
                    foreach ($page as $v) {
                        ...
                        $new_page = new SitetempPage();
                        //$new_page -> scenarios ='edit';
                        $new_page->attributes = $data;
                        $new_page->save();
                    }
                }
            } else {
        ...
        $this->redirect(Url::toRoute(['sysmodule']));
    }


    //删除系统模板
    public function actionDelsystem()
    {
        ...
        //echo 1;die;
        SitetempBar::deleteAll(['tempid' => $id]);
        SitetempPage::deleteAll(['tempid' => $id]);
        $SitetempTemp = SitetempTemp::findOne($id);
        $SitetempTemp->delete();

        $return = [
            'status' => 200,
            'msg' => Url::toRoute(['sysmodule'])
        ];
        $this->renderJson($return);
    ...
    public function actionPagelist()
    {
        ...
        $tempid = \Yii::$app->request->get('tid');

        $where['uniacid'] = $this->store->acid;
        $where['tempid'] = $tempid;

        $tempPage = SitetempPage::find()->where($where)->orderBy(['id' => SORT_DESC])->asArray()->all();

        return $this->render('pagelist', [
            'tempPage' => $tempPage,
            'tempid' => $tempid,
            'pagination' => count($tempPage)
        ]);
    ...
    public function actionSavepage()
    {
        ...
        $getdata = htmlspecialchars_decode(\Yii::$app->request->post('data'));
        $getdata = json_decode($getdata, true);

        $basic = htmlspecialchars_decode(\Yii::$app->request->post('basic'));
        ...
        $temp = SitetempTemp::findOne(['id' => $tid, 'uniacid' => $this->store->acid]);
        ...
        $content = serialize(array('basic' => $basic, 'data' => $getdata));

        if ($id > 0) {

            $SitetempTemp = SitetempPage::findOne($id);
            $SitetempTemp->params = $content;
            $SitetempTemp->name = $name;
            $SitetempTemp->createtime = time();
            $SitetempTemp->tempid = $tid;

            $res = $SitetempTemp->save();

        } else {

            $SitetempTemp = new SitetempPage();
            $SitetempTemp->params = $content;
            $SitetempTemp->name = $name;
            $SitetempTemp->createtime = time();
            $SitetempTemp->tempid = $tid;
    ...
    public function actionSavebar()
    {
        ...
        $isset = SitetempBar::findOne(['uniacid' => $this->store->acid, 'tempid' => $tid]);
        if ($isset) {
            $isset->data = serialize(\Yii::$app->request->post('data'));
            $isset->createtime = time();
            $isset->save();
        } else {
            $bar = new SitetempBar();
            $bar->uniacid = $this->store->acid;
            $bar->data = serialize(\Yii::$app->request->post('data'));
            $bar->createtime = time();
            $bar->tempid = $tid;
    ...
    public function actionDelpage()
    {
        ...
        if (is_array($id)) {
            $res = SitetempPage::delete([
                'in', 'id', $id
            ]);
        } else {
            $SitetempPage = SitetempPage::findOne($id);
            $res = $SitetempPage->delete();
        }

        $this->redirect(Url::toRoute(['site/module/pagelist', 'tid' => $tempid]));
    }
    ...
    //使用模板
    public function actionSetacttemp()
    {
        ...
        $sitetempTemp = SitetempTemp::findOne($where);
        if ($sitetempTemp) {
            $sitetempTemp->isact = 0;
            $res = $sitetempTemp->save();
        } else {
            $res = true;
        }
    ...
    public function actionEditview()
    {
        ...
        $module = SitetempTemp::findOne($tid);

        return $this->render('editview', [
            'module' => $module
        ]);
    ...
    public function actionEditmd()
    {
        ...
        if ($id && $model) {
            $SitetempTemp = SitetempTemp::findOne($id);
            $SitetempTemp->name = $model['name'];
            $SitetempTemp->number = $model['number'];
            $SitetempTemp->img = $model['img'];
            if ($SitetempTemp->save()) {
                $return = [
                    'status' => 200,
                    'msg' => '成功'
    ...
    public function actionEditnav()
    {
        ...
        $page = SitetempPage::findOne($id);
        if (!empty($page)) $page = $page->toArray();

        $page['params'] = unserialize($page['params']);

        return $this->render('editnav', [
            'page' => $page,
            '_W' => $_W
        ]);
    }
    ...
    public function actionEditbar()
    {

        $_W = $this->getW();

        $tid = \Yii::$app->request->get('tid');
        $page = SitetempBar::findOne(['uniacid' => $this->store->acid, 'tempid' => $tid]);
        if (!empty($page)) {
            $page = $page->toArray();
            $page['data'] = unserialize($page['data']);
        }
        return $this->render('editbar', [
            'page' => $page,
            '_W' => $_W
        ]);
    }
    ...
    public function insertddPage($tid)
    {
        global $_W;
        $tid = intval($tid);
        ...
        if ($tid > 0) {
            ...
            $dd1 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/dd_1.jpg';
            $dd2 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/dd_2.jpg';
            $dd3 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/dd_3.jpg';
            $dd4 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/dd_4.jpg';
            $dd5 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/dd_5.jpg';
            $dd6 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/dd_6.jpg';
            $dd7 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/dd_7.jpg';

            $ddn1 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/dd_n1.png';
            $ddn2 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/dd_n2.png';
            $ddn3 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/dd_n3.png';
            $ddn4 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/dd_n4.png';

            $foot1 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/home1.png';
            $foot2 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/home2.png';
            $foot3 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/tel1.png';
            $foot4 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/tel2.png';
            $foot5 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/location1.png';
            $foot6 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/location2.png';
            $foot7 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/share1.png';
            $foot8 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/share2.png';
            ...
            $SitetempPage = new SitetempPage();
            $SitetempPage -> attributes = $pdata;
            $sitetempPage -> save();
            
            $aboutid = $sitetempPage -> id;
            ...
            $bdata = array(
                ...
                'tempid' => $tid,
                'createtime' => time(),
                'data' => 'a:6:{s:3:"num";s:1:"4";s:7:"padding";s:1:"5";s:7:"bgcolor";s:7:"#ffffff";s:5:"color";s:4:"#999";s:8:"actcolor";s:7:"#ed414a";s:4:"data";a:4:{i:0;a:9:{s:2:"id";s:8:"00000001";s:4:"name";s:6:"首页";s:3:"img";s:80:"' . $foot1 . '";s:6:"actimg";s:80:"' . $foot2 . '";s:3:"url";s:38:"/zjhj_mall/pages/page/page?pid=' . $indexid . '";s:4:"type";s:3:"url";s:9:"$$hashKey";s:9:"object:20";s:6:"pageid";s:2:"' . $indexid . '";s:7:"urlname";s:6:"首页";}i:1;a:8:{s:2:"id";s:8:"00000002";s:4:"name";s:6:"电话";s:3:"img";s:80:"' . $foot3 . '";s:6:"actimg";s:80:"' . $foot4 . '";s:3:"url";s:0:"";s:4:"type";s:3:"tel";s:9:"$$hashKey";s:9:"object:21";s:3:"tel";s:11:"13112345678";}i:2;a:9:{s:2:"id";s:15:"m01507451962587";s:3:"img";s:80:"' . $foot5 . '";s:6:"actimg";s:80:"' . $foot6 . '";s:3:"url";s:0:"";s:4:"name";s:6:"地图";s:4:"type";s:3:"map";s:9:"$$hashKey";s:9:"object:51";s:3:"lng";s:18:"114.05121803283691";s:3:"lat";s:18:"22.561628756102063";}i:3;a:7:{s:2:"id";s:15:"m11507451962587";s:3:"img";s:80:"' . $foot7 . '";s:6:"actimg";s:80:"' . $foot8 . '";s:3:"url";s:0:"";s:4:"name";s:6:"分享";s:4:"type";s:5:"share";s:9:"$$hashKey";s:9:"object:52";}}}'
            );

            $SitetempBar = new SitetempBar();
            $SitetempBar->attributes = $bdata;
            $SitetempBar->save();
    ...
    public static function insertwlPage($tid)
    {
        global $_W;
        $tid = intval($tid);
        ...
        if ($tid > 0) {
            ...
            $dd1 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_as1.jpg';
            $dd2 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_s4.jpg';
            $dd3 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_as3.jpg';

            $fx1 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_f1.jpg';
            $fx2 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_f2.jpg';
            $fx3 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_f3.jpg';
            $fx4 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_f4.jpg';

            $m1 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_m1.jpg';
            $m2 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_m2.jpg';
            $m3 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_m3.jpg';
            $m4 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_m4.jpg';
            $m5 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_m5.jpg';

            $xcx1 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_x1.jpg';
            $xcx2 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_d2.jpg';
            $xcx3 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_d3.jpg';
            $xcx4 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_d4.jpg';
            $xcx5 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_d5.jpg';
            $xcx6 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_d6.jpg';

            $index1 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_s1.jpg';
            $index2 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_s2.jpg';
            $index3 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_s3.png';
            $index4 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_s4.png';

            $index5 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_n1.png';
            $index6 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_n2.png';
            $index7 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_n3.png';
            $index8 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_n4.png';

            $index9 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_p1.png';
            $index10 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_p2.png';
            $index11 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_p3.png';
            $index12 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_p4.png';
            $index13 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_p5.png';
            $index14 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_p6.png';

            $index15 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_ma1.jpg';
            $index16 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/wl_ma2.jpg';

            $foot1 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/home1.png';
            $foot2 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/home2.png';
            $foot3 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/contact1.png';
            $foot4 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/contact2.png';
            $foot5 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/tel1.png';
            $foot6 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/tel2.png';
            $foot7 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/about1.png';
            $foot8 = $_SERVER['HTTP_HOST'] . '/addons/zjhj_mall/core/web/statics/images/about2.png';
...
div;
            ...
            $sitetemp_page = new SitetempPage();
            $sitetemp_page->scenario = 'edit';
            $sitetemp_page->attributes = $pdata;
            $sitetemp_page->save();
            $aboutid = $sitetemp_page->id;
...
div;
            ...
            $sitetemp_page->attributes = $pdata;
            $sitetemp_page->save();
            $indexid = $sitetemp_page->id;



            $params = <<<div
a:6:{s:3:"num";s:1:"4";s:7:"padding";s:1:"5";s:7:"bgcolor";s:7:"#ffffff";s:5:"color";s:4:"#999";s:8:"actcolor";s:7:"#ed414a";s:4:"data";a:4:{i:0;a:9:{s:2:"id";s:8:"00000001";s:4:"name";s:6:"首页";s:3:"img";s:80:"{$foot1}";s:6:"actimg";s:80:"{$foot2}";s:3:"url";s:38:"/zjhj_mall/pages/page/page?pid={$indexid}";s:4:"type";s:3:"url";s:9:"$$hashKey";s:9:"object:20";s:6:"pageid";s:2:"{$indexid}";s:7:"urlname";s:6:"首页";}i:1;a:7:{s:2:"id";s:8:"00000002";s:4:"name";s:6:"咨询";s:3:"img";s:80:"{$foot3}";s:6:"actimg";s:80:"{$foot4}";s:3:"url";s:0:"";s:4:"type";s:4:"kefu";s:9:"$$hashKey";s:9:"object:21";}i:2;a:9:{s:2:"id";s:15:"m01507472225651";s:3:"img";s:80:"{$foot5}";s:6:"actimg";s:80:"{$foot6}";s:3:"url";s:38:"/zjhj_mall/pages/page/page?pid={$contactid}";s:4:"name";s:12:"联系我们";s:4:"type";s:3:"url";s:9:"$$hashKey";s:9:"object:54";s:6:"pageid";s:2:"{$contactid}";s:7:"urlname";s:12:"联系我们";}i:3;a:9:{s:2:"id";s:15:"m11507472225651";s:3:"img";s:80:"{$foot7}";s:6:"actimg";s:80:"{$foot8}";s:3:"url";s:38:"/zjhj_mall/pages/page/page?pid={$aboutid}";s:4:"name";s:12:"关于我们";s:4:"type";s:3:"url";s:9:"$$hashKey";s:9:"object:55";s:6:"pageid";s:2:"{$aboutid}";s:7:"urlname";s:12:"关于我们";}}}
div;
            $bdata = array(
                'uniacid' => $_W['uniacid'],
                'tempid' => $tid,
            ...
            $sitetemp_page = new SitetempBar();
            $sitetemp_page->scenario = 'edit';
            $sitetemp_page->attributes = $bdata;
            $sitetemp_page->save();
    ...
    {
        $sitetempPage = new SitetempPage();
        $page = $sitetempPage->find()->where(array('uniacid' => $this->store->acid))->asArray()->select(['id', 'name'])->all();
        if( !empty( $page ) ){
            foreach ( $page as &$v ) {
        ...
        $news = SitetempArticle::find()->where(array('uniacid' => $this->store->acid))->asArray()->select(['id','title'])->all();
        if( !empty( $news ) ){
            foreach ( $news as &$v ) {
        ...
        $this->renderJson($this->success('OK', array('page' =>$page,'news'=>$news)));
    }
}