...
class UserController extends Controller
{
    ...
    public function actionEdit($id=null)
    {
        ...
        if(\Yii::$app->request->isAjax){
            ...
            if($parent_id > 0 && $user->parent_id != $parent_id)
            {
                $parentIdInfo= User::findOne(['id'=>$parent_id,'store_id'=>$this->store->id]);
                if(empty($parentIdInfo))
                {
                    return json_encode([
                        'code'  => 1,
                        'msg'   => '上级会员不存在'
     ...
     * 后台用户余额充值
     */
    public function actionRechangebalance()
    {
        ...
        $admin = \Yii::$app->user->identity;
        if ($rechangeType == '2'){
            if ($balance > $user->balance || $user->balance == ""){
                return json_encode([
                    'code'  => 1,
                    'msg'   => '用户当前余额不足'
                ],JSON_UNESCAPED_UNICODE);
        ...
        if ($rechangeType == '2'){
            $balance_log->content = "管理员： ".$admin->username." 后台操作账号：".$user->nickname." 余额扣除：".$balance." 元";
        }elseif ($rechangeType == '1'){
            $balance_log->content = "管理员： ".$admin->username." 后台操作账号：".$user->nickname." 余额充值：".$balance." 元";
        }

        $balance_log->money = $balance;
