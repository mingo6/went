...
//            $EXCEL_OUT .= iconv('UTF-8', 'GB2312', implode($out,',')."\n");
    ...
    {
        $title = "序号,订单号,用户,商品名,规格,数量,收件人,电话,地址,总金额（含运费）,运费,实际付款,付款状态,申请状态,发货状态,收货状态,快递单号,快递公司,下单时间";
        $title .= "\n";
        $EXCEL_OUT = iconv('UTF-8','GBK',$title);
        $count = 1;
        foreach($info as $index=>$value){
            foreach($value['goods_list'] as $i=>$v){
                // $price = round($v['total_price'] * $value['pay_price'] / ($value['total_price'] - $value['express_price'] - $value['coupon_sub_price']),2);
                $price = $v['total_price'];
                $goods_str = "";//规格
                $out = array();
                $out[] = $count;
