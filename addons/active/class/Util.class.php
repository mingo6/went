...
//    }
	...
	//在一张表里查一列数据,组成数组
    static function getColumnDataInSingleTable($tablename,$array,$column)
    {
        $data = self::structWhereStringOfAnd($array);
        $sql = "SELECT $column FROM " . tablename($tablename) . " WHERE $data[0]";
        $res = pdo_fetch($sql, $data[1]);
        $result = array();
        foreach ($res as $k=>$v)
        {
            array_push($result,(int)$v);
        }
        return $result;
    }
...
//		if ($isuni) $array['uniacid'] = $_W['uniacid'];
	...
	//注册jssdk，因为勤瀚堂自带的方法内没有加openAddress，所以重新写一个。
