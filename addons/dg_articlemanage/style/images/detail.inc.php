<?php
    ...
    if($power['appoint']==2){
            ...
            //查找访问用户的id
            $usql="select id from ".tablename('dg_article_user')."  where openid=:openid";
            $mparm=array(":openid"=>$_W['openid']);
            ...
            $flag = true;
            foreach( $uid as $val ){
                if( !in_array($val, $powerid) ){
                $flag = false;
                break;
                }
            }
            if(!$flag){
                echo '<div style="width:80% height:50%; margin:40% auto;text-align:center;font-size:24px;color:red;">';
                echo '<h1>只对指定的用户开放阅览！</h1>';
                echo '<img style="width:80%;" src="'.$_SERVER['HTTP_HOST'].'/alidata/www/tx/addons/dg_articlemanage/style/images" />';
                echo '</div>';
                exit();
            }
    }
