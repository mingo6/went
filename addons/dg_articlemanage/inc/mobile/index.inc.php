...
$chcategroy=pdo_fetchall("select * from ".tablename('dg_article_category')." where parentid=:cid",array(":cid"=>$cid));
$catenum=count($chcategroy);
$title = $category['name'];
$op = $_GPC['op'];
...
if ($cid > 0) {
   ...
   if ($defaultid>0) {
       $list = pdo_fetchall("SELECT * FROM " . tablename('dg_article') . " WHERE uniacid={$uniacid} AND pcate={$cid} AND ccate={$defaultid} ORDER BY displayorder desc,id desc  ");
    } else {
