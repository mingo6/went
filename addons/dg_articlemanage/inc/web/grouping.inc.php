...
$sql="select * from ".tablename('dg_article_group')." where ".$condition." uniacid=:uniacid  order by createtime desc limit ".intval($pindex-1)*$psize.",".$psize;
...
if(!empty($_GPC['del'])){
    $id=$_GPC['id'];
    pdo_delete('dg_article_group',array('id'=>$id,'uniacid'=>$uniacid));
