...
if($_GPC['type']=="sy"){
    $sy_id=$_GPC['sy_id'];
    $recommend=pdo_get("dg_article",array('id'=>$sy_id));
    if($recommend['recommend']==1){
        $recommend['recommend']=2;
    }else{
        $recommend['recommend']=1;
    }
    pdo_update("dg_article",array('recommend'=>$recommend['recommend']),array('id'=>$sy_id));
...
if ($op == 'display') {
    if (!empty($_GPC['displayorder'])) {
        ...
        message('课程排序更新成功！', $this->createWebUrl('article', array('op' => 'display')), 'success');
    }
...
//    }else
    ...
    if (checksubmit('submit')) {
       ...
       //empty($_GPC['catagroy']['childid']) ? message('亲,必选选择二级分类') : $_GPC['catagroy']['childid'];
        ...
        if($_GPC['types']==2){
            $images=array_filter($_GPC['images']);
            for($i=0;$i<count($images);$i++){
                $images[$i]=tomedia($images[$i]);
            }

            $data['images'] = implode(",",array_filter($images));
            $data['img_free'] = $_GPC['img_free'];
        }
        if($_GPC['types']==3){

            if(strstr($_GPC['bg_music'],'http:') || strstr($_GPC['bg_music'],'https:')){
                $bg_music=$_GPC['bg_music'];
            }else{
                $bg_music=$_W['siteroot']."attachment/".$_GPC['bg_music'];
            }
             $data['bg_music'] = $bg_music;
             $data['aud_free'] = $_GPC['aud_free'];
        }  
        if($_GPC['types']==4){
            if(strstr($_GPC['vedio_url'],'http:')||strstr($_GPC['vedio_url'],'https:')){
                $vedio_url=$_GPC['vedio_url'];
            }else{
                $vedio_url=$_W['siteroot']."attachment/".$_GPC['vedio_url'];
            }
            $data['vedio_url'] = $vedio_url;
            $data['ved_free'] = $_GPC['ved_free'];
        }
        if (!empty($_GPC['thumb'])) {
...
//				}
        ...
        message('课程更新成功！', $this->createWebUrl('article', array('foo' => 'display')), 'success');
    }
} elseif ($op == 'delete') {
    load()->func('file');
    $id = intval($_GPC['id']);
