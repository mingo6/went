...
class ExchangeModel extends PluginModel 
{
	...
	public function sendExchangeMessage($openid, $num, $type = 0) 
	{
		...
		if ($type == 0) 
		{
			...
			$text = '恭喜您' . $credittext . '兑换成功！' . "\n\n" . $credittext . '兑换：' . $num . '积分币' . "\n" . '兑换时间：' . $time . "\n" . '充值方式：积分币兑换' . "\n" . $credittext . '余额：' . (int) $member['credit1'] . $credittext . ' ' . "\n" . $remark;
			$message = array( 'first' => array('value' => '亲爱的' . $member['nickname'] . '，恭喜您' . $credittext . '兑换成功，具体如下:', 'color' => '#ff0000'), 'keyword1' => array('title' => '获得时间', 'value' => $time, 'color' => '#000000'), 'keyword2' => array('title' => '获得积分币', 'value' => $num . $credittext, 'color' => '#000000'), 'keyword3' => array('title' => '获得原因', 'value' => $credittext . '兑换', 'color' => '#000000'), 'keyword4' => array('title' => '当前' . $credittext, 'value' => (double) $member['credit1'] . $credittext, 'color' => '#ff0000'), 'remark' => array('value' => "\n" . $_W['shopset']['shop']['name'] . '感谢您的支持，如有疑问请联系在线客服。', 'color' => '#000000') );
		}
		else if ($type == 1) 
		{
