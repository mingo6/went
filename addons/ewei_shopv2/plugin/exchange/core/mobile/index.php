...
class Index_EweiShopV2Page extends PluginMobileLoginPage 
{
	public function main() 
	{
		...
		if ($is_exchange[0] === '1') 
		{
			...
			headers(mobileUrl('exchange.' . $is_exchange[1]));
		}
		else 
		{
	...
	public function score() 
	{
		...
		if ($exchange == '1') 
		{
			...
			$balance_res = $this->chongzhi('credit1', $member['id'], 0, $score, '兑换中心:积分币兑换');
			$balance_res = intval($balance_res);
			if ($balance_res === 1) 
			{
				$repeatcount = $this->model->setRepeatCount($key);
				...
				pdo_insert('ewei_shop_exchange_record', $record);
				pdo_query('UPDATE ' . $table1 . ' SET `use` = `use` + 1 WHERE id = :id AND uniacid = :uniacid', array(':id' => $groupResult['id'], ':uniacid' => $_W['uniacid']));
				show_json(1, '成功兑换了' . $score . '个积分币');
				$this->message('成功兑换了' . $score . '个积分币');
			}
			else 
			{
				show_json(0, '很遗憾，兑换失败了');
		...
		{
			$this->message('点击兑换积分币', mobileUrl('exchange.score', array('exchange' => 1)));
		}
	}
	...
	public function calculate() 
	{
		...
		if (0 < $price) 
		{
			...
			headers(mobileUrl('order.create', array('exchange' => 1)));
		}
		else 
		{
