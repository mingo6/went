...
class Log_EweiShopV2Page extends PluginWebPage 
{
	public function main() 
	{
		...
		{
			headers(webUrl('creditshop/log/exchange'));
		}
		else if (cv('creditshop.log.draw')) 
		{
			headers(webUrl('creditshop/log/draw'));
		}
		else if (cv('creditshop.log.order')) 
		{
			headers(webUrl('creditshop/log/order'));
		}
		else if (cv('creditshop.log.convey')) 
		{
			headers(webUrl('creditshop/log/convey'));
		}
		else if (cv('creditshop.log.finish')) 
		{
			headers(webUrl('creditshop/log/finish'));
		}
		else if (cv('creditshop.log.verifying')) 
		{
			headers(webUrl('creditshop/log/verifying'));
		}
		else if (cv('creditshop.log.verifyover')) 
		{
			headers(webUrl('creditshop/log/verifyover'));
		}
		else if (cv('creditshop.log.verify')) 
		{
			headers(webUrl('creditshop/log/verify'));
		}
		else 
		{
			headers(webUrl('creditshop'));
		}
		exit();
	}
	...
	public function doexchange() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			else if ($goods['goodstype'] == 3) 
			{
				...
				$params = array('openid' => $log['openid'], 'tid' => $log['logno'] . rand(1, 100), 'send_name' => '积分币商城红包兑换', 'money' => $money, 'wishing' => '红包领到手抽筋，别人加班你加薪!', 'act_name' => '积分币商城红包兑换', 'remark' => '积分币商城红包兑换');
			...
			plog('creditshop.log.doexchange', '积分币商城兑换 兑换记录ID: ' . $id);
			show_json(1);
		}
