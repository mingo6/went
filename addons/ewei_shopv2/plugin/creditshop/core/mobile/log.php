...
class Log_EweiShopV2Page extends PluginMobileLoginPage 
{
	...
	public function Receivepacket() 
	{
		...
		$params = array('openid' => $openid, 'tid' => $log['logno'], 'send_name' => ($set['sendname'] ? $set['sendname'] : $_W['shopset']['shop']['name']), 'money' => $money, 'wishing' => ($set['sendname'] ? $set['wishing'] : '红包领到手抽筋，别人加班你加薪!'), 'act_name' => '积分币兑换红包', 'remark' => '积分币兑换红包');
	...
	public function express() 
	{
		...
		if (empty($logid)) 
		{
			headers(mobileUrl('creditshop/log'));
			exit();
		}
	...
	public function paydispatch() 
	{
		...
		if ($paytype == 'wechat') 
		{
			...
			$params['title'] = $set['shop']['name'] . ((empty($goods['type']) ? '积分币兑换' : '积分币抽奖')) . ' 支付运费单号:' . $dispatchno;
		...
		else if ($paytype == 'alipay') 
		{
			...
			$params['user'] = $openid;
			$params['fee'] = $goods['dispatch'];
			$params['title'] = $set['shop']['name'] . '积分币兑换运费支付' . ' 单号:' . $log['logno'];
	...
	public function wechat_dispatch_complete() 
	{
		...
		{
			headers(mobileUrl('creditshop/log/detail', array('id' => $log['id']), true));
		}
	}
