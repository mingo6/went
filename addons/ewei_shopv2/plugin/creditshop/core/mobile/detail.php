...
class Detail_EweiShopV2Page extends CreditshopMobilePage 
{
	...
	public function pay($a = array(), $b = array()) 
	{
		...
		if ($needpay) 
		{
			...
			else if ($paytype == 'wechat') 
			{
				...
				$params['title'] = $set['shop']['name'] . ((empty($goods['type']) ? '积分币兑换' : '积分币抽奖')) . ' 单号:' . $log['logno'];
	...
	public function lottery() 
	{
		...
		if (empty($log['paystatus'])) 
		{
			...
			{
				show_json(0, array('status' => '-1', 'message' => '积分币不足!'));
			}
		...
		if ((0 < ($goods['money'] + $goods['dispatch'])) && ($log['paystatus'] < 1)) 
		{
			if ($log['paytype'] == 0) 
			{
				m('member')->setCredit($openid, 'credit2', -($goods['money'] + $goods['dispatch']), '积分币商城抽奖扣除余额度 ' . $goods['money']);
				$update['paystatus'] = 1;
			}
			if ($log['paytype'] == 1) 
			{
		...
		{
			m('member')->setCredit($openid, 'credit1', -$goods['credit'], '积分币商城抽奖扣除积分币 ' . $goods['credit']);
			$update['creditpay'] = 1;
		...
		if ($status == 2) 
		{
			...
			else if ($goods['goodstype'] == 2) 
			{
				...
				$data = array('openid' => $openid, 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => $set['name'] . '积分币商城兑换余额', 'money' => $num, 'remark' => $creditstr, 'rechargetype' => 'creditshop');
