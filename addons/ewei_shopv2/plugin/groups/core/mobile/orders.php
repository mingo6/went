...
class Orders_EweiShopV2Page extends PluginMobileLoginPage 
{
	...
	public function express() 
	{
		...
		if (empty($orderid)) 
		{
			headers(mobileUrl('groups/orders'));
			exit();
		}
		...
		if (empty($order)) 
		{
			headers(mobileUrl('groups/order'));
			exit();
		}
		if (empty($order['addressid'])) 
		{
	...
	public function confirm() 
	{
		...
		try 
		{
			...
			if ($_W['ispost']) 
			{
				...
				{
					headers(mobileUrl('groups/address/post'));
					exit();
				}
				if ($isverify) 
				{
				...
				headers(MobileUrl('groups/pay', array('teamid' => (empty($teamid) ? $order['teamid'] : $teamid), 'orderid' => $orderid)));
			}
			$this->model->groupsShare();
			include $this->template();
		}
