...
class Pay_EweiShopV2Page extends PluginMobileLoginPage 
{
	public function main() 
	{
		...
		if (empty($order)) 
		{
			headers(mobileUrl('groups'));
			exit();
		}
		if ($order['status'] == -1) 
		{
			headers(mobileUrl('groups/goods', array('id' => $order['goodid'])));
			exit();
		}
		...
		{
			headers(mobileUrl('groups/goods', array('id' => $order['id'])));
			exit();
		}
		if (empty($log)) 
		{
	...
	public function complete() 
	{
		...
		if ($type == 'credit') 
		{
			...
			{
				headers(mobileUrl('groups/team/detail', array('orderid' => $orderid, 'teamid' => $orderid)));
				exit();
			}
		}
		else if ($type == 'wechat') 
