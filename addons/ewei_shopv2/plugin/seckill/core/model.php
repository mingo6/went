...
class SeckillModel extends PluginModel 
{
				...
				foreach ($orders as $orderid ) 
				{
					...
					if (!(empty($o)) && ($o['status'] == 0)) 
					{
						if ($o['isparent'] == 0) 
						{
							...
							if (0 < $o['deductprice']) 
							{
								m('member')->setCredit($o['openid'], 'credit1', $o['deductcredit'], array('0', $_W['shopset']['shop']['name'] . '秒杀自动关闭订单返还抵扣积分币 积分币: ' . $o['deductcredit'] . ' 抵扣金额: ' . $o['deductprice'] . ' 订单号: ' . $o['ordersn']));
							}
						}
	...
	public function orderRefund($item) 
	{
		...
		if (0 < $credits) 
		{
			m('member')->setCredit($item['openid'], 'credit1', -$credits, array(0, $shopset['name'] . '退款扣除购物赠送积分币: ' . $credits . ' 订单号: ' . $item['ordersn']));
		}
		if (0 < $item['deductcredit']) 
		{
			m('member')->setCredit($item['openid'], 'credit1', $item['deductcredit'], array('0', $shopset['name'] . '购物返还抵扣积分币 积分币: ' . $item['deductcredit'] . ' 抵扣金额: ' . $item['deductprice'] . ' 订单号: ' . $item['ordersn']));
		}
		if (!(empty($refundtype))) 
		{
			if ($realprice < 0) 
