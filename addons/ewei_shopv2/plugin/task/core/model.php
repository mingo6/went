...
class TaskModel extends PluginModel 
{
	...
	public function notice_complain($templete, $member, $poster, $scaner = '', $type = 1) 
	{
		...
		if ($templete) 
		{
			...
			if ($poster['reward_data']) 
			{
				...
				$templete = str_replace('[积分币奖励]', $poster['reward_data'][$reward_type]['credit'], $templete);
				$reward_text = '';
			...
			else 
			{
				...
				$templete = str_replace('[积分币奖励]', '0', $templete);
			}
		...
		if (($join_info['needcount'] == $count) && ($join_info['is_reward'] == 0)) 
		{
			...
			foreach ($reward_data as $key => $val ) 
			{
				if ($key == 'rec') 
				{
					...
					{
						m('member')->setCredit($qr['openid'], 'credit1', $val['credit'], array(0, '推荐扫码关注积分币+' . $val['credit']));
					}
				...
				else if ($key == 'sub') 
				{
					if (0 < $val['credit']) 
					{
						m('member')->setCredit($openid, 'credit1', $val['credit'], array(0, '扫码关注积分币+' . $val['credit']));
					}
					if (0 < $val['bribery']) 
					{
						$tid = rand(1, 1000) . time() . rand(1, 10000);
	...
	public function sendReward($reward_data = array(), $btn = 0, $openid = NULL) 
	{
		...
		{
			m('member')->setCredit($openid, 'credit1', $reward_data['score'], array(0, '完成任务积分币+' . $reward_data['score']));
		}
