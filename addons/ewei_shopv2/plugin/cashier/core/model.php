...
class CashierModel extends PluginModel 
{
	...
	public function refund($id) 
	{
		...
		{
			m('member')->setCredit($pay_log['openid'], 'credit1', -$pay_log['present_credit1'], $_W['cashieruser']['title'] . ' 收银台退款收回赠送的积分币! 退款订单号' . $out_trade_no);
		}
	...
	public function sendCredit1($log, $userset = NULL) 
	{
		...
		$credit1 = com_run('sale::getCredit1', $log['openid'], $price, 37, 1, 0, $log['title'] . '收银订单号 : ' . $log['logno'] . '  收银台消费送积分币');
		return $credit1;
	}
