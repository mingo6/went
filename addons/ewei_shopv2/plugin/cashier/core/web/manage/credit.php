...
class Credit_EweiShopV2Page extends CashierWebPage 
{
	...
	public function recharge_submit() 
	{
		...
		{
			show_json(0, '积分币充值尚未开启!');
		}
		$logno = m('common')->createNO('cashier_log', 'logno', 'ST');
		$log = array('uniacid' => $_W['uniacid'], 'cashierid' => $_W['cashierid'], 'paytype' => $paytype, 'logno' => $logno, 'title' => '收银台积分币充值!', 'createtime' => time(), 'status' => 0, 'money' => $money);
		...
		$params = array('uniacid' => $_W['uniacid'], 'tid' => $logno, 'fee' => $money, 'title' => '收银台积分币充值');
		if ($paytype == 0) 
		{
