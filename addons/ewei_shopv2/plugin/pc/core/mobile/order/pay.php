...
class Pay_EweiShopV2Page extends MobileLoginPage 
{
	public function main() 
	{
		...
		if (!(empty($ispeerpay))) 
		{
			...
			{
				headers(mobileUrl('pc.order/pay/peerpayshare', array('id' => $orderid)));
				exit();
			}
			else 
			{
		...
		{
			headers(mobileUrl('pc.order/pay/complete', array('id' => $order['id'], 'type' => 'credit')));
			exit();
		}
		load()->model("payment");
	...
	public function peerpay() 
	{
		...
		if (!(empty($peerpay))) 
		{
			headers(mobileUrl('pc.order/pay/peerpayshare', array('id' => $peerpay)));
			exit();
		}
	...
	public function peerpayshare() 
	{
		...
		{
			headers(mobileUrl('pc.order/pay/peerpaydetail', array('id' => $peerid)));
			exit();
		}
