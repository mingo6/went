		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($member2))) 
			{
				if (strexists($member2['openid'], 'sns_wx_') || strexists($member2['openid'], 'sns_qq_') || strexists($member2['openid'], 'wap_user_')) 
				{
					...
					{
						m('member')->setCredit($member['openid'], 'credit1', abs($member2['credit1']), 'WAP会员数据合并增加积分币 +' . $member2['credit1']);
					}
					...
					if ($changeAgentid) 
					{
						pdo_update('ewei_shop_member', array('agentid' => $member2['agentid'],'agentid_str' => $member2['agentid_str']), array('id' => $member['id']));
					}
