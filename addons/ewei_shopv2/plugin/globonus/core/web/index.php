...
class Index_EweiShopV2Page extends PluginWebPage 
{
	public function main() 
	{
		...
		{
			headers(webUrl('globonus/partner'));
			exit();
			return NULL;
		}
		if (cv('globonus.level')) 
		{
			headers(webUrl('globonus/level'));
			exit();
			return NULL;
		}
		if (cv('globonus.bonus')) 
		{
			headers(webUrl('globonus/bonus'));
			exit();
			return NULL;
		}
		if (cv('globonus.bonus.send')) 
		{
			headers(webUrl('globonus/bonus/send'));
			exit();
			return NULL;
		}
		if (cv('globonus.notice')) 
		{
			headers(webUrl('globonus/notice'));
			exit();
			return NULL;
		}
		if (cv('globonus.set')) 
		{
			headers(webUrl('globonus/set'));
			exit();
		}
	}
