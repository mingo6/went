...
class Goods_EweiShopV2Page extends MerchWebPage 
{
	...
	protected function post() 
	{
		...
		if (0 < count($options)) 
		{
			...
			else 
			{
				...
				$html .= '<th"><div class=""><div style="padding-bottom:10px;text-align:center;">积分币</div></div></th>';
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				...
				plog('creditshop.goods.edit', '编辑积分币商城商品 ID: ' . $id . ' <br/>商品名称: ' . $data['title']);
			}
			else 
			{
				...
				$id = pdo_insertid();
				plog('creditshop.goods.add', '添加积分币商城商品 ID: ' . $id . '  <br/>商品名称: ' . $data['title']);
			}
			if ($data['hasoption']) 
			{
				$totalstocks = 0;
	...
	public function delete() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			plog('creditshop.goods.delete', '删除积分币商城商品 ID: ' . $item['id'] . '  <br/>商品名称: ' . $item['title'] . ' ');
		}
	...
	public function property() 
	{
		...
		if (in_array($type, array('istop', 'isrecommand', 'status', 'displayorder', 'title'))) 
		{
			...
			plog('creditshop.goods.edit', '修改积分币商城商品' . $typestr . '状态   ID: ' . $id . ' ' . $statusstr . ' ');
		}
		show_json(1);
	}
