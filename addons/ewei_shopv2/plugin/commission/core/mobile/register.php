	...
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if ($template_flag == 1) 
			{
				...
				$m_data['isagent'] = 1;
				$m_data['agentid'] = $mid;
				$m_data['agentid_str'] = m('member')->getAgentidStr($mid,$iagent['agentid_str']);
			...
			{
				$agentid_str = m('member')->getAgentidStr($mid,$iagent['agentid_str']);
				$data = array('isagent' => 1, 'agentid' => $mid, 'agentid_str' => $agentid_str, 'status' => $become_check, 'realname' => $_GPC['realname'], 'mobile' => $_GPC['mobile'], 'weixin' => $_GPC['weixin'], 'agenttime' => ($become_check == 1 ? time() : 0));
		...
		if (empty($set['become'])) 
		{
			...
			{
				$agentid_str = m('member')->getAgentidStr($mid,$iagent['agentid_str']);
				$data = array('isagent' => 1, 'agentid' => $mid, 'agentid_str' => $agentid_str, 'status' => $become_check, 'realname' => trim($_GPC['realname']), 'mobile' => trim($_GPC['mobile']), 'weixin' => trim($_GPC['weixin']), 'agenttime' => ($become_check == 1 ? time() : 0));
				pdo_update('ewei_shop_member', $data, array('id' => $member['id']));
				if ($become_check == 1) 
				{
					$this->model->sendMessage($member['openid'], array('nickname' => $member['nickname'], 'agenttime' => $data['agenttime']), TM_COMMISSION_BECOME);
