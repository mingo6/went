...
class Rank_EweiShopV2Page extends CommissionMobileLoginPage 
{
	...
	public function main() 
	{
		...
		{
			headers(mobileUrl('commission'));
			exit();
		}
		$commission_rank = $this->commission_rank;
		switch ($this->commission_rank['type']) 
		{
			case '0': $user = pdo_fetch('SELECT id,uid,credit1,nickname,avatar,commission_total FROM ' . tablename('ewei_shop_member') . ' WHERE uniacid = :uniacid AND openid = :openid', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['mopenid']));
			...
			break;
			case '1': $user = pdo_fetch('SELECT id,uid,credit1,nickname,avatar,commission_total FROM ' . tablename('ewei_shop_member') . ' WHERE uniacid = :uniacid AND openid = :openid', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['mopenid']));
			...
			break;
			case '2': $user = pdo_fetch('SELECT id,uid,credit1,nickname,avatar,commission_total FROM ' . tablename('ewei_shop_member') . ' WHERE uniacid = :uniacid AND openid = :openid', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['mopenid']));
	...
	public function ajaxpage() 
	{
		...
		switch ($this->commission_rank['type']) 
		{
			case '0': $this->commissionTotal($pindex, $psize);
			break;
			case '1': $this->commissionPay($pindex, $psize);
			break;
			case '2': $this->commissionVirtual($pindex, $psize);
			break;
		}
	}
