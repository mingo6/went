...
class Index_EweiShopV2Page extends PluginWebPage 
{
	public function main() 
	{
		...
		{
			headers(webUrl('commission/agent'));
			exit();
		}
		else if (cv('commission.apply.view1')) 
		{
			headers(webUrl('commission/apply', array('status' => 1)));
			exit();
		}
		else if (cv('commission.apply.view2')) 
		{
			headers(webUrl('commission/apply', array('status' => 2)));
			exit();
		}
		else if (cv('commission.apply.view3')) 
		{
			headers(webUrl('commission/apply', array('status' => 3)));
			exit();
		}
		...
		{
			headers(webUrl('commission/increase'));
			exit();
		}
		else if (cv('commission.notice')) 
		{
			headers(webUrl('commission/notice'));
			exit();
		}
		else if (cv('commission.cover')) 
		{
			headers(webUrl('commission/cover'));
			exit();
		}
		else if (cv('commission.level')) 
		{
			headers(webUrl('commission/level'));
			exit();
		}
		else if (cv('commission.set')) 
		{
			headers(webUrl('commission/set'));
			exit();
		}
	}
	...
	public function set() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			plog('commission.set.edit', '修改基本设置<br>' . '分销内购 -- ' . $selfbuy . '<br>成为团队条件 -- ' . $become_child . '<br>成为分销商条件 -- ' . $become);
