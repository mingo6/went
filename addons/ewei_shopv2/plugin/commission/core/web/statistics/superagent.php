...
class Superagent_EweiShopV2Page extends PluginWebPage 
{
	public function main() 
	{
		...
		if (!(empty($searchfield)) && !(empty($keyword))) 
		{
			...
			elseif($searchfield == 'superagent')
			{
				$condition .= ' and ( dm.realname like :keyword or dm.nickname like :keyword or dm.mobile like :keyword) AND dm.issuperagent = 1';
				$params[':keyword'] = '%' . $keyword . '%';
			}
		...
		if (!(empty($_GPC['ordertime']['start'])) && !(empty($_GPC['ordertime']['end']))) 
		{
			...
			$order_cndition .= ' AND o.createtime between :starttime and :endtime';
			$member_condition .= ' AND createtime between :starttime and :endtime';
			$order_param[':starttime'] = $order_param[':starttime'] = $starttime;
			$member_param[':starttime'] = $member_param[':endtime'] = $endtime;
		}
		...
		foreach ($list as &$row ) 
		{
			...
			$row['level1_ordermoney'] = $info['order1money'];
			$row['level2_ordermoney'] = $info['order2money'];
			$row['level3_ordermoney'] = $info['order3money'];
			...
			$openidStr = '';
			if(!empty($row['agentid_str']))
			{
				$sql = 'SELECT openid FROM '.tablename('ewei_shop_member').' WHERE id in ('.$row['agentid_str'].')' . $member_condition;
				$memberList = pdo_fetchall($sql,$member_param);
				if(!empty($memberList))
				{
					foreach($memberList AS $key=>$memberInfo)
					{
						$openidStr .= '"'.$memberInfo['openid'].'"'.',';
					}
					$openidStr = trim($openidStr,',');
				}
				$row['levelcount'] = count($memberList);
			}
			else
			{
				$row['levelcount'] = 0;
			...
			if(!empty($openidStr)){
				$sql = 'SELECT (o.price) AS price,count(o.id) AS count FROM '.tablename('ewei_shop_order').' AS o WHERE o.openid in ('. $openidStr .') AND o.openid != :openid AND  o.status>=1 AND o.uniacid=:uniacid'.$order_cndition;
				$order_param[':openid'] = $row['openid'];
				$order_param[':uniacid'] = $_W['uniacid'];
				$arrOrderTotalPrice = pdo_fetch($sql,$order_param);
				$row['total_order_price'] = $arrOrderTotalPrice['price'];
				$row['total_order_count'] = $arrOrderTotalPrice['count'];
			}else{
				$row['total_order_price'] = 0;
				$row['total_order_count'] = 0;
			}
		...
		if ($_GPC['export'] == '1') 
		{
			...
			plog('commission.agent.export', '导出合伙人统计数据');
			foreach ($list as &$row ) 
			{
			...
			unset($row);
			$columns = array( array('title' => 'ID', 'field' => 'id', 'width' => 12), array('title' => '昵称', 'field' => 'nickname', 'width' => 12), array('title' => '姓名', 'field' => 'realname', 'width' => 12), array('title' => '手机号', 'field' => 'mobile', 'width' => 12), array('title' => '微信号', 'field' => 'weixin', 'width' => 12), array('title' => 'openid', 'field' => 'openid', 'width' => 24), array('title' => '推荐人', 'field' => 'parentname', 'width' => 12), array('title' => '点击数', 'field' => 'clickcount', 'width' => 12), array('title' => '团队合伙人总数', 'field' => 'levelcount', 'width' => 12), array('title' => '团队合伙人业绩', 'field' => 'levelcount', 'width' => 12), array('title' => '注册时间', 'field' => 'createtime', 'width' => 12), array('title' => '成为合伙人时间', 'field' => 'superagenttime', 'width' => 12), array('title' => '审核状态', 'field' => 'createtime', 'width' => 12), array('title' => '是否关注', 'field' => 'followstr', 'width' => 12) );
			if (p('diyform')) 
			{
				$columns[] = array('title' => '合伙人会员自定义信息', 'field' => 'member_diyformdata', 'width' => 36);
				$columns[] = array('title' => '合伙人申请自定义信息', 'field' => 'agent_diyformdata', 'width' => 36);
			}
			m('excel')->export($list, array('title' => '合伙人统计数据-' . date('Y-m-d-H-i', time()), 'columns' => $columns));
		}
	...
	public function delete() 
	{
		...
		foreach ($members as $member ) 
		{
			...
			plog('commission.agent.delete', '取消合伙人资格 <br/>合伙人信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
		}
	...
	public function user() 
	{
		...
		if ($_GPC['export'] == 1) 
		{
			...
			unset($row);
			$columns = array( array('title' => 'ID', 'field' => 'id', 'width' => 12), array('title' => '昵称', 'field' => 'nickname', 'width' => 12), array('title' => '姓名', 'field' => 'realname', 'width' => 12), array('title' => '手机号', 'field' => 'mobile', 'width' => 12), array('title' => '微信号', 'field' => 'weixin', 'width' => 12), array('title' => 'openid', 'field' => 'openid', 'width' => 24), array('title' => '推荐人', 'field' => 'parentname', 'width' => 12), array('title' => '合伙人等级', 'field' => 'levelname', 'width' => 12), array('title' => '点击数', 'field' => 'clickcount', 'width' => 12), array('title' => '团队合伙人总数', 'field' => 'levelcount', 'width' => 12), array('title' => '一级团队合伙人数', 'field' => 'level1', 'width' => 12), array('title' => '二级团队合伙人数', 'field' => 'level2', 'width' => 12), array('title' => '三级团队合伙人数', 'field' => 'level3', 'width' => 12), array('title' => '累计佣金', 'field' => 'commission_total', 'width' => 12), array('title' => '打款佣金', 'field' => 'commission_pay', 'width' => 12), array('title' => '注册时间', 'field' => 'createtime', 'width' => 12), array('title' => '成为合伙人时间', 'field' => 'createtime', 'width' => 12), array('title' => '审核状态', 'field' => 'createtime', 'width' => 12), array('title' => '是否关注', 'field' => 'followstr', 'width' => 12) );
			if (p('diyform')) 
			{
				$columns[] = array('title' => '合伙人自定义信息', 'field' => 'agent_diyformdata', 'width' => 36);
			}
			m('excel')->export($list, array('title' => '推广团队-' . date('Y-m-d-H-i', time()), 'columns' => $columns));
		}
		load()->func('tpl');
	...
	public function check() 
	{
		...
		foreach ($members as $member ) 
		{
			...
			if ($status == 1) 
			{
				...
				plog('commission.agent.check', '审核合伙人 <br/>合伙人信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			...
			else 
			{
				...
				plog('commission.agent.check', '取消审核 <br/>合伙人信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
		}
		show_json(1, array('url' => referer()));
	}
	public function agentblack() 
	{
		...
		foreach ($members as $member ) 
		{
			...
			if ($agentblack == 1) 
			{
				...
				plog('commission.agent.agentblack', '设置黑名单 <br/>合伙人信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
			else 
			{
				pdo_update('ewei_shop_member', array('isagent' => 1, 'status' => 1, 'agentblack' => 0), array('id' => $member['id']));
				plog('commission.agent.agentblack', '取消黑名单 <br/>合伙人信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
		}
		show_json(1, array('url' => referer()));
	}
