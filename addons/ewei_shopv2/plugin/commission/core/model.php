...
if (!(class_exists('CommissionModel'))) 
{
	class CommissionModel extends PluginModel 
	{
		public function getSet($uniacid = 0) 
		{
			...
			$set['texts'] = array('agent' => (empty($set['texts']['agent']) ? '分销商' : $set['texts']['agent']), 'shop' => (empty($set['texts']['shop']) ? '小店' : $set['texts']['shop']), 'myshop' => (empty($set['texts']['myshop']) ? '我的小店' : $set['texts']['myshop']), 'center' => (empty($set['texts']['center']) ? '分销中心' : $set['texts']['center']), 'become' => (empty($set['texts']['become']) ? '成为分销商' : $set['texts']['become']), 'withdraw' => (empty($set['texts']['withdraw']) ? '提现' : $set['texts']['withdraw']), 'commission' => (empty($set['texts']['commission']) ? '佣金' : $set['texts']['commission']), 'commission1' => (empty($set['texts']['commission1']) ? '分销佣金' : $set['texts']['commission1']), 'commission_total' => (empty($set['texts']['commission_total']) ? '累计佣金' : $set['texts']['commission_total']), 'commission_ok' => (empty($set['texts']['commission_ok']) ? '可提现佣金' : $set['texts']['commission_ok']), 'commission_apply' => (empty($set['texts']['commission_apply']) ? '已申请佣金' : $set['texts']['commission_apply']), 'commission_check' => (empty($set['texts']['commission_check']) ? '待打款佣金' : $set['texts']['commission_check']), 'commission_lock' => (empty($set['texts']['commission_lock']) ? '未结算佣金' : $set['texts']['commission_lock']), 'commission_detail' => (empty($set['texts']['commission_detail']) ? '提现明细' : (($set['texts']['commission_detail'] == '佣金明细' ? '提现明细' : $set['texts']['commission_detail']))), 'commission_pay' => (empty($set['texts']['commission_pay']) ? '成功提现佣金' : $set['texts']['commission_pay']), 'commission_wait' => (empty($set['texts']['commission_wait']) ? '待收货佣金' : $set['texts']['commission_wait']), 'commission_fail' => (empty($set['texts']['commission_fail']) ? '无效佣金' : $set['texts']['commission_fail']), 'commission_charge' => (empty($set['texts']['commission_charge']) ? '扣除个人所得税' : $set['texts']['commission_charge']), 'order' => (empty($set['texts']['order']) ? '分销订单' : $set['texts']['order']), 'c1' => (empty($set['texts']['c1']) ? '一级' : $set['texts']['c1']), 'c2' => (empty($set['texts']['c2']) ? '二级' : $set['texts']['c2']), 'c3' => (empty($set['texts']['c3']) ? '三级' : $set['texts']['c3']), 'mydown' => (empty($set['texts']['mydown']) ? '我的团队' : $set['texts']['mydown']), 'down' => (empty($set['texts']['down']) ? '团队' : $set['texts']['down']), 'up' => (empty($set['texts']['up']) ? '推荐人' : $set['texts']['up']), 'yuan' => (empty($set['texts']['yuan']) ? '元' : $set['texts']['yuan']), 'icode' => (empty($set['texts']['icode']) ? '邀请码' : $set['texts']['icode']));
			return $set;
		}
		...
		public function getInfo($openid, $options = NULL) 
		{
			...
			if (1 <= $level) 
			{
				if (in_array('ordercount0', $options)) 
				{
					...
					$order10money += $level1_ordercount['ordermoney'];
				}
				if (in_array('ordercount', $options)) 
				{
					...
					$order1money += $level1_ordercount['ordermoney'];
				}
				...
				if (in_array('total', $options)) 
				{
					...
					foreach ($level1_commissions as $c ) 
					{
						...
						{
							$tempCommissionValue = (((isset($commission['level' . $agentLevel['id']]) ? $commission['level' . $agentLevel['id']] : isset($commission['default'])) ? $commission['default'] : 0));
						}
						else 
						{
							$tempCommissionValue = ((isset($commissions['level1']) ? floatval($commissions['level1']) : 0));
						}
						$commission_total += $tempCommissionValue;
						$commission_total1 += $tempCommissionValue;
					}
				}
			...
			if (2 <= $level) 
			{
				if (0 < $level1) 
				{
					if (in_array('ordercount0', $options)) 
					{
						...
						$order20money += $level2_ordercount['ordermoney'];						
					}
					if (in_array('ordercount', $options)) 
					{
						...
						$order2money += $level2_ordercount['ordermoney'];
					}
					if (in_array('ordercount3', $options)) 
					{
					...
					if (in_array('total', $options)) 
					{
						...
						foreach ($level2_commissions as $c ) 
						{
							...
							if (empty($commissions)) 
							{
								$tempCommissionValue = ((isset($commission['level' . $agentLevel['id']]) ? $commission['level' . $agentLevel['id']] : $commission['default']));
							}
							else 
							{
								$tempCommissionValue = ((isset($commissions['level2']) ? $commissions['level2'] : 0));
							}
							$commission_total += $tempCommissionValue;
							$commission_total2 += $tempCommissionValue;
						}
					}
					if (in_array('ok', $options)) 
					{
			...
			if (3 <= $level) 
			{
				if (0 < $level2) 
				{
					if (in_array('ordercount0', $options)) 
					{
						...
						$order30money += $level3_ordercount['ordermoney'];
					}
					if (in_array('ordercount', $options)) 
					{
						...
						$order3money += $level3_ordercount['ordermoney'];
					}
					if (in_array('ordercount3', $options)) 
					{
					...
					if (in_array('total', $options)) 
					{
						...
						foreach ($level3_commissions as $c ) 
						{
							...
							{
								$tempCommissionValue = ((isset($commissions['level3']) ? $commissions['level3'] : 0));
							}
							$commission_total += $tempCommissionValue;
							$commission_total3 += $tempCommissionValue;
						}
					}
					if (in_array('ok', $options)) 
					{
			...
			$member['order1money'] = $order1money;
			$member['order2money'] = $order2money;
			$member['order3money'] = $order3money;
			$member['order10money'] = $order10money;
			$member['order20money'] = $order20money;
			$member['order30money'] = $order30money;
			...
			$member['commission_total1'] = round($commission_total1, 2);
			$member['commission_total2'] = round($commission_total2, 2);
			$member['commission_total3'] = round($commission_total3, 2);
				...
				if ($member['id'] != $parent['id']) 
				{
					if (empty($become_child)) 
					{
						if (empty($member['fixagentid'])) 
						{
							...
							$author = p('author');
							$agentid_str = m('member')->getAgentidStr($parent['id'],$parent['agentid_str']);
							if ($author) 
							{
								pdo_update('ewei_shop_member', array('agentid_str'=>$agentid_str,'agentid' => $parent['id'], 'childtime' => $time, 'authorid' => $authorid), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
							}
							else 
							{
								pdo_update('ewei_shop_member', array('agentid_str'=>$agentid_str,'agentid' => $parent['id'], 'childtime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
							}
							if ($author) 
							{
		...
		public function sendMessage($openid = '', $data = array(), $message_type = '') 
		{
			...
			if (($message_type == TM_COMMISSION_AGENT_NEW) && empty($usernotice['commission_agent_new'])) 
			{
				...
				$msg = array( 'keyword1' => array('value' => (!(empty($tm['commission_agent_newtitle'])) ? $tm['commission_agent_newtitle'] : '新增团队通知'), 'color' => '#73a68d'), 'keyword2' => array('value' => $message, 'color' => '#73a68d') );
			...
			if (($message_type == TM_COMMISSION_ORDER_PAY) && empty($usernotice['commission_order_pay'])) 
			{
				...
				$msg = array( 'keyword1' => array('value' => (!(empty($tm['commission_order_paytitle'])) ? $tm['commission_order_paytitle'] : '团队付款通知')), 'keyword2' => array('value' => $message) );
			...
			if (($message_type == TM_COMMISSION_ORDER_FINISH) && empty($usernotice['commission_order_finish'])) 
			{
				...
				$message = str_replace('[订单金额]', $data['price'], $message);
				$message = str_replace('[商品详情]', $data['goods'], $message);
				$msg = array( 'keyword1' => array('value' => (!(empty($tm['commission_order_finishtitle'])) ? $tm['commission_order_finishtitle'] : '团队确认收货通知'), 'color' => '#73a68d'), 'keyword2' => array('value' => $message, 'color' => '#73a68d') );
		...
		protected function sendMore($touser, $tm, $tag, $msg, $datas) 
		{
			...
			$set = $this->getSet();
			$msgbk = $msg;
			$msgbk['keyword2']['value'] = str_replace('[团队层级]', $set['texts']['c1'], $msgbk['keyword2']['value']);
			$msgbk['keyword2']['value'] = str_replace('[团队]', $set['texts']['down'], $msgbk['keyword2']['value']);
			...
			foreach ($res as $key => $value ) 
			{
				...
				if ($key == 0) 
				{
					$msgbk['keyword2']['value'] = str_replace('[团队层级]', $set['texts']['c2'], $msgbk['keyword2']['value']);
					$msgbk['keyword2']['value'] = str_replace('[团队]', $set['texts']['c2'] . $set['texts']['down'], $msgbk['keyword2']['value']);
					if (($tag == 'commission_order_finish') || ($tag == 'commission_order_pay')) 
					{
						if ($datas['isagent']) 
						{
				...
				if ($key == 1) 
				{
					$msgbk['keyword2']['value'] = str_replace('[团队层级]', $set['texts']['c3'], $msgbk['keyword2']['value']);
					$msgbk['keyword2']['value'] = str_replace('[团队]', $set['texts']['c3'] . $set['texts']['down'], $msgbk['keyword2']['value']);
					if (($tag == 'commission_order_finish') || ($tag == 'commission_order_pay')) 
					{
						if ($datas['isagent']) 
						{
		...
		protected function sendMoreAdvanced($touser, $tm, $tag, $msg, $url, $datas) 
		{
			...
			$msgbk = $this->replaceArray($msgbk, '[团队层级]', $set['texts']['c1']);
			...
			foreach ($res as $key => $value ) 
			{
				if ($key == 0) 
				{
					...
					$msgbk = $this->replaceArray($msgbk, '[团队层级]', $set['texts']['c2']);
					if (($tag == 'commission_order_finish_advanced') || ($tag == 'commission_order_pay_advanced')) 
					{
				...
				if ($key == 1) 
				{
					...
					$msgbk = $this->replaceArray($msgbk, '[团队层级]', $set['texts']['c3']);
					if (($tag == 'commission_order_finish_advanced') || ($tag == 'commission_order_pay_advanced')) 
					{
