...
class PosterProcessor extends PluginProcessor 
{
	...
	private function responseSubscribe($obj) 
	{
		...
		if (empty($log) && ($openid != $qr['openid'])) 
		{
			...
			{
				m('member')->setCredit($openid, 'credit1', $poster['subcredit'], array(0, '扫码关注积分币+' . $poster['subcredit']));
			}
			...
			if (empty($reward_totle['reccredit_totle']) || (intval($reward_real['reccredit_totle']) <= intval($reward_totle['reccredit_totle']))) 
			{
				...
				{
					m('member')->setCredit($qr['openid'], 'credit1', $poster['reccredit'], array(0, '推荐扫码关注积分币+' . $poster['reccredit']));
				}
			}
						...
						if (!(empty($poster['bedown']))) 
						{
							if (empty($member['agentid'])) 
							{
								if (empty($member['fixagentid'])) 
								{
									...
									$author = p('author');
									$agentid_str = m('member')->getAgentidStr($qrmember['id'],$qrmember['agentid_str']);
									if ($author) 
									{
										$p->upgradeLevelByAgent($qrmember['id']);
										pdo_update('ewei_shop_member', array('agentid' => $qrmember['id'], 'agentid_str' => $agentid_str, 'childtime' => $time, 'authorid' => $authorid), array('id' => $member['id']));
									}
									else 
									{
										pdo_update('ewei_shop_member', array('agentid' => $qrmember['id'], 'agentid_str' => $agentid_str, 'childtime' => $time), array('id' => $member['id']));
									}
									if ($author) 
									{
