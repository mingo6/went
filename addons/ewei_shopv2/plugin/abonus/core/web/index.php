...
class Index_EweiShopV2Page extends PluginWebPage 
{
	public function main() 
	{
		...
		if (cv('abonus.agent')) 
		{
			headers(webUrl('abonus/agent'));
			exit();
			return;
		}
		if (cv('abonus.level')) 
		{
			headers(webUrl('abonus/level'));
			exit();
			return;
		}
		if (cv('abonus.bonus')) 
		{
			headers(webUrl('abonus/bonus'));
			exit();
			return;
		}
		if (cv('abonus.bonus.send')) 
		{
			headers(webUrl('abonus/bonus/send'));
			exit();
			return;
		}
		if (cv('abonus.notice')) 
		{
			headers(webUrl('abonus/notice'));
			exit();
			return;
		}
		if (cv('abonus.set')) 
		{
			headers(webUrl('abonus/set'));
			exit();
		}
	}
