	...
	public function getpage() 
	{
		...
		if (!(empty($_W['shopset']['shop']['indexrecommands']))) 
		{
			...
			if (!(empty($goodids))) 
			{
				...
				$indexrecommands = pdo_fetchall('select id, title, thumb, marketprice,ispresell,presellprice, productprice, minprice, total,isnodiscount,discounts from ' . tablename('ewei_shop_goods') . ' where id in( ' . $goodids . ' ) and uniacid=:uniacid and status=1 order by instr(\'' . $goodids . '\',id),displayorder desc', array(':uniacid' => $uniacid));
				foreach ($indexrecommands as $key => $value ) 
				{
					...
					$indexrecommands[$key]['memberprice'] = m('goods')->getMemberPrice($value,$level);
				}
			}
		}
