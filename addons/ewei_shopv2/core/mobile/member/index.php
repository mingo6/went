...
class Index_EweiShopV2Page extends MobileLoginPage 
{
	public function main() 
	{
		...
		// 测试 获取用户微信数据 START
		// if(!empty($_GET['checkmodel']) && $_GET['checkmodel'] == '7412369855230')
		// {
		// 	// $acc = WeiXinAccount::create($_W['acid']);
		// 	// $token = $acc->getAccessToken();
		// 	// $openid = 'odIvP0wslwBQWxRIVaI030yoO7fw';
		// 	// $url = "https://api.weixin.qq.com/cgi-bin/user/info?access_token={$token}&openid={$openid}&lang=zh_CN";
		// 	// $response = ihttp_get($url);
			
		// 	$sql = 'SELECT m.openid,ml.ordermoney FROM ' . tablename('ewei_shop_member') . ' as m LEFT JOIN  ' . tablename('ewei_shop_member_level') . ' as ml ON ml.id=m.level limit 800,200';
		...
		// 		// $url = "https://api.weixin.qq.com/cgi-bin/user/info?access_token={$token}&openid={$openid}&lang=zh_CN";
		// 		// $response = ihttp_get($url);
		// 		$response = $acc->fansQueryInfo($openid);
		// 		$all_user[ $val['openid'] ] = $response['unionid'];
		// 	}
		// 	$res = file_put_contents(ATTACHMENT_ROOT.'information5.txt', json_encode($all_user));
		// 	var_dump($res);
		// 	exit;
		// }
		// 测试 获取用户微信数据 END
		...
		if ($com_sign) 
		{
			...
			{
				$hassign = ((empty($_W['shopset']['trade']['credittext']) ? '健康币' : $_W['shopset']['trade']['credittext']));
