...
class Create_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function main() 
	{
		...
		if (!($packageid)) 
		{
			...
			foreach ($goods as &$g ) 
			{
				...
				{
					if(!isset($level['id']) && $level['id'] <= 0 && $member['level'] <= 0 && $member['agentid'] > 0){	//推荐优惠
						$isRecommend = true;
				...
				else if (empty($bargain_id) || $isRecommend) 
				{
	...
	public function caculate() 
	{
		...
		if (is_array($goodsarr)) 
		{
			...
			foreach ($goodsarr as &$g ) 
			{
				...
				{
					if(!isset($level['id']) && $level['id'] <= 0 && $member['agentid'] > 0){	//推荐优惠
						$isRecommend = true;
						$level = m('member')->getMinLevel();
					}
	...
	public function submit() 
	{
		...
		if (0 < $deductcredit) 
		{
			m('member')->setCredit($openid, 'credit1', -$deductcredit, array('0', $_W['shopset']['shop']['name'] . '购物积分币抵扣 消费积分币: ' . $deductcredit . ' 抵扣金额: ' . $deductmoney . ' 订单号: ' . $ordersn));
		}
		if (0 < $buyagainprice) 
		{
