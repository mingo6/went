...
class Pay_EweiShopV2Page extends MobileLoginPage 
{
	public function main() 
	{
		...
		if (!(empty($ispeerpay))) 
		{
			...
			{
				headers(mobileUrl('order/pay/peerpayshare', array('id' => $orderid)));
				exit();
			}
			else 
			{
		...
		if ($order['status'] == -1) 
		{
			headers(mobileUrl('order/detail', array('id' => $order['id'])));
			exit();
		}
		...
		{
			headers(mobileUrl('order/pay/complete', array('id' => $order['id'], 'type' => 'credit')));
			exit();
		}
		load()->model('payment');
	...
	public function complete() 
	{
		...
		if (1 <= $order['status']) 
		{
			...
			{
				headers(mobileUrl('order/pay/success', array('id' => $order['id'], 'result' => $pay_result ,'isminiprogram' => $_GPC['isminiprogram'])));
				exit();
			}
		}
	...
	public function success() 
	{
		...
		//解决小程序BUG.
		if($_GPC['isminiprogram'] && $_SESSION[EWEI_SHOPV2_PREFIX . '_order_pay_complete']){
			if($_SESSION[EWEI_SHOPV2_PREFIX . '_order_pay_complete'] >= 2){
				unset($_SESSION[EWEI_SHOPV2_PREFIX . '_order_pay_complete']);
			}else{
				$_SESSION[EWEI_SHOPV2_PREFIX . '_order_pay_complete']++;
			}
		}else{
			unset($_SESSION[EWEI_SHOPV2_PREFIX . '_order_pay_complete']);
		}
	...
	public function peerpay() 
	{
		...
		if (!(empty($peerpay))) 
		{
			headers(mobileUrl('order/pay/peerpayshare', array('id' => $peerpay)));
			exit();
		}
	...
	public function peerpayshare() 
	{
		...
		{
			headers(mobileUrl('order/pay/peerpaydetail', array('id' => $peerid)));
			exit();
		}
