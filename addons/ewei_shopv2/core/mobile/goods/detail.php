...
if(!class_exists('MobileMiniappPage')){
	if(file_exists(EWEI_SHOPV2_CORE . 'inc/page_mobile_miniapp.php')){
		require_once EWEI_SHOPV2_CORE . 'inc/page_mobile_miniapp.php';
	}else{
		echo 'MobileMiniappPage 类不存在';
	}
}
class Detail_EweiShopV2Page extends MobilePage 
{
	public function main() 
	{
		...
		if(!isset($level['id']) && $level['id'] <= 0 && $member['level'] <= 0 && $member['agentid'] > 0){	//是否有推荐价
			$level = m('member')->getMinLevel();
			$isRecommend = false;
		}
		if (empty($is_task_goods)) 
		{
			$memberprice = m('goods')->getMemberPrice($goods, $level);
			if($goods['minprice'] != $memberprice && isset($isRecommend)){
				$isRecommend = true;
			}
		}
	...
	public function shareQrcode(){
		...
		$mid = $_W['mid'];
		$id = $_GPC['id'];
		$isminiapp = $_GPC['isminiapp'];
		$shareUrl = mobileUrl('goods/detail', array('id'=>$id,'mid'=>$mid),true);
		if($isminiapp){	//小程序
			load()->func('wxapi');
			$account_api = new wxapi(MobileMiniappPage::$APPID,MobileMiniappPage::$APPSECRET);
			$token = $account_api->wxAccessToken();
			if(is_error($token)){
				show_json(1, $account_api->error['msg']);
			}
			$url = 'https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token='.$token;
			$scene = '20,' . $id . ',' . $mid;
			$data = '{"scene":"' . $scene . '","page":"pages/pay/success","width":"430"}';
			$result = $account_api->wxHttpsRequest($url,$data);
			$jsoninfo    = json_decode($result, true);
			if($jsoninfo['errcode'] > 0){
				show_json(1, $jsoninfo['errmsg']);
			}
			header('Content-type:image/png');
			echo $result;
		}else{
			require_once IA_ROOT . '/framework/library/qrcode/phpqrcode.php';
			QRcode::png($shareUrl, false, QR_ECLEVEL_L, 4);
		}
	}
