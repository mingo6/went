...
class Coupon_EweiShopV2ComModel extends ComModel 
{
	...
	public function payResult($logno) 
	{
		...
		if (empty($log['status'])) 
		{
			...
			{
				m('member')->setCredit($log['openid'], 'credit1', -$coupon['credit'], '购买优惠券扣除积分币 ' . $coupon['credit']);
	...
	public function useRechargeCoupon($log) 
	{
		...
		if (!(empty($backcredit))) 
		{
			...
			if (0 < $backcredit) 
			{
				...
				m('member')->setCredit($data['openid'], 'credit1', $backcredit, array(0, '充值优惠券返积分币'));
			}
		}
	...
	public function backConsumeCoupon($orderid) 
	{
		...
		if ($canback) 
		{
			...
			if (!(empty($backcredit))) 
			{
				...
				{
					$gives['credit'] = $backcredit;
					m('member')->setCredit($order['openid'], 'credit1', $backcredit, array(0, '充值优惠券返积分币'));
				}
			}
			if (!(empty($backmoney))) 
			{
