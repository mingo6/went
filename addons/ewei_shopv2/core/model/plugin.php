...
class Plugin_EweiShopV2Model 
{
    ...
    /***********20180420修改***********/
	public function getAllnew($iscom = false, $status = '')
	{
		global $_W;
		$plugins = '';
		if ($status !== '') 
		{
			$status = 'and a.status = ' . intval($status);
		}
		if ($iscom) 
		{
			$plugins = m('cache')->getArray('coms2', 'global');
			if (!empty($plugins))
			{
				$plugins = pdo_fetchall('select a.* from ' . tablename('ewei_shop_plugin') . ' as a left join '.tablename('ewei_shop_plugin_uniacid').' as b on a.id = b.plugin_id where a.iscom=1 and a.deprecated=0 and b.uniacid='.$_W['uniacid'].'  order by a.displayorder asc');
				m('cache')->set('coms2', $plugins, 'global');
			}
		}
		else 
		{
			...
			if (!empty($plugins))
			{
				$plugins = pdo_fetchall('select a.* from ' . tablename('ewei_shop_plugin') . ' as a left join '.tablename('ewei_shop_plugin_uniacid').' as b on a.id = b.plugin_id where a.iscom=0 and a.deprecated=0 and b.uniacid='.$_W['uniacid'].'  order by a.displayorder asc');
				m('cache')->set('plugins2', $plugins, 'global');
			}
		}
		return $plugins;
