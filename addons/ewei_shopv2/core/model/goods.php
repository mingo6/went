...
class Goods_EweiShopV2Model 
{
	...
	public function getList($args = array()) 
	{
		...
		if (!($random)) 
		{
			$sql = 'SELECT id,title,thumb,marketprice,productprice,minprice,maxprice,isdiscount,discounts,isnodiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,hasoption' . "\r\n" . '            FROM ' . tablename('ewei_shop_goods') . ' where 1 ' . $condition . ' ORDER BY ' . $order . ' ' . $orderby . ' LIMIT ' . (($page - 1) * $pagesize) . ',' . $pagesize;
		...
		if(!isset($level['id']) || $level['id'] <= 0){
			$level = pdo_fetch('select id,levelname,discount from ' . tablename('ewei_shop_member_level') . ' WHERE uniacid=:uniacid ORDER BY ordermoney ASC limit 1', array(':uniacid' => $_W['uniacid']));
		}
		if(!empty($list)){
			foreach ($list AS $key => &$goods){
				$goods['memberprice'] = $this->getMemberPrice($goods,$level);
			}
		}
	 ...
	 * 获取推荐价格
	 */
	public function getRecommendPrice($goods){
		$recommendRate = $this->getRecommendRate();
		if($recommendRate <= 0){
			return 1;
		}
		$options = m('goods')->getOptions($goods);
		if(!empty($options)){
			...
			foreach ($options as $option) 
			{
				$optionprice = $option['marketprice'] * $recommendRate;
				$marketprice[] = $optionprice;
			}
			$minprice = min($marketprice);
			$maxprice = max($marketprice);
			$memberprice = array('minprice' => (double) ($minprice * $recommendRate), 'maxprice' => (double) ($maxprice * $recommendRate));
			if ($memberprice['minprice'] < $memberprice['maxprice']) 
			{
				$memberprice = $memberprice['minprice'] . '~' . $memberprice['maxprice'];
			}
			...
			$memberprice = $goods['minprice'] * $recommendRate;
		}
		return $memberprice;
	}

	public function getRecommendRate(){
