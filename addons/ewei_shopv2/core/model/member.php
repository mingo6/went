...
class Member_EweiShopV2Model 
{
	...
	public function getMinLevel(){
		global $_W;
		$level = pdo_fetch('select * from ' . tablename('ewei_shop_member_level') . ' where uniacid=:uniacid ORDER BY ordermoney ASC limit 1', array(':uniacid' => $_W['uniacid']));
		return $level;
	}
	...
	public function setRechargeCredit($openid = '', $money = 0) 
	{
		...
		if (0 < $credit) 
		{
			$this->setCredit($openid, 'credit1', $credit, array(0, $set['shop']['name'] . '会员充值积分币:credit2:' . $credit));
		}
	}
	 ...
	 * 获取当前用户所有上级。
	 */
	public function getAgentidStr($arentAgentid,$arentAgentidStr)
	{
		if(empty($arentAgentid))
		{
			return '';
		}
		if(empty($arentAgentidStr))
		{
			$agentid_str = $arentAgentid;
		}
		else
		{
			$agentid_str = $arentAgentidStr . ',' . $arentAgentid;
		}
		return $agentid_str;
	}

	public function updateAgentidStr($id , $agentidStr, $oldAgentid, $newAgentid)
	{
		global $_W;
		if($oldAgentid == $newAgentid)
		{
			return error(-1, '您已经是此用户的上级！');
		}
		$newParentInfo = pdo_fetch('select id,agentid,agentid_str from ' . tablename('ewei_shop_member') . ' where uniacid=:uniacid and id=:id and status>=1 Limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $newAgentid));
		if(empty($newParentInfo))
		{
			return error(-1, '上级用户不存在!(agentid:' . $newAgentid . ')');
		}
		if($newParentInfo['agentid'] == $id)
		{
			return error(-1, $newAgentid.'已经是你的下级，你不能成为它的下级。');
		}
		//判断用户是否是你的下级。
		if(!empty($newParentInfo['agentid_str']))
		{
			$arrAgentidStr = explode(',',$newParentInfo['agentid_str']);
			if(in_array($id,$arrAgentidStr)){
				return error(-1, $newAgentid.'已经是你的下级，你不能成为它的下级。');
			}
		}
		$sql = 'SELECT id,agentid,agentid_str FROM '.tablename('ewei_shop_member').' WHERE uniacid=:uniacid AND (FIND_IN_SET(:id,agentid_str) OR agentid=:id OR id=:id)';
		$lowerLevelList = pdo_fetchall($sql,array(':uniacid' => $_W['uniacid'],':id'=>$id));
		if(!empty($lowerLevelList))
		{
			$parentAgentidStr = explode(',',m('member')->getAgentidStr($newParentInfo['id'],$newParentInfo['agentid_str']));
			foreach($lowerLevelList AS $lowerLevelKey=>$lowerLevelInfo)
			{
				if($lowerLevelInfo['agentid'] == $oldAgentid)
				{
					$lowerLevelInfo['agentid'] = $newAgentid;
				}
				$tempAgentidStrArr = $this->subAgentidStr($tempAgentidStrArr,$lowerLevelInfo['agentid_str'],$oldAgentid);
				$tempAgentidStrArr = array_merge($parentAgentidStr,$tempAgentidStrArr);
				if(!empty($tempAgentidStrArr))
				{
					$lastAgentId = end($tempAgentidStrArr);
					if($lastAgentId != $lowerLevelInfo['agentid'])
					{
						$tempAgentidStrArr[] = $lowerLevelInfo['agentid'];
					}
					// $tempAgentidStrArr = array_unique($tempAgentidStrArr);
					$tempAgentidStrArr = array_reverse($tempAgentidStrArr);
					$tempAgentidStrArr = array_flip($tempAgentidStrArr);
					$tempAgentidStrArr = array_flip($tempAgentidStrArr);
					$tempAgentidStrArr = array_reverse($tempAgentidStrArr);
					$agentidStr = implode(',',$tempAgentidStrArr);
				}
				else
				{
					$agentidStr = '';
				}
				pdo_update('ewei_shop_member',array('agentid_str'=>$agentidStr),array('uniacid'=>$_W['uniacid'],'id'=>$lowerLevelInfo['id']));
			}
		}
		return error(0,'成功');
	}
	...
	//清除上级
	public function clearAgentidStr($id)
	{
		global $_W;
		$sql = 'SELECT id,agentid,agentid_str FROM '.tablename('ewei_shop_member').' WHERE uniacid=:uniacid AND id != :id AND (FIND_IN_SET(:id,agentid_str) OR agentid=:id OR id=:id)';
		$lowerLevelList = pdo_fetchall($sql,array(':uniacid' => $_W['uniacid'],':id'=>$id));
		if(!empty($lowerLevelList))
		{
			foreach($lowerLevelList AS $lowerLevelKey=>$lowerLevelInfo)
			{
				$data = array();
				if($lowerLevelInfo['agentid'] == $id)
				{
					$data['agentid'] = 0;
				}
				$tempAgentidStrArr = $this->subAgentidStr($tempAgentidStrArr,$lowerLevelInfo['agentid_str'],$id);
				if(!empty($tempAgentidStrArr))
				{
					$agentidStr = implode(',',$tempAgentidStrArr);
				}
				...
				$data['agentid_str'] = $agentidStr;
				pdo_update('ewei_shop_member',$data,array('uniacid'=>$_W['uniacid'],'id'=>$lowerLevelInfo['id']));
			}
		}
		return error(0,'清除成功！');
	}

	public function subAgentidStr($array,$agentidStr,$oldAgentid)
	{
		if(!empty($agentidStr))
		{
			$array = explode(',',$agentidStr);
			$index = array_search($oldAgentid,$array);
		}
		else
		{
			$array = array();
