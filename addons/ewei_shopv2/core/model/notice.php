...
class Notice_EweiShopV2Model 
{
	...
	public function sendTeamMessage($orderid = '0', $delRefund = false) 
	{
		...
		$orderpricestr = ' ¥' . $price . '元 (包含运费: ¥' . $order['freight'] . '元，积分币抵扣: ¥' . $order['creditmoney'] . '元)';
	...
	public function sendMemberPointChange($openid, $pointchange = 0, $changetype = 0) 
	{
		...
		$credittext = ((empty($_W['shopset']['trade']['credittext']) ? '积分币' : $_W['shopset']['trade']['credittext']));
		$pointtext = '';
		$pointcolor = '';
		if ($changetype == 0) 
		{
		...
		$datas = array( array('name' => '商城名称', 'value' => $_W['shopset']['shop']['name']), array('name' => '粉丝昵称', 'value' => $member['nickname']), array('name' => '积分币变动', 'value' => $pointtext), array('name' => '赠送时间', 'value' => date('Y-m-d H:i', time())), array('name' => '积分币余额', 'value' => (double) $member['credit1'] . $credittext) );
		$remark = "\n" . '[商城名称]感谢您的支持，如有疑问请联系在线客服。';
		$text = '亲爱的[粉丝昵称]， 您的' . $credittext . '发生变动，具体内容如下：' . "\n\n" . '积分币变动：[积分币变动]' . "\n" . '变动时间：[赠送时间]' . "\n" . '充值方式：管理员后台处理' . "\n" . '当前积分币余额：[积分币余额] ' . "\n" . $remark;
		$message = array( 'first' => array('value' => '亲爱的' . $member['nickname'] . '，您的' . $credittext . '发生变动，具体如下:', 'color' => '#ff0000'), 'keyword1' => array('title' => '获得时间', 'value' => date('Y-m-d H:i', time()), 'color' => '#000000'), 'keyword2' => array('title' => '获得积分币', 'value' => $pointtext, 'color' => $pointcolor), 'keyword3' => array('title' => '获得原因', 'value' => '管理员后台手动处理', 'color' => '#000000'), 'keyword4' => array('title' => '当前积分币', 'value' => (double) $member['credit1'] . $credittext, 'color' => '#ff0000'), 'remark' => array('value' => "\n" . $_W['shopset']['shop']['name'] . '感谢您的支持，如有疑问请联系在线客服。', 'color' => '#000000') );
	...
	public function sendMemberLogMessage($log_id = '', $channel = 0, $isback = false) 
	{
		...
		$datas[] = array('name' => '积分币余额', 'value' => $member['credit1'] . $credittext);
