...
class Order_EweiShopV2Model 
{
	...
	public function payResult($params) 
	{
		...
		if (strpos($params['tid'], '_miniapp') !== false) 
		{
			$params['tid'] = str_replace('_miniapp', '', $params['tid']);
		}
		$ordersn = $params['tid'];
	...
	public function setStocksAndCredits($orderid = '', $type = 0) 
	{
		...
		if (0 < $credits) 
		{
			...
			if ($type == 1) 
			{
				m('member')->setCredit($order['openid'], 'credit1', $credits, array(0, $shopset['name'] . '购物积分币 订单号: ' . $order['ordersn']));
			...
			else if ($type == 2) 
			{
				if (1 <= $order['status']) 
				{
					m('member')->setCredit($order['openid'], 'credit1', -$credits, array(0, $shopset['name'] . '购物取消订单扣除积分币 订单号: ' . $order['ordersn']));
	...
	public function getTotals($merch = 0) 
	{
		...
		$totals['status6'] = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename('ewei_shop_apply_after_sale') . '' . ' WHERE uniacid = :uniacid AND status=:status', array(':uniacid' => $_W['uniacid'], ':status' => 0));
		return $totals;
	}
	...
	public function getGoodsRecommendPrice($g ,$typ = 0){
		if ($type == 0) 
		{
			$total = $g['total'];
		}
		...
		$gprice = $g['marketprice'] * $total;
		$rate = m('goods')->getRecommendRate();
		if($rate <= 0){
			$rate = 1;
		}
		$realprice = $gprice * $rate;
		$recommendPrice = $gprice - $realprice;
		$unitprice = round($realprice / $total,2);

		return array('price'=>$realprice,'unitprice'=>$unitprice,'discountprice'=>$recommendPrice,'discounttype'=>0);
	}
