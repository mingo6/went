...
class Upgrade_EweiShopV2Page extends SystemPage 
{
	public function main() 
	{
		...
		$updatehost = 'http://update.yxk17.cn/update.php';
		...
		if($_GPC['op'] == 'display'){
			...
			$ver2 = $ver+0.1;
			$chosturl = $updatehost . '?a=chanage&v=' . $ver2. '&u=' . $hosturl;		
		...
		elseif($_GPC['op'] == 'update'){
			...
			if (strstr($updatenowinfo, 'zip')){		
					...
					if($isgot){				
						...
						$newver = "<?php if(!defined('IN_IA')) {exit('Access Denied');}if(!defined('EWEI_SHOPV2_VERSION')) {define('EWEI_SHOPV2_VERSION', '{$lastver}');}if(!defined('EWEI_SHOPV2_VERSION')) {define('EWEI_SHOPV2_VERSION', '20170529123456');}";
						$f = fopen(IA_ROOT . '/addons/ewei_shopv2/version.php','w+');
						fwrite($f,$newver);
						fclose($f);
						@unlink($updatedir . '/update.sql');
						$this->deldir($updatedir);
						
						message('已将系统更新到最新版本！',webUrl('system/auth/upgrade'),'success');
					}
					else{
					...
					message('小七宝提醒您:请检查授权状态或联系小七授权！',referer(),'error');
			}
		}
