...
class Index_EweiShopV2Page extends WebPage 
{
	public function main() 
	{
		...
		$list = pdo_fetchall('SELECT gp.*,p.identity,p.name,p.category,p.version,p.author,p.status FROM ' . tablename('ewei_shop_system_plugingrant_plugin') . ' as gp' . "\n" . '                left join ' . tablename('ewei_shop_plugin') . ' as p on p.id = gp.pluginid' . "\n" . '                WHERE 1 ' . $condition . ' ORDER BY ' . $sortcondition . ' LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
	...
	public function detail() 
	{
		...
		{
			$item = pdo_fetch('SELECT gp.*,p.identity,p.name,p.category,p.version,p.author,p.status,p.thumb,p.desc,p.isv2 FROM ' . tablename('ewei_shop_system_plugingrant_plugin') . ' gp' . "\n" . '                left join ' . tablename('ewei_shop_plugin') . ' as p on p.id = gp.pluginid ' . "\n" . '                WHERE gp.id =:id limit 1', array(':id' => $id));
			if (!(empty($item))) 
			{
	...
	public function create() 
	{
		...
		{
			$item = pdo_fetch('SELECT gp.*,p.identity,p.name,p.category,p.version,p.author,p.status,p.thumb,p.desc FROM ' . tablename('ewei_shop_system_plugingrant_plugin') . ' gp' . "\n" . '                left join ' . tablename('ewei_shop_plugin') . ' as p on p.id = gp.pluginid ' . "\n" . '                WHERE gp.id =:id and state = 1 limit 1', array(':id' => $id));
			$item['data'] = unserialize($item['data']);
			$title = $item['name'];
		}
		include $this->template();
