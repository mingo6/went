...
class List_EweiShopV2Page extends WebPage 
{
		...
		if ($_W['ispost']) 
		{
			...
			if ($hascommission) 
			{
				if (cv('commission.agent.edit')) 
				{
					...
					if($adata['issuperagent'] == 1){
						if($member['superagenttime'] <= 0){
							$adata['superagenttime'] = time();
						}
					}
					if (!(empty($adata))) 
					{
						if ($adata['agentid'] != $member['agentid']) 
						{
							...
							{
								$error = m('member')->updateAgentidStr($member['id'], $member['agentid_str'], $member['agentid'],$adata['agentid']);
								if(is_error($error)){
									show_json(0, $error['message']);
								}
	...
	public function delete() 
	{
		...
		$members = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_member') . ' WHERE id in( ' . $id . ' ) AND uniacid=:uniacid',array(':uniacid'=>$_W['uniacid']));
		foreach ($members as $member ) 
		{
			pdo_delete('ewei_shop_member', array('id' => $member['id']));
			//重新计算agentid_str字段的值
			m('member')->clearAgentidStr($member['id']);
