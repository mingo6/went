...
class Credit_EweiShopV2Page extends WebPage 
{
	protected function main($type = 'credit1') 
	{
		...
		if ($_GPC['export'] == 1) 
		{
			if ($_GPC['type'] == 1) 
			{
				plog('finance.credit.credit1.export', '导出积分币明细');
			}
			else 
			{
			...
			$columns[] = array('title' => ($type == 'credit1' ? '积分币变化' : '余额变化'), 'field' => 'num', 'width' => 12);
			...
			m('excel')->export($list, array('title' => (($type == 'credit1' ? '会员积分币明细-' : '会员余额明细')) . date('Y-m-d-H-i', time()), 'columns' => $columns));
		}
