	...
	class CommissionModel extends PluginModel
	{
					...
					if (empty($become_child)) {
						if (empty($member['fixagentid'])) {
							...
							if (p('author')) {
								...
								pdo_update('ewei_shop_member', array('agentid' => $parent['id'], 'agentid_str' => $agentid_str, 'childtime' => $time, 'authorid' => $authorid), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
							}
							 else {
								pdo_update('ewei_shop_member', array('agentid' => $parent['id'], 'agentid_str' => $agentid_str, 'childtime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
							}
		...
		public function sendMessage($openid = '', $data = array(), $message_type = '')
		{
			...
			if (($message_type == TM_COMMISSION_AGENT_NEW) && empty($usernotice['commission_agent_new'])) {
				...
				$msg = array(
					'keyword1' => array('value' => (!empty($tm['commission_agent_newtitle']) ? $tm['commission_agent_newtitle'] : '新增团队通知'), 'color' => '#73a68d'),
			...
			if (($message_type == TM_COMMISSION_ORDER_PAY) && empty($usernotice['commission_order_pay'])) {
				...
				$msg = array(
					'keyword1' => array('value' => (!empty($tm['commission_order_paytitle']) ? $tm['commission_order_paytitle'] : '团队付款通知')),
			...
			if (($message_type == TM_COMMISSION_ORDER_FINISH) && empty($usernotice['commission_order_finish'])) {
				...
				$message = str_replace('[商品详情]', $data['goods'], $message);
				$msg = array(
					'keyword1' => array('value' => (!empty($tm['commission_order_finishtitle']) ? $tm['commission_order_finishtitle'] : '团队确认收货通知'), 'color' => '#73a68d'),
					'keyword2' => array('value' => $message, 'color' => '#73a68d')
					);
