...
if(!empty($input)){
	$bool = file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/data/logs/wxpay_'.date('Y-m-d').'.log',"时间:".date('Y-m-d H:i:s')."\r\n".var_export($input,true) . "\r\n",FILE_APPEND);
}
...
class EweiShopWechatPay 
{
	...
	public function water()
	{
		...
		$orderInfo = pdo_fetch('select id, status from ' . tablename('ewei_shop_waterorder') . ' where order_sn=:orderno limit 1', array(':orderno' => $orderno));
		if(!empty($orderInfo) && $orderInfo['status'] < 2){
			pdo_update('ewei_shop_waterorder', array('pay_type' => 'wechat', 'status' => 2, 'is_pay' => 1, 'pay_time' => time() ), array('id' => $orderInfo['id']));
		}
		if (p('water') && method_exists(p('water'), 'payResult')) 
		{
			p('groups')->payResult($orderno, 'wechat', ($this->isapp ? true : false));
		}
	}
	...
	public function publicMethod() 
	{
		...
		if (empty($payment['is_new']) || ($this->get['trade_type'] == 'APP')) 
		{
			...
			if (is_array($this->setting['payment']) || ($this->set['weixin_jie'] == 1) || ($this->set['weixin_sub'] == 1) || ($this->set['weixin_jie_sub'] == 1) || ($this->get['trade_type'] == 'APP')) 
			{
				...
				$this->isminiapp = strpos($this->get['out_trade_no'],'_miniapp') !== false;
	...
	private function setRealOrderSn(){
		if (strexists($this->get['out_trade_no'], '_miniapp') !== false) 
		{
			$this->get['out_trade_no'] = str_replace('_miniapp', '', $this->get['out_trade_no']);
		}
	}
}
?>