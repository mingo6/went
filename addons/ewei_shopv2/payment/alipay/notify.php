...
class aliApy 
{
	...
	public function batch_refund_notify_rf($id, $money) 
	{
		...
		if (0 < $credits) 
		{
			m('member')->setCredit($item['openid'], 'credit1', -$credits, array(0, '退款扣除购物赠送积分币: ' . $credits . ' 订单号: ' . $item['ordersn']));
		}
		if (0 < $item['deductcredit']) 
		{
			m('member')->setCredit($item['openid'], 'credit1', $item['deductcredit'], array('0', '购物返还抵扣积分币 积分币: ' . $item['deductcredit'] . ' 抵扣金额: ' . $item['deductprice'] . ' 订单号: ' . $item['ordersn']));
		}
		if (!(empty($refundtype))) 
		{
			if ($money < 0) 
