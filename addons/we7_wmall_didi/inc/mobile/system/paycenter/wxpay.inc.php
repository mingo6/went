...
if( $_GPC["done"] == "1" ) 
{
    ...
    if( !empty($log) ) 
    {
        ...
        else
        {
            ...
            imessage("用户已支付成功,服务器没有接受到微信官方的支付成功推送。这个问题不是模块的问题,可能是环境配置问题。单号：" . $record["order_id"] . ",类型：" . $record["order_type"], "", "error");
        }
...
$record = pdo_get('tiny_wmall_paylog', array('uniacid' => $_W['uniacid'], 'id' => $log['plid']));
if(empty($record)){
    exit( "订单不存在." );
}
...
<script>
    function cancelPay(){
        var url = '<?php echo imurl('system/paycenter/pay', array('id' => $record['order_id'], 'order_type' => $record['order_type'], 'type' => 1));?>';
        location.href = url;
    }
</script>