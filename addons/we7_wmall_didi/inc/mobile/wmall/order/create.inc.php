...
$store = store_fetch($sid, array('agentid', 'id', 'cid', 'is_rest', 'title', 'logo', 'location_x', 'location_y', 'invoice_status', 'delivery_type', 'delivery_mode', 'delivery_price', 'delivery_fee_mode', 'delivery_areas', 'delivery_time', 'delivery_free_price', 'pack_price', 'delivery_within_days', 'delivery_reserve_days', 'order_note', 'data', 'not_in_serve_radius', 'serve_radius', 'region_id'));
if(is_error($store)) {
...
if($ta == 'index') {
	...
	$address['region_name'] = '无';
	if($address['region_id'] > 0){
		$regionInfo = pdo_fetch('SELECT `name` FROM ' . tablename("tiny_wmall_region") . ' WHERE id = :region_id', array('region_id' => $address['region_id']));
		if(!empty($regionInfo)){
			$address['region_name'] = $regionInfo['name'];
		}
	}
	...
	$send_diff = 0;
	if($send_price > ($cart['price'] + $cart['box_price'])) {
...
if($ta == 'submit') {
	$callback =  dirname($_W['siteurl']) . '/' . imurl('wmall/order/create/index', array('sid' => $_GPC['sid']));	
	if(!$_W['member']['mobile']){
		imessage(error(-1, '请绑定手机号码！'), imurl('wmall/member/profile/bind', array('callback' => $callback)), 'ajax');
	}
	if($_W['member']['region_id'] <= 0){
		imessage(error(-1, '请绑定区域！'), imurl('wmall/member/profile/edit', array('type' => 'region' ,'callback' => $callback)), 'ajax');
	}
	if($store['region_id'] != $_W['member']['region_id']){
		imessage(error(-1, '请重新下单！'), imurl('wmall/store/goods'), 'ajax');
	}
	if(!$_W['isajax']) {
	...
	$order['region_id'] = $_W['member']['region_id'] ? $_W['member']['region_id'] : $address['region_id'] ;
	if($order['region_id'] <= 0){
		$order['region_id'] = intval($_GPC['region_id']);
	}
	//区域判断
	if($order['region_id'] <= 0){
		imessage(error(-1, '区域不能为空！'), '', 'ajax');
	}
	$regionInfo = pdo_get('tiny_wmall_region', array('uniacid' => $_W['uniacid'], 'id' => $order['region_id']), array('id'));
	if(!$regionInfo){
