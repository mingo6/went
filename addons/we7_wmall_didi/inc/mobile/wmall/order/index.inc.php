...
if( $ta == "cancel" ) 
{
    ...
    if($order['status'] > 4){
        imessage(error(-1, "此订单不可取消"), "", "ajax");
    }

    if( ($order["status"] != 1 && $config_order['cancel_after_handle'] == 0)) 
    {
        imessage(error(-1, "商户已接单,如需取消订单请联系商户处理"), "", "ajax");
    }

    if( !$order["is_pay"] || $config_order['cancel_after_handle'] == 1 ) 
    {
