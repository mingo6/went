...
if( $ta == "post" ) 
{
    ...
    if( $_W["ispost"] ) 
    {
        ...
        $_GPC['region_id'] = $_W['member']['region_id'];
        $data = array( "uniacid" => $_W["uniacid"], "uid" => $_W["member"]["uid"], "realname" => trim($_GPC["realname"]), "sex" => trim($_GPC["sex"]), "mobile" => trim($_GPC["mobile"]), "address" => trim($_GPC["address"]), "number" => trim($_GPC["number"]), "location_x" => floatval($_GPC["location_x"]), "location_y" => floatval($_GPC["location_y"]), "type" => 1, 'region_id' => intval($_GPC['region_id']) );
        
        /* if($data['region_id'] <= 0){
            imessage(error(-1, "区域不能为空！"), "", "ajax");
        }
        $regionInfo = pdo_get('tiny_wmall_region', array("uniacid" => $_W["uniacid"], "id" => $data['region_id']), array('id'));
        if(!$regionInfo){
        ...
        /* if($_W['member']['region_id'] <= 0){
            pdo_update("tiny_wmall_members", array('region_id' => $data['region_id']), array( "uid" => $_W['member']['uid'], "uniacid" => $_W["uniacid"] ));
        } */
