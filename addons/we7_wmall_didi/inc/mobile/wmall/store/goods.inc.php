...
if(empty($_W['member']) || empty($_W['member']['mobile'])){
    imessage(false, imurl("wmall/member/profile/bind", array(), true));
}
if(!empty($_W['member']) && $_W['member']['region_id'] > 0 && $_W['member']['region_id'] != $store['region_id']){
    $tempStore = store_fetch_by_region_id($_W['member']['region_id']);
    if(is_error($tempStore)){
        //修改用户区域
        imessage('您当前区域没有店铺，请到个人中心修改区域！', imurl("wmall/member/profile/edit", array('type' => 'region')));
    }else{
        imessage(false, imurl("wmall/store/goods", array( "sid" => $tempStore['id'] )));
    }
}
...
if( is_error($store) ) 
{
    if($_W['member']['region_id'] > 0){
        pdo_update('tiny_wmall_member', array('region_id' => 0), array('uid' => $_W['member']['uid']));
    }
