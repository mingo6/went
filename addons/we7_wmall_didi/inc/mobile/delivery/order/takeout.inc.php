...
if(empty($_W['deliveryer']['perm_takeout'])) {
	imessage('您没有订单的配送权限，请联系管理员授权', imurl('wmall/member/mine'), 'error');
}

if($ta == 'list') {
	$_W['page']['title'] = '订单列表';
	$status = isset($_GPC['status']) ? intval($_GPC['status']) : 3;
	$can_collect_order = getCanCollectOrder($config_takeout);
	$data = getOrderList($status, $_deliveryer, $config_takeout);
	$orders = $data['orders'];
	$min = $data['min'];
	$stores = $data['stores'];
	...
	//待抢
	$num3_condition = " WHERE uniacid = :uniacid and agentid = :agentid and {$_W['deliveryer']['work_status']} and delivery_status = :status AND region_id = :region_id";
	...
	$num3 = pdo_fetch('select count(*) as num from ' . tablename('tiny_wmall_order') . $num3_condition, array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid'], ':status' => 3, ':region_id' => $_W['deliveryer']['region_id']));
	//待取
...
if($ta == 'more') {
	...
	$data = getOrderList($status, $_deliveryer, $config_takeout);
	$respon = array('errno' => 0, 'message' => $data['orders'], 'min' => $data['min']);
...
if($ta == 'collect') {
	...
	imessage(error(0, '接单成功'), referer(), 'ajax');
}
...
if($ta == 'notice') {
	...
	$return = order_status_notice($id, 'delivery_notice', $content);
	if(!$return){
		imessage(error(-1, '通知失败'), referer(), 'ajax');
	}
...
elseif($ta == 'direct_transfer_reply') {
	$id = intval($_GPC['id']);
...
function getCanCollectOrder($config_takeout)
{
	$can_collect_order = 1;
	if($config_takeout['order']['dispatch_mode'] != 1 && !$config_takeout['order']['can_collect_order']) {
		$can_collect_order = 0;
	}
	return $can_collect_order;
}

function getOrderList($status, $_deliveryer, $config_takeout)
{
	global $_W,$_GPC;
	...
	$can_collect_order = getCanCollectOrder($config_takeout);
	if($status == 3) {
		$condition .= " and {$_W['deliveryer']['work_status']} and delivery_status = :status AND region_id = :region_id";
		$params[':status'] = $status;
		$params[':region_id'] = $_W['deliveryer']['region_id'];
		if($_W['deliveryer']['perm_takeout'] == 1) {
	...
	$sql = 'SELECT id,order_plateform,serial_sn, addtime, is_pay, pay_type,order_type, `status`, username, mobile, `address`, `number`,distance, delivery_status,deliveryer_id,plateform_deliveryer_fee, delivery_type, delivery_fee, delivery_time,`sid`, num, final_fee, delivery_collect_type, transfer_delivery_status FROM ' . tablename('tiny_wmall_order') . $condition;
	...
	if(!empty($orders)) {
		...
		foreach($orders as &$da) {
			...
			//ajax需要数据
			$da['addtime_cn'] = date('Y-m-d H:i:s', $da['addtime']);
			$da['status_color'] = $delivery_status[$da['delivery_status']]['color'];
			$da['status_cn'] = $delivery_status[$da['delivery_status']]['text'];
			$da['store'] = pdo_fetch('select address, telephone from ' . tablename('tiny_wmall_store') . ' where uniacid = :uniacid and id = :sid', array(':uniacid' => $_W['uniacid'], ':sid' => $da['sid']));
			$label = order_timeout_remind($da['id']);
			$da['label'] = $label;
