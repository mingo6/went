...
if(empty($deliveryer['is_takeout'])) {
	message(ierror(-1, '您没有平台送水的配送权限，请联系管理员授权'), '', 'ajax');
}
...
if($op == 'list') {
	...
	$data = array(
		...
		'can_collect_order_cn' => $can_collect_order == 1 ? '' : '当前调度模式不允许抢单,请等待管理员/系统派单',
	);
...
elseif($op == 'detail') {
	...
	$order['delivery_assign_time_cn'] = $order['delivery_instore_time_cn'] = $order['delivery_success_time_cn'] = $order['delivery_takegoods_time_cn'] = array(
		'day' => '未知',
		'time' => '未知',
	);
	if(!empty($order['delivery_assign_time'])) {
		$order['delivery_assign_time_cn'] = array(
			'day' => date('m-d', $order['delivery_assign_time']),
			'time' => date('H:i', $order['delivery_assign_time']),
		);
	}
	if(!empty($order['delivery_instore_time'])) {
		$order['delivery_instore_time_cn'] = array(
			'day' => date('m-d', $order['delivery_instore_time']),
			'time' => date('H:i', $order['delivery_instore_time']),
		);
	}
	if(!empty($order['delivery_success_time'])) {
		$order['delivery_success_time_cn'] = array(
			'day' => date('m-d', $order['delivery_success_time']),
			'time' => date('H:i', $order['delivery_success_time']),
		);
	}
	if(!empty($order['delivery_takegoods_time'])) {
		$order['delivery_takegoods_time_cn'] = array(
			'day' => date('m-d', $order['delivery_takegoods_time']),
			'time' => date('H:i', $order['delivery_takegoods_time']),
		);
	}
...
elseif($op == 'direct_transfer_begin') {
	...
	$extra = array(
		'from_deliveryer_id' => $deliveryer['id'],
		'to_deliveryer_id' => $deliveryer_id,
		'note' => $reason
	);
...
elseif($op == 'direct_transfer_reply') {
	$id = intval($_GPC['id']);
