...
if( $op == "index" ) 
{
    ...
    $channels = array( "eleme" => "饿了么", "meituan" => "美团", "wxapp" => "小程序", "wap" => "H5", "h5app" => "用户APP", "plateformCreate" => "后台创建" );
    ...
    if( $_W["isajax"] ) 
    {
        $chart = array( "fields" => array( "total_success_order", "order_eleme", "order_meituan", "order_wxapp", "order_wap", "order_h5app", "order_plateformCreate" ), "titles" => array( "有效订单量", "饿了么订单", "美团订单", "小程序订单", "H5订单", "用户APP订单", "后台创建订单" ) );
        $i = $starttime;
        while( $i <= $endtime ) 
        {
            $chart["days"][] = $i;
