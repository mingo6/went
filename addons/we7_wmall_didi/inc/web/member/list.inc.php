...
if( $op == "list" ) 
{
    $condition = " AS m where m.uniacid = :uniacid";
    ...
    if( !empty($key) ) 
    {
        ...
        if( $key == "success_30" ) 
        {
            $condition .= " and m.success_last_time >= :time";
        }
        else
        {
            if( $key == "noorder_30" ) 
            ...
            else
            {
                if( $key == "cancel_30" ) 
                {
                    $condition .= " and m.cancel_last_time >= :time";
                }
    ...
    if( 0 < $groupid ) 
    {
        $condition .= " and m.groupid = :groupid";
    ...
    if( !empty($keyword) ) 
    {
        $condition .= " and (m.uid = :uid or m.realname like :keyword or m.mobile like :keyword or m.nickname like :keyword)";
        $params[":uid"] = $keyword;
    ...
    if( !empty($data) ) 
    {
        foreach( $data as &$val ) 
        {
            ...
            $val['region'] = '未选择区域';
            if($val['region_id'] > 0){
                $regionInfo = pdo_get('tiny_wmall_region', array('id' => $val['region_id']));
                if($regionInfo){
                    $val['region'] = $regionInfo['name'];
                }
            }
            unset($val);
        }
...
if( $op == "del" ) 
{
    ...
    {
        imessage(error(0, "用户不存在"), referer(), "ajax");
    }
