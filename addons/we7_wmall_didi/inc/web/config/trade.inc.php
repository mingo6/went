...
if( $op == "recharge" ) 
{
    ...
    if( $_W["ispost"] ) 
    {
        ...
        $_GPC['invoice_fee_scale'] = min(100, abs(intval($_GPC['invoice_fee_scale'])));
        $recharges = array( "status" => intval($_GPC["status"]), "items" => $recharges, 'invoice_fee_scale' => $_GPC['invoice_fee_scale'], 'added_invoices_fee_scale' => $_GPC['added_invoices_fee_scale'] );
