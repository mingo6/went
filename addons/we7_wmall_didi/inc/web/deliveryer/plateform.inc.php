...
if($op == 'list') {
	...
	foreach($data as &$row) {
		...
		$work_hours = iunserializer($row['work_hours']);
		if(!empty($work_hours) && is_array($work_hours)){
			$row['work_hours'] = $work_hours['start'] . '-' . $work_hours['end'];
		}else{
			$row['work_hours'] = '未设置';
		}
	}
...
if($op == 'post') {
	...
	if($_W['ispost']) {
		...
		if($deliveryer_errander_fee_type == 1) {
			...
			$deliveryer_errander_fee = floatval($_GPC['deliveryer_errander_fee_4']);
		} 
		$delivery_fee =  array(
			'takeout' => array(
		...
		$work_hours = array('start' => $_GPC['start_work_time'], 'end' => $_GPC['end_work_time']);
		$data['work_hours'] = iserializer($work_hours);
		if($_W['is_agent']) {
	...
	if(!empty($deliveryer)) {
		...
		if(!empty($deliveryer['work_hours'])){
			$deliveryer['work_hours'] = iunserializer($deliveryer['work_hours']);
		}else{
			$deliveryer['work_hours'] = array();
		}
	}
	mload()->model('agent');
	$agents = get_agents();
