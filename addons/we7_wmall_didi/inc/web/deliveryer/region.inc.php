...
if( $op == "post" ) 
{
    ...
    if( $_W["ispost"] ) 
    {
        ...
        imessage(error(0, "区域列表更新成功"), iurl("deliveryer/region/index"), "ajax");
    }
...
if( $op == "del" ) 
{
    ...
    $regionList = pdo_fetchall("select * from" . tablename("tiny_wmall_region") . "where uniacid = :uniacid order by id asc", array( ":uniacid" => $_W["uniacid"] ), "id");
    set_system_config("member.region", $regionList);
    imessage(error(0, "删除区域列表成功"), iurl("deliveryer/region/index"), "ajax");
}

include(itemplate("deliveryer/region"));
?>