...
if( $op == "post" ) 
{
    ...
    if( $_W["ispost"] ) 
    {
        ...
        imessage(error(0, "添加商城成功"), iurl("store/shop/setting", array( "_sid" => $sid )), "ajax");
    }
...
if( $op == "copy" ) 
{
    ...
    if( empty($store) ) 
    {
        imessage(error(-1, "商城不存在或已删除"), "", "ajax");
    }
...
if( $op == "lots" ) 
{
    ...
    if( $_W["ispost"] && $_GPC["set"] == 1 ) 
    {
        ...
        if( empty($sid) ) 
        {
            imessage(error(-1, "请选择需要修改的商城"), "", "ajax");
        }
    ...
    if( empty($ids) ) 
    {
        imessage(error(-1, "请选择需要操作的商城"), "", "ajax");
    }
...
if( $op == "sailed" ) 
{
    ...
    imessage(error(0, "同步商城销量成功"), "", "ajax");
}
?>