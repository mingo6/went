...
if ($ta == 'openid') {
	...
	if (!empty($oauth) && !is_error($oauth)) {
		...
		if(empty($fans)) {
			...
			$fans['uid'] = $record['uid'] = $fans['uid'] = $uid;
			$_SESSION['uid'] = $uid;
		...
		if(empty($wmall_member)) {
			$wmall_member = array(
				'uniacid' => $_W['uniacid'],
				'uid' => $fans['uid'] ? $fans['uid'] : date('His') . random(3, true),
				'openid_wxapp' => $oauth['openid'],
				'unionId' => $oauth['unionid'],
				'is_sys' => 1,
				'status' => 1,
				'token' => random(32),
	...
	if(empty($wmall_member)) {
		$wmall_member = array(
			'uniacid' => $_W['uniacid'],
			'uid' => $fans['uid'] ? $fans['uid'] : date('His') . random(3, true),
			'openid_wxapp' => $userinfo['openId'],
			'unionId' => $userinfo['unionId'],
			'nickname' => $userinfo['nickName'],
			'realname' => '',
			'mobile' => '',
			'sex' => ($userinfo['gender'] == 1 ? '男' : '女'),
			'avatar' => rtrim(rtrim($userinfo['avatarUrl'], '0'), 132) . 132,
			'is_sys' => 1,
			'status' => 1,
			'token' => random(32),
			'addtime' => TIMESTAMP,
