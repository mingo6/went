...
if( !empty($unisetting["oauth"]["account"]) ) 
{
	...
	{
		$_W["account"]["oauth"] = array( "key" => $oauth["key"], "secret" => $oauth["secret"], "acid" => $oauth["acid"], "type" => $oauth["type"], "level" => $oauth["level"] );
		$_W["oauth_account"] = $_W["account"]["oauth"];
		unset($oauth);
	}
	else 
	{
		$_W["account"]["oauth"] = array( "key" => $_W["account"]["key"], "secret" => $_W["account"]["secret"], "acid" => $_W["account"]["acid"], "type" => $_W["account"]["type"], "level" => $_W["account"]["level"] );
		$_W["oauth_account"] = $_W["account"]["oauth"];
	}
}
else 
