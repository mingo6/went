...
class WxPay{
	...
	public function __construct($pay_type = '') {
		...
		$this->wxpay = array(
			'appid' => $wechat['appid'],
			'mch_id' => $wechat['mchid'],
			'sub_mch_id' => $wechat['sub_mch_id'],
			'key' => $wechat['apikey'],
		);
		$this->cert = array(
			'apiclient_cert' => $wechat['apiclient_cert'],
			'apiclient_key' => $wechat['apiclient_key'],
			'rootca' => $wechat['rootca'],
		);
	}
	...
	public function mktTransfers($params, $check_type = 'FORCE_CHECK') {
		...
		$extra = array(
			CURLOPT_SSLCERT => MODULE_ROOT . "/cert/{$this->cert['apiclient_cert']}/apiclient_cert.pem",
			CURLOPT_SSLKEY => MODULE_ROOT . "/cert/{$this->cert['apiclient_key']}/apiclient_key.pem",
			CURLOPT_CAINFO => MODULE_ROOT . "/cert/{$this->cert['rootca']}/rootca.pem",
		);
...
/*			if($result['err_code'] == 'OPENID_ERROR' && $params_origin['check'] == 0) {
				$params_origin['check'] = 1;
				$pay = new WxPay('wxapp');
				$result = $pay->mktTransfersRepeat($params_origin);
				return $result;
			}*/
