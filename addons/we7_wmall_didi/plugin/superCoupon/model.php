...
function superCoupon_get_group_uids($id) {
	...
	} elseif($group['group_condition']['store_member_type'] == 2) {
		$condition .= ' and stat_day >= :start_day and stat_day <= :end_day';
	...
	if($group['group_condition']['store_member_type'] == 1) {
		//新用户
		if($member_limit['store_new_member_type'] == 1) {//未完成
			...
			$store_uids = array_keys($footmark);//进店的保存6天
			$uids = array_diff($store_uids, $has_order_uids);
		}
	} elseif($group['group_condition']['store_member_type'] == 3) {
		...
		if($group['group_condition']['store_member_type'] == 0) {
			...
			if($member_limit['order_stat_date_type'] == 1) {//工作日筛选
			...
			if($member_limit['order_stat_time_type'] == 1) {//订餐时间
				$meals_cn_condition = '';
		...
		if(!empty($member_limit['consume_num_type']) || !empty($member_limit['consume_price_type'])) {
			...
			foreach($orders as $key => $order) {
				if($member_limit['consume_num_type'] == 1) {
					...
					if($member_limit['min_consume_num'] > 0 && $order['total_num'] < $member_limit['min_consume_num']) {
						unset($orders[$key]);
					}
					if($member_limit['max_consume_num'] > 0 && $member_limit['max_consume_num'] > $member_limit['min_consume_num'] && $order['total_num'] > $member_limit['max_consume_num']) {
						unset($orders[$key]);
					}
				}
				...
				if($member_limit['consume_price_type'] == 1) {
					...
					if($member_limit['avg_min_consume_price'] > 0 && $avg_price < $member_limit['avg_min_consume_price']) {
						unset($orders[$key]);
					}
					if($member_limit['avg_max_consume_price'] > 0 && $member_limit['avg_max_consume_price'] > $member_limit['avg_min_consume_price'] && $avg_price > $member_limit['avg_max_consume_price']) {
						unset($orders[$key]);
					}
				}
			}
		...
		if($member_limit['is_favorite'] != -1 && $group['group_condition']['store_member_type'] == 0) {
			...
			} elseif($member_limit['is_favorite'] == 1) {//已收藏
				$uids = array_intersect($is_favorite, $uids);
			}
		}
		if($group['group_condition']['store_member_type'] == 2) {//最近$member_limit['no_order_days']有下单记录的
...
function superCoupon_grant_num($sid) {
	...
	$over_max_limit = 0;
	if($num >= $max_limit && $max_limit > 0) {
		$over_max_limit = 1;
	}
	return array(
		'max_limit' => $max_limit,
		'grant_num' => $num,
		'over_max_limit' => $over_max_limit
	);
}
