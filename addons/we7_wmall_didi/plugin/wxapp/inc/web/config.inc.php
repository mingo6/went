...
if ($op == 'basic') {
    ...
    if ($_W['ispost']) {
        $data = array('status' => intval($_GPC['status']), 'audit_status' => intval($_GPC['audit_status']), 'default_sid' => intval($_GPC['default_sid']), 'key' => trim($_GPC['key']), 'secret' => trim($_GPC['secret']), 'wxapp_consumer_notice_channel' => trim($_GPC['wxapp_consumer_notice_channel']), 'tpl_consumer_url' => trim($_GPC['tpl_consumer_url']));
