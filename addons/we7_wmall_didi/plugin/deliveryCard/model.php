	...
	if( !$order["is_pay"] ) 
	{
		...
		if( !empty($card) ) 
		{
			...
			if( !empty($member) ) 
			{
				...
				pdo_update("tiny_wmall_members", array( "setmeal_id" => $card["id"], "setmeal_day_free_limit" => $card["day_free_limit"], "setmeal_deliveryfee_free_limit" => $card["delivery_fee_free_limit"], "setmeal_starttime" => $starttime, "setmeal_endtime" => $endtime ), array( "uid" => $member["uid"], "uniacid" => $_W["uniacid"] ));
