...
function advertise_trade_update($id, $type = "pay") 
{
	...
	if( $type == "pay" ) 
	{
		...
		$preTrade = pdo_get("tiny_wmall_advertise_trade", array( "uniacid" => $trade["uniacid"], "sid" => $trade["sid"], "type" => $trade["type"], "status" => 1 ), array( "endtime" ));
...
function advertise_cron($trade_id = 0) 
{
	...
	if( !empty($trades) ) 
	{
		...
		foreach( $trades as $trade ) 
		{
			...
			{
				if( $trade["type"] == "stick" && $update_store["is_stick"] == 1 ) 
				{
