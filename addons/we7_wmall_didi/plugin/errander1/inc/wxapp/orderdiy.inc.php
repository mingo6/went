...
if($op == 'create') {
	...
	$data['plateform_serve_fee'] = $data['delivery_fee'] - $data['discount_fee'] - $data['deliveryer_fee'];
	$data['plateform_serve'] = iserializer(array(
		'fee' => $data['plateform_serve_see'],
		'note' => "订单配送费 ￥{$data['delivery_fee']} - 使用红包 ￥{$data['discount_fee']} - 配送员配送费 ￥{$data['deliveryer_fee']}"
	));
