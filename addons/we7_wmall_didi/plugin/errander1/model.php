...
function errander_order_fetch($id) {
	...
	if($order['order_type'] == 'buy') {
		...
		$order['goods_price'] = !empty($order['goods_price']) ? $order['goods_price'] : '未填写,请联系用户沟通';
	}
...
function errander_order_status_notice($id, $status, $note = '') {
	...
	if(!empty($order['openid'])) {
		...
		if(is_error($status)) {
			slog('wxtplNotice', "跑腿订单状态改变微信通知用户-order_id:{$order['id']}", $send, $status['message']);
		}
	}
	return true;
}
...
function errander_order_deliveryer_notice($id, $type, $deliveryer_id = 0, $note = '') {
	...
	if(in_array($type, array('new_delivery', 'direct_transfer', 'direct_transfer_refuse'))) {
		...
		if($deliveryer['extra']['accept_wechat_notice'] == 1) {
			$status = $acc->sendTplNotice($deliveryer['openid'], $_W['we7_wmall']['config']['notice']['wechat']['public_tpl'], $send, $url);
			if(is_error($status)) {
				slog('wxtplNotice', "跑腿订单通知配送员用户已取消订单-{$deliveryer['title']}", $send, $status['message']);
			}
		}
		if(!empty($deliveryer['token'])) {
			$audience = array(
...
function errander_order_update_bill($id, $extra = array()) {
	...
	$plateform_serve = array(
		...
		'note' => "订单配送费 ￥{$order['delivery_fee']} + 订单小费 ￥{$order['delivery_tips']} - 使用红包 ￥{$order['discount_fee']}",
	);
	if($_W['is_agent']) {
		mload()->model('agent');
