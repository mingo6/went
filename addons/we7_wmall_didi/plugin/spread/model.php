...
function spread_group_update($spread_id = 0, $wx_tpl = false) 
{
	...
	{
		if( $config["group_update_mode"] == "order_money_1" ) 
		{
		...
		{
			if( $config["group_update_mode"] == "order_count" ) 
			{
			...
			{
				if( $config["group_update_mode"] == "order_count_1" ) 
				{
					$condition .= " and is_pay = 1 and status = 5 and spread1 = :spread";
					$params[":spread"] = $spread_id;
					$result = pdo_fetchcolumn("select count(*) from" . tablename("tiny_wmall_order") . $condition, $params);
				...
				{
					if( $config["group_update_mode"] == "self_order_money" ) 
					{
					...
					{
						if( $config["group_update_mode"] == "self_order_count" ) 
						{
							$condition .= " and is_pay = 1 and status =5 and uid = :uid";
							$params[":uid"] = $spread_id;
							$result = pdo_fetchcolumn("select count(*) from" . tablename("tiny_wmall_order") . $condition, $params);
						...
						{
							if( $config["group_update_mode"] == "down_count" ) 
							{
							...
							{
								if( $config["group_update_mode"] == "down_count_1" ) 
								{
...
function sys_notice_spread_down($uid, $type, $extra = array( )) 
{
	...
	if( $type == "pseudo_down" ) 
	{
		if( !empty($spread1["openid"]) ) 
		{
			...
			$remark = array( "用户昵称: " . $member["nickname"] );
	...
	else 
	{
		if( $type == "new_down" ) 
		{
			...
			$remark = array( "用户昵称: " . $member["nickname"], (string) $member["nickname"] . "今后在" . $config_mall["title"] . "下单,您将会获得平台的返佣" );
		}
	}
	$remark = implode("\n", $remark);
...
function spread_order_balance($order_id) 
{
	...
	if( $balance == 1 ) 
	{
		if( !empty($commission) ) 
		{
			...
			if( 0 <= $fee_spread1 ) 
			{
				$extra = array( "trade_type" => 1, "extra" => $order["id"], "remark" => "订单号:" . $order["id"] . ", 一级下线佣金费率:" . $commission["spread1_rate"] . ", 佣金:" . $fee_spread1 . "元" );
				spread_update_credit2($order["spread1"], $fee_spread1, $extra);
			}
			$fee_spread2 = $commission["spread2"];
			if( 0 <= $fee_spread2 ) 
			{
				$extra = array( "trade_type" => 1, "extra" => $order["id"], "remark" => "订单号:" . $order["id"] . ", 二级下线佣金费率:" . $commission["spread2_rate"] . ", 佣金:" . $fee_spread2 . "元" );
...
function member_spread_bind() 
{
	...
	if( $config_spread["relate"]["become_child_limit"] == 2 ) 
	{
		if( 180 < TIMESTAMP - $_W["member"]["addtime"] ) 
		{
			return error(-1, "您已经关注过平台,不能成为他人推广对象和下线");
		}
	}
	else 
	{
		...
		{
			if( $config_newmember_condition == $config_spread["relate"]["become_child_limit"] ) 
			{
				return error(-1, "您已经是平台老用户,不能参与推广活动");
			}
			$where = "where uniacid = :uniacid and uid = :uid";
			if( $config_spread["relate"]["become_child_limit"] == 1 ) 
			{
				$where = "where uniacid = :uniacid and uid = :uid and status != 6";
			}
			$is_exist = pdo_fetch("select id from " . tablename("tiny_wmall_order") . $where, array( ":uniacid" => $_W["uniacid"], ":uid" => $_W["member"]["uid"] ));
			if( $is_exist ) 
			{
				return error(-1, "您已经是平台老用户,不能参与推广活动");
			}
	...
	if( !empty($invite_uid) ) 
	{
		...
		$update = array( "spread1" => $spread["uid"], "spread2" => $spread["spread1"], "spreadfixed" => 0 );
