...
function store_fetch($id, $field = array()) {
	...
	$data = pdo_fetch("SELECT {$field_str} FROM " . tablename('tiny_wmall_store') . ' WHERE uniacid = :uniacid AND id = :id AND status != :status', array(':uniacid' => $_W['uniacid'], ':id' => $id, ':status' => 4));
	if(empty($data)) {
...
function store_fetch_by_region_id($region_id, $field = array())
{
	global $_W;
	$field_str = '*';
	if(!empty($field)) {
	...
	$data = pdo_fetch("SELECT {$field_str} FROM " . tablename('tiny_wmall_store') . ' WHERE uniacid = :uniacid AND region_id = :region_id', array(':uniacid' => $_W['uniacid'], ':region_id' => $region_id));
	if(empty($data)) {
		return error(-1, '门店不存在或已删除');
	}
	return $data;
...
function store_account($sid, $fileds = array()) {
	...
	if(!empty($account)) {
		...
		$account['fee_goods'] = iunserializer($account['fee_goods']);
	}
	return $account;
}
...
function store_update_account($sid, $fee, $trade_type, $extra, $remark = '') {
	global $_W;
	//$trade_type 1: 订单入账, 2: 申请提现, 3: 账户清零 5：平台平衡商家账户。
...
function store_delivery_times($sid, $force_update = false) {
	...
	$store = store_fetch($sid, array('id', 'delivery_reserve_days', 'delivery_within_days', 'delivery_time', 'delivery_times', 'delivery_fee_mode', 'delivery_price', 'business_hours'));
	...
	$timestamp = array();
	//营业时间
	foreach ($store['business_hours'] as $key => $value) {
		$store['business_hours'][$key]['s'] = strtotime($value['s']);
		$store['business_hours'][$key]['e'] = strtotime($value['e']);
	}
	if(!empty($times)) {
		foreach($times as $key => &$row) {
			...
			foreach($store['business_hours'] AS $hours){
				if(($startTime >= $hours['s'] && $startTime <= $hours['e']) || ($endTime >= $hours['s'] && $endTime <= $hours['e'])){
					$flag = true;
					break 1;
				}
			}
...
function store_getcash_transfers($id) {
	...
	$params = array(
		...
		'desc' => "{$store['title']}" . date('Y-m-d H:i', $log['addtime']) . "提现申请"
	);
