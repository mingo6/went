...
function order_cancel_types($role = 'clerker') {
	$types = array(
		'clerker' => array(
			'fakeOrder' => '用户信息不符',
			'foodSoldOut' => '商品已经售完',
			'restaurantClosed' => '平台已经打烊',
			'distanceTooFar' => '超出配送范围',
			'restaurantTooBusy' => '平台现在太忙',
...
function order_insert_status_log($id, $type, $note = '', $role = '', $role_cn = '') {
	...
	$notes = array(
		'place_order' => array(
			'status' => 1,
			'title' => '订单提交成功',
			'note' => "单号:{$order['ordersn']},请耐心等待平台确认",
			'ext' => array(
				array(
					'key' => 'pay_time_limit',
					'title' => '订单待支付',
		...
		'delivery_instore' => array(
			'status' => 12,
			'title' => '配送员已到店',
			'note' => '配送员已到店, 正等待平台为您出餐'
		),
		'delivery_takegoods' => array(
			'status' => 12,
			'title' => '配送员已取单',
			'note' => '平台已出单, 骑士将骑上战马为您急速送达'
		),
...
function order_status_notice($id, $status, $note = '') {
	...
	if(!empty($order['openid'])) {
		...
		if($order_channel == 'wap') {
			...
			elseif($type == 'delivery_assign') {
				...
				$end_remark = "配送员已接单, 正赶至平台为您取货。";
			}
			elseif($type == 'delivery_instore') {
				$title = '配送员已到店，正等待平台出餐';
				$remark = array(
					// "门店名称: {$store['title']}",
					"订单类型: {$order['order_type_cn']}",
				);
				$end_remark = "配送员已到店，正等待平台为您出餐。";
			}
			...
			if(!empty($form_id)) {
				if($form_type == 'formId') {
					...
					$prepay_times = $order['data']['prepay_times']--;
		...
		if(is_error($status)) {
			slog('wxtplNotice', "订单状态改变微信通知用户-order_id:{$order['id']}", $send, $status['message']);
		}
	}
	if($order['order_channel'] == 'h5app') {
		...
		$router = array(
			...
			'handle' => array(
				'title' => '您的订单平台已接单',
				'content' => '平台已接单,正在为您准备商品中',
			),
			'delivery_assign' => array(
				'title' => '您的订单已分配配送员',
				'content' => "配送员:{$deliveryer['title']},手机号:{$deliveryer['mobile']},配送员已接单, 正赶至平台取货, 骑士将骑上战马为您急速送达, 请保持电话畅通",
			),
			'delivery_instore' => array(
				'title' => '配送员已到店',
				'content' => "配送员已到店,正在为您取单,配送员:{$deliveryer['title']},手机号:{$deliveryer['mobile']}",
			),
			'delivery_takegoods' => array(
				'title' => '配送员已取单',
				'content' => "配送员已到店取单,正在为您配送中,配送员:{$deliveryer['title']},手机号:{$deliveryer['mobile']}",
			),
			'end' => array(
				'title' => '您的订单已完成',
...
function order_clerk_notice($id, $type, $note = '') {
	...
	if($type == 'place_order') {
		...
		$title = "有新的订水订单,订单号为 #{$order['serial_sn']} ,订单金额:{$order['final_fee']}元,请请尽快处理";
		...
		$title = "您订单号为: {$order['ordersn']}的订水单已被配送员接单";
		$remark = array(
			// "门店名称: {$store['title']}",
			"下单时间: " . date('Y-m-d H:i', $order['addtime']),
		);
	...
	foreach($clerks as $clerk) {
		if($clerk['region_id'] != $order['region_id']){
			continue;
		}
...
function order_deliveryer_notice($id, $type, $deliveryer_id = 0, $note = '') {
	...
	if($type == 'new_delivery') {
		$title = "有新的订水配送订单, 配送地址为{$order['number']}, 快去处理吧";
		$remark = array(
			// "门店名称: {$store['title']}",
			"下单时间: " . date('Y-m-d H:i', $order['addtime']),
			"总金　额: {$order['final_fee']}",
		...
		$title = "有新的配送订单, 配送地址为{$order['number']}";
		$remark = array(
			// "门店名称: {$store['title']}",
			"下单时间: " . date('Y-m-d H:i', $order['addtime']),
			"下单　人: {$order['username']}",
		...
		if($store['delivery_mode'] == 2 && $order['delivery_type'] == 2) {//平台配送才会语音提示
			Jpush_deliveryer_send('您有新的订水待抢订单', $title, array('voice_text' => $title, 'notify_type' => 'ordernew', 'redirect_type' => 'takeout', 'redirect_extra' => 3));
		}
	} else if($type == 'cancel') {
		$title = "收货地址为{$order['number']}, 收货人为{$order['username']}的订单已取消,请及时调整配送顺序";
		$remark = array(
			// "门店名称: {$store['title']}",
			"下单时间: " . date('Y-m-d H:i', $order['addtime']),
			"下单　人: {$order['username']}",
		...
		$title = "{$from_deliveryer['title']}向您发起转单申请，收货地址为{$order['number']},请及时做出回复";
		$remark = array(
			// "门店名称: {$store['title']}",
			"下单时间: " . date('Y-m-d H:i', $order['addtime']),
		...
		$title = "{$to_deliveryer['title']}拒绝了收获地址为{$order['number']}的定向转单申请,此订单将由您继续配送";
		$remark = array(
			// "门店名称: {$store['title']}",
			"下单时间: " . date('Y-m-d H:i', $order['addtime']),
			"下单　人: {$order['username']}",
	...
	foreach($deliveryers as $deliveryer) {
		if($order['region_id'] != $deliveryer['region_id']){
			continue;
		}
		...
		if($store['delivery_mode'] == 2 && $order['delivery_type'] == 2) {//平台配送才会语音提示
			if(!empty($deliveryer['token'])) {
				...
				if($type == 'new_delivery') {
					Jpush_deliveryer_send('您有新的订水配送订单', $title, array('voice_text' => $title, 'notify_type' => 'orderassign', 'redirect_type' => 'takeout', 'redirect_extra' => 7), $audience);
				} elseif($type == 'cancel') {
					...
					Jpush_deliveryer_send("{$to_deliveryer['title']}拒绝了收获地址为{$order['number']}的定向转单申请", $title, array('voice_text' => $title, 'notify_type' => 'direct_transfer_refuse','redirect_type' => 'takeout', 'redirect_extra' => 3), $audience);
				}
			}
		}
	}
...
function order_refund_notice($order_id, $type, $note = '') {
	...
	if($type == 'apply') {
		...
		if(!empty($order['openid'])) {
			...
			if(is_error($status)) {
				slog('wxtplNotice', "申请退款成功微信通知用户-order_id:{$order['id']}", $send, $status['message']);
			}
		}
	} elseif($type == 'fail') {
		if($order['agentid'] > 0) {
...
function order_redPacket_available($price, $category = array(), $scene = 'waimai') {
	global $_W;
...
function order_begin_payrefund($order_id) {
	...
	if($refund['pay_type'] == 'credit') {
		...
		return error(0, '退款成功,支付金额已退款至用户的平台余额');
		...
		order_insert_refund_log($refund['order_id'], 'handle');
		order_insert_refund_log($refund['order_id'], 'success');
		return error(0, "退款成功,支付金额已退款至用户的支付宝账户:{$response['buyer_logon_id']}");
		...
		order_insert_refund_log($refund['order_id'], 'handle');
		order_insert_refund_log($refund['order_id'], 'success');
		return error(0, "退款成功,支付金额已退款至用户一码付账户");
		...
		order_insert_refund_log($refund['order_id'], 'handle');
		order_insert_refund_log($refund['order_id'], 'success');
		return error(0, '退款成功,支付金额已退款至用户的APP账户余额');
	}
}
...
function order_query_payrefund($order_id) {
	...
	if($refund['pay_type'] == 'wechat') {
		...
		if($response['refund_status_0'] == 'SUCCESS') {
			...
			pdo_update('tiny_wmall_order', array('refund_status' => 3), array('uniacid' => $_W['uniacid'], 'id' => $refund['order_id']));
			order_insert_refund_log($refund['order_id'], 'success');
			return error(0, "退款成功,支付金额已退款至用户的微信账号:{$response['refund_recv_accout_0']}");
		} else {
...
function order_status_update($id, $type, $extra = array()) {
	...
	if($type == 'handle') {
		...
		if(!$order['is_pay'] && $order['order_type'] <= 2) {
			return error(-1, '该订单属于订水单,并且未支付,不能接单');
		}
		...
		mload()->model('deliveryer');
		if($order['order_type'] > 1) {
			return error(-1, '订单类型不是订水单,不需要通知配送员抢单');
		}
		...
		mload()->model('deliveryer');
		//订单状态, 1:已提交,待确认 2:平台已接单 3: 待配送 4:配送中 5:已完成 6:已取消
		if($order['status'] == 5) {
		...
		if(is_open_order($order) || !$order['is_pay'] || $order['final_fee'] <= 0 || ($order['is_pay'] == 1 && ($order['pay_type'] == 'delivery' || $order['pay_type'] == 'cash'))) {
			...
			if(!pdo_insert('tiny_wmall_order_refund', $refund)){
				return error(-1, '退款失败！');
			}
			$is_refund = 1;
...
function order_assign_deliveryer($order_id, $deliveryer_id, $update_deliveryer = false, $note = '') {
	...
	if($store['delivery_mode'] == 1) {
		...
		if(empty($deliveryer['is_takeout'])) {
			return error(-1, "该配送员没有平台订水订单的配送权限");
		}
	}
	$update = array(
		'status' => 4,
	...
	//自动确认取货
	$result = order_deliveryer_update_status($order_id, 'delivery_takegoods', array('deliveryer_id' => $deliveryer_id, 'delivery_handle_type' => 'auto'));
	if(is_error($result)) {
...
function order_system_status_update($id, $type, $extra = array()) {
	...
	if($type == 'pay') {
		...
		//首次下单赠送积分
		$count = pdo_fetchcolumn('SELECT count(id) FROM ' . tablename('tiny_wmall_order'). ' WHERE uid=:uid AND uniacid=:uniacid AND is_pay=:is_pay AND id!=:id', array(':uid' => $order['uid'], ':uniacid' => $order['uniacid'], ':is_pay' => 1, ':id' => $order['id']));
		if($count <= 0){
			if(FIRST_ORDER_MONEY > 0){
				mload()->model("member");
				member_credit_update($order["uid"], 'credit2', FIRST_ORDER_MONEY, array($order["uid"], '首次下单赠送余额' . FIRST_ORDER_MONEY . '元'));
			}
		}
		//减库存
...
function order_deliveryer_update_status($id, $type, $extra = array()) {
	...
	if($type == 'delivery_assign') {
		...
		if(in_array($extra['role'], array('eleme', 'meituan','dada'))) {
			...
			if($order['delivery_type'] == 2) {
				if($deliveryer['collect_max_takeout'] > 0 && $deliveryer['order_takeout_num'] >= $deliveryer['collect_max_takeout']) {
					return error(-1, "每人最多可抢{$deliveryer['collect_max_takeout']}个订水单");
				}
			}
		}
		$update = array(
		...
		//自动确认取货
		$result = order_deliveryer_update_status($order['id'], 'delivery_takegoods', array('deliveryer_id' => $deliveryer_id, 'delivery_handle_type' => 'auto'));
		if(is_error($result)) {
			return error(-1, $result['message']);
		}
		return error(0, '抢单成功');
...
function order_manager_notice($order_id, $type, $note = '') {
	...
	if($type == 'new_delivery') {
		...
		$title = '平台有新的订水订单，系统自动调度失败，请登录后台人工调度';
		$remark = array(
			// "门店名称: {$store['title']}",
			"订单类型: {$order['order_type_cn']}",
			"支付方式: {$order['pay_type_cn']}",
		...
		$title = '平台有新的待配送订水订单,但没有接单中的配送员,请尽快协调';
		$remark = array(
			"订单类型: 订水订单",
		);
	}
...
function order_calculate_deliveryer_fee($order, $deliveryerOrid = 0) {
	...
	}else if($config_takeout['deliveryer_fee_type'] == 4){
		if($config_takeout['deliveryer_fee'] > 0){
			$plateform_deliveryer_fee = $order['num'] * $config_takeout['deliveryer_fee'];
		}
	}
...
function order_update_bill($order_id, $extra_data = array()) {
	...
	if($order['order_type'] == 1) {
		if($order['delivery_type'] == 1) {
			...
			//平台配送模式下,如果平台设置承担额外配送费,则要从平台的利润中扣除额外配送费
			...
			if(!empty($store_bear_deliveryprice)) {
				...
				$plateform_serve['note'] .= " + 平台额外承担配送费 ￥{$store_bear_deliveryprice}";
			}
		}
	}
...
function order_delivery_info($store, $predict_index = -1, $condition = array()) {
	...
	if($sys_predict_index !== false) {
		...
		if($sys_predict_index > $predict_index && $predict_day == date('m-d')) {
			...
			//用户选择的配送时间已经过去了。比如：用户选择12-12:30，当前时间为14点
			$predict_index = -1;
		}
	} else {
	...
	if($predict_index == -1) {
		if(!$delivery_time['reserve']) {
			...
			if($sys_predict_index !== false) {
				...
				//如果当天没有配送时间段，默认为下一天，如果仅限当天，平台应该设置营业时间与配送时间段匹配
