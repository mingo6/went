...
function clerk_fetchall($sid)
{
    ...
    if( !empty($data) ) 
    {
        ...
        {
            $deliveryInfo = pdo_fetch('SELECT region_id FROM ' . tablename('tiny_wmall_deliveryer') . ' WHERE mobile = :mobile AND uniacid = :uniacid', array(':mobile' => $row['mobile'], 'uniacid' => $_W['uniacid']));
            if(!$deliveryInfo){
                unset($data[$key]);
                continue;
            }
            $row['region_id'] = $deliveryInfo['region_id'];
