...
function cloud_w_build_wxapp_authorize_params($wxapp_type = 'we7_wmall', $uniacid = 0) {
	...
	if($wxapp_type == 'we7_wmall') {
		...
		if(!empty($wxapp['menu']['data'])) {
			$menus = array(
				'selectedColor' => $wxapp['menu']['css']['selectedColor'],
				'backgroundColor' => $wxapp['menu']['css']['backgroundColor'],
				'color' => $wxapp['menu']['css']['color'],
	...
	$params = array(
		...
		'wxapp' => array(
			...
			'extPages' => $extPages,
			'window' => $window,
			'use_diy_home' => $wxapp['diy']['use_diy_home'],
			'test' => $wxapp['test'],
			'category' => array(
