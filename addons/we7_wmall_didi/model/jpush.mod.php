...
/*	$jpush_ios = array(
	...
	$extra = array(
		//'Authorization' => "Basic " . base64_encode("{$config['push_key']}:{$config['push_secret']}")
		'Authorization' => "Basic " . base64_encode("0c6d4c4fa27202b3a3cde1731:227c13954167b692bc6f8a3be")
	);
	$response = ihttp_request('https://api.jpush.cn/v3/push', json_encode($jpush_andriod), $extra);
	$return = Jpush_response_parse($response);
	if(is_error($return)) {
		slog('plateformappJpush', '平台管理App极光推送(andriod)通知管理', $jpush_andriod, $return['message']);
	}
	return true;
}
