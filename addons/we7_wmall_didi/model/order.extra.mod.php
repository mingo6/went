...
function order_dispatch_analyse($id, $extra = array()) {
	...
	if($order['region_id'] > 0){
		$filter['region_id'] = $order['region_id'];
	}
	$deliveryers = deliveryer_fetchall(0, $filter);
	if(!empty($deliveryers)) {
		$currtime = time();
		foreach($deliveryers as $key => &$deliveryer) {
			...
			//判断是否在工作时间
			if(!empty($deliveryer['work_hours'])){
				$work_hours = iunserializer($deliveryer['work_hours']);
				$work_hours['start'] = !empty($work_hours['start']) ? strtotime($work_hours['start']) : 0;
				$work_hours['end'] = !empty($work_hours['end']) ? strtotime($work_hours['end']) : 0;
				if(!empty($work_hours) && is_array($work_hours)){
					if($currtime < $work_hours['start'] || $currtime >= $work_hours['end']){
						unset($deliveryers[$key]);
						continue;
					}
				}
			}
			...
			$condition = ' WHERE delivery_status in (4,7) and ((deliveryer_id = :deliveryer_id and transfer_delivery_status = 0) or (delivery_collect_type = :delivery_collect_type and transfer_deliveryer_id = :transfer_deliveryer_id and transfer_delivery_status = 1))';
			$deliveryer['store2deliveryer_distance'] = pdo_fetchcolumn('SELECT count(id) FROM ' . tablename('tiny_wmall_order') . $condition, $params);
			unset($deliveryer);
		}
		if(empty($deliveryers)){
			return error(-1, '没有合适的平台配送员，无法进行自动调度，请管理员自行分配！');
		}
		if(empty($extra['sort'])) {
