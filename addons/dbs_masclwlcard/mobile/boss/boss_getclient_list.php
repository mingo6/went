...
if (!$_GPC["card_id"]) {
    $this->dexit(array("error" => 1, "msg" => "没有对应的员工"));
    exit;
}
$pindex = max(1, intval($_GPC["page"]));
$psize = 20;
$condition = " uniacid = :uniacid ";
$Data = pdo_fetchall("SELECT *  FROM " . tablename("dbs_masclwlcard_card_member") . " WHERE {$condition} AND aid=:card_id ORDER BY addtime DESC LIMIT " . ($pindex - 1) * $psize . "," . $psize, array(":uniacid" => $_W["uniacid"], ":card_id" => $_GPC["card_id"]));
if (!empty($Data)) {
    foreach ($Data as $k => $v) {
        $Data[$k]["now_status"] = -1;
        $info = array();
        if ($v["addtime"] < strtotime(date("Y-m-d", time() - 24 * 60 * 60))) {
            $info = pdo_fetch("SELECT *  FROM " . tablename("dbs_masclwlcard_card_closer") . " WHERE {$condition}   AND card_mid=:card_mid AND card_id=:card_id order by addtime desc ", array(":uniacid" => $_W["uniacid"], ":card_id" => $_GPC["card_id"], ":card_mid" => $v["id"]));
            if ($info) {
