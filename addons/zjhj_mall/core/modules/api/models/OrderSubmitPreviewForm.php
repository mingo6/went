...
class OrderSubmitPreviewForm extends Model
{
    ...
    public function search()
    {
        ...
        //商品打开才有
        if($res['data']['form']['is_form'] == 1){
            if(!empty($res['data']['list'])){
                $bool = false;
                foreach($res['data']['list'] AS $key => $orderInfo){
                    if(isset($orderInfo->is_form) && $orderInfo->is_form == 1){
                        $bool = true;
                        break;
                    }
                }
                if(!$bool){
                    $res['data']['form']['is_form'] = 0;
                }
            }
        }
...
//            $express_price = PostageRules::getExpressPrice($this->store_id, $address['province_id']);
    ...
    private function getDataByGoodsInfo($goods_info, $store)
    {
        ...
        $goods_info->is_form = $goods->is_form;
        $attr_id_list = [];
