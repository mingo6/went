    ...
    public function save()
    {
        ...
        if ($order->is_pay == 1) {
            ...
            if($order->pay_type == 2){
                ...
                $balance -> operator = $admin -> username;
                $balance -> store_id = $order -> store_id;
                $balance -> operator_id = $admin -> id;
                $balance -> addtime = time();
                $balance -> money = $order -> pay_price;
                $balance -> content = "管理员：".$admin->username."同意了 ".$user->nickname." 的退款申请，".$order->pay_price."元退还账户余额";
