...
class CouponIndexForm extends Model
{
    ...
    public function search()
    {
        ...
        $list = $query->orderBy('uc.addtime DESC')
            ->limit(200)
            ->select('uc.id user_coupon_id,uc.coupon_id,c.name as coupon_name,c.sub_price,c.min_price,uc.begin_time,uc.end_time,uc.is_use,uc.is_expire,cas.event,uc.type')->asArray()->all();
        ...
        foreach ($list as $i => $item) {
            ...
            $list[$i]['event_desc'] = $item['type']==3?'购物所得':$events[$item['event']];
        }
        return [
            'code' => 0,
            'msg' => 'success',
