...
class OrderPayDataForm extends Model
{
    ...
    public function search()
    {
        ...
        if($this->order['order_type'] == OrderSubmitForm::ORDER_TYPE_CHARGE) 
        {
            $goods_names = '订单-余额充值';
        }
        else
    ...
    private function setReturnData($order)
    {
        ...
        $share_commission_score_first = 0;//一级分销总佣金
        $share_commission_score_second = 0;//二级分销总佣金
        $share_commission_score_third = 0;//三级分销总佣金
        foreach ($order_detail_list as $item) {
            ...
            if ($item['individual_share'] == 1) {
                ...
                if ($item['share_type'] == 1) {
                    ...
                    $share_commission_score_first += $rate_first_score * $item['num'];
                    $share_commission_score_second += $rate_second_score * $item['num'];
                    $share_commission_score_third += $rate_third_score * $item['num'];
                } else {
                    ...
                    $share_commission_score_first += $item_price * $rate_first_score / 100;
                    $share_commission_score_second += $item_price * $rate_second_score / 100;
                    $share_commission_score_third += $item_price * $rate_third_score / 100;
                }
            } else {
        ...
        $order->first_score = $share_commission_score_first < 1 ? 0 : $share_commission_score_first;
        $order->second_score = $share_commission_score_second < 1 ? 0 : $share_commission_score_second;
        $order->third_score = $share_commission_score_third < 1 ? 0 : $share_commission_score_third;
        $order->save();
    }
