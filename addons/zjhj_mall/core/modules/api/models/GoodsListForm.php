...
class GoodsListForm extends Model
{
    ...
    public function search()
    {
        ...
        $query = Goods::find()->alias('g')->leftJoin(GoodsLevel::tableName() . ' gl', 'g.id=gl.goods_id')->where([
            'g.status' => 1,
            'g.is_delete' => 0,
        ]);
        if ($this->store_id)
        ...
        $user = $this->user;
        if($user && $user->id > 0){
            $userInfo = \app\models\User::find()->select('level')->where(array('id' => $user->id))->one();
            if(!empty($userInfo['level'])){
                if($userInfo['level'] != -1){
                    $query->andWhere(
                        [
                            'OR',
                            ['g.is_open_privilege' => 0],
                            ['gl.level_id' => $userInfo['level']],
                        ]
                    );
                }
            }
        }else{
            $query->andWhere(['g.is_open_privilege' => 0]);
        }
        if ($this->keyword)
