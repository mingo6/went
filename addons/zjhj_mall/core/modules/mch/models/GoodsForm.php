...
class GoodsForm extends Model
{
    ...
    public $share_commission_first_score;
    public $share_commission_second_score;
    public $share_commission_third_score;
    public $weight;
    public $freight;
    ...
    {
        return [
            [['name', 'goods_sn', 'service', 'unit'], 'trim'],
            [['store_id', 'name', 'price', 'cat_id', 'detail', 'goods_pic_list', 'cover_pic'], 'required'],
            [['store_id', 'sort', 'virtual_sales', 'freight','share_type', 'is_form', 'is_open_privilege'], 'integer'],
            ...
            [['share_commission_first', 'share_commission_second', 'share_commission_third', 'share_commission_first_score', 'share_commission_second_score', 'share_commission_third_score', 'freight',], 'default', 'value' => 0],
            [['share_commission_first', 'share_commission_second', 'share_commission_third', 'share_commission_first_score', 'share_commission_second_score', 'share_commission_third_score',], 'number', 'min' => 0],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'share_commission_first_score' => '一级积分比例',
            'share_commission_second_score' => '二级积分比例',
            'share_commission_third_score' => '三级积分比例',
            'weight' => '重量',
            'freight' => '运费规则ID',
            'full_cut' => '满减',
            'integral' => '积分设置',
            'goods_num' => '商品库存',
            'is_form' => '是否开启表单',
            'is_open_privilege' => '是否开启查看商品等级权限',
        ];
    }
    ...
    public function save()
    {
        if ($this->validate()) {
            ...
            if($goods->attributes['goods_sn']){
                $query = Goods::find()->where(['store_id' => $this->store_id, 'goods_sn' => $goods->attributes['goods_sn']])->select(['id']);
                if($goods['id']){
                    $query->andWhere(['!=', 'id', $goods['id']]);
                }
                $goodsSnExists = $query->one();
                if($goodsSnExists){
                    return [
                        'code' => 1,
                        'msg' => '商品货号已存在！',
