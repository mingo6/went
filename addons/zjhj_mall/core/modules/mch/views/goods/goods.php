    ...
    <table class="table table-bordered bg-white">
        ...
        <?php foreach ($list as $index => $goods): ?>
            <tr>
                ...
                <td class="nowrap">
                    <?php if ($goods->is_open_privilege == '1'): ?>
                        <a class="btn btn-sm btn-primary" href="<?= $urlManager->createUrl(['mch/goods-level/index', 'goods_id' => $goods->id]) ?>">会员查看权限管理</a>
                    <?php endif ?>
                    <a class="btn btn-sm btn-primary" href="<?= $urlManager->createUrl(['mch/goods/goods-coupon', 'id' => $goods->id]) ?>">优惠券管理</a>
                    <a class="btn btn-sm btn-primary" href="<?= $urlManager->createUrl(['mch/goods/goods-edit', 'id' => $goods->id]) ?>">修改</a>
                    <a class="btn btn-sm btn-primary copy" data-clipboard-text="/pages/goods/goods?id=<?= $goods->id ?>" href="javascript:" hidden>复制链接</a>
                    <a class="btn btn-sm btn-danger del" href="<?= $urlManager->createUrl(['mch/goods/goods-del', 'id' => $goods->id]) ?>">删除</a>
                </td>
            </tr>
        <?php endforeach; ?>
        </tbody>
