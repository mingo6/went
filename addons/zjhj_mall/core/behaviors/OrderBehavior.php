...
//        \Yii::warning('==>2');
     ...
     *
     */
    private function money($parent_id, $money = 0, $score = 0)
    {
        if($money <=0 && $score <= 0){
            return [
                'code' => 1,
        ...
        if($score > 0){
            $parent->total_integral += $score;
            $parent->integral += $score;
        }
        if ($parent->save()) {
            return [
                'code' => 0,
...
//            \Yii::warning('==>'.$id.'5');
            ...
            $user_1->total_integral += $order->first_score;
            $user_1->integral += $order->first_score;
            $user_1->save();
//            \Yii::warning('==>'.$id.'6');
            UserShareMoney::set($user_1->id, $order->id, $order->first_price, $order->first_score, 0, 1, $this->store_id);
//            \Yii::warning('==>'.$id.'7');
            $order->is_price = 1;
            $order->save();
        }
        ...
        if ($this->share_setting->level >= 2) {
            ...
            if (!$user_2) {
                if ($user_1->parent_id != 0 && $order->parent_id_1 == 0) {
                    $res = self::money($user_1->parent_id, $order->second_price, $order->second_score);
                    UserShareMoney::set($user_1->parent_id, $order->id, $order->second_price, $order->second_score, 0, 2, $this->store_id);
                    if ($res['parent_id'] != 0 && $this->share_setting->level == 3) {
                        $res = self::money($res['parent_id'], $order->third_price, $order->third_score);
                        UserShareMoney::set($res['parent_id'], $order->id, $order->third_price, $order->third_score, 0, 3, $this->store_id);
                    }
                }
                return;
            }
            ...
            $user_2->total_integral += $order->second_score;
            $user_2->integral += $order->second_score;
            $user_2->save();
            UserShareMoney::set($user_2->id, $order->id, $order->second_price, $order->second_score, 0, 2, $this->store_id);
        }
        //三级佣金发放
        if ($this->share_setting->level >= 3) {
            ...
            if (!$user_3) {
                if ($user_2->parent_id != 0 && $order->parent_id_2 == 0) {
                    self::money($user_2->parent_id, $order->third_price, $order->third_score);
                    UserShareMoney::set($user_2->parent_id, $order->id, $order->third_price, $order->third_score, 0, 3, $this->store_id);
                }
                return;
            }
            $user_3->total_price += $order->third_price;
            $user_3->price += $order->third_price;
            $user_3->total_integral += $order->third_score;
            $user_3->integral += $order->third_score;
            $user_3->save();
            UserShareMoney::set($user_3->id, $order->id, $order->third_price, $order->third_score, 0, 3, $this->store_id);
        }
    }
