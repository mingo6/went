...
function iurl($segment, $params = array(), $addhost = false) {
	...
	if($_W['_controller'] == 'store' || $ctrl == 'store') {
		$params['i'] = $_W['uniacid'];
		$url = iwurl('site/entry', $params, './index.php?');
...
//ajax成功返回
function isuccess($result_message = '调用接口成功', $data = array('resultCode' => '')) {
	return ierror(0, $result_message, $data);
}

if(!function_exists('ajaxPagination')){
	//分页返回数据结构
	function ajaxPagination(&$list, $total, $currpage = 1, $pagesize = 10){
