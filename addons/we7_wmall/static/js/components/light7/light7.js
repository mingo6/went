...
+function ($) {
  "use strict";
        ...
        if (defaults.modalTemplate) {
            ...
            if (params.buttons && params.buttons.length > 0) {
                for (var i = 0; i < params.buttons.length; i++) {
                    buttonsHTML += '<span class="modal-button border-1px-r' + (params.buttons[i].bold ? ' modal-button-bold' : '') + '">' + params.buttons[i].text + '</span>';
                }
            }
            ...
            modalHTML = '<div class="modal ' + noButtons + '"><div class="modal-inner border-1px-b">' + (titleHTML + textHTML + afterTextHTML) + '</div><div class="modal-buttons ' + verticalButtons + '">' + buttonsHTML + '</div></div>';
        }
    ...
    $.prompt2 = function (text, title, callbackOk, callbackCancel) {
        if (typeof title === 'function') {
            callbackCancel = arguments[2];
            callbackOk = arguments[1];
            title = undefined;
        ...
        return $.modal({
            text: text || '',
            title: typeof title === 'undefined' ? defaults.modalTitle : title,
            afterText: '<input type="text" class="modal-text-input modal-text-input2" placeholder="请输入快递公司名称"><input type="text" class="modal-text-input modal-text-input3" placeholder="请输入快递单号">',
            buttons: [
                {
                    text: defaults.modalButtonCancel
                },
            ...
            onClick: function (modal, index) {
                if (index === 0 && callbackCancel) callbackCancel($('.modal-text-input').eq(0).val() + ',' + $('.modal-text-input').eq(1).val());
                if (index === 1 && callbackOk) callbackOk($('.modal-text-input').eq(0).val() + ',' + $('.modal-text-input').eq(1).val());
            }
        });
    };
