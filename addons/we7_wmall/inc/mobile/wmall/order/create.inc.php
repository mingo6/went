...
if($ta == 'submit') {
	...
	if ($cart['type']!=0){
        ...
        if ($cart['type']==1){
            //修改销量
            $order['active_id'] = $cart['active_id'];
            $cutPrice_goods_id = pdo_get('tiny_wmall_cutPrice_user',array('id'=>$cart['active_id']),array('kid'));
        ...
        //秒杀订单
        elseif ($cart['type']==3){
            pdo_query("update " . tablename("tiny_wmall_secondKill_goods") .  " set sale_num = sale_num + 1 , surplus = surplus - 1 where id = :id", array( ":id" => $cart['active_id'] ));
            icheckauth();
            $user_data = array('sid'=>$cart['active_id'],'uid'=>$_W["member"]["uid"],'openid'=>$_W["member"]["openid"],'nickname'=>$_W["member"]["nickname"],'headimgurl'=>$_W["member"]["avatar"],'price'=>$cart['price'],'createtime'=>time(),'ip'=>$_W['clientip']);
            pdo_insert('tiny_wmall_secondKill_user',$user_data);
            $order['active_id'] = pdo_insertid();
        }
    }
