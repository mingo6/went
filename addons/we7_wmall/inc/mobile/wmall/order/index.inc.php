    ...
    if( !empty($orders) ) 
    {
        ...
        foreach( $orders as &$da ) 
        {
            ...
            if ($da['type']==2){
                ...
                $da['goods']['goods_title'] = $group_info['goods_name'];
            }
        }
...
if( $ta == "more" ) 
{
    ...
    if( !empty($orders) ) 
    {
        ...
        foreach( $orders as &$order ) 
        {
            ...
            //拼团订单
            if ($order['type']==2){
                $group_info = $AllgroupgoodsOrder->getGroupInfo($order['active_id']);
                $order['active_state'] = $group_info['state'];
                $order['goods']['goods_title'] = $group_info['goods_name'];
            }
        }
        $min = min(array_keys($orders));
    }
...
if( $ta == "detail" ) 
{
    ...
    if ($type==1) {
        $allgroupgoods_order = pdo_get("tiny_wmall_order",array("type"=>2,"active_id"=>$id),array("id"));
        if( empty($allgroupgoods_order) ) imessage("订单不存在或已删除", "", "error");
        $id = $allgroupgoods_order["id"];
    }
