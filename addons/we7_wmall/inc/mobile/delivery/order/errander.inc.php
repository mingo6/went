...
if($ta == 'list') {
	$_W['page']['title'] = '订单列表';
	$condition = ' where a.uniacid = :uniacid and b.type = :btype and a.agentid = :agentid and a.is_pay = 1 and a.status != 4';
	$params = array(
		':uniacid' => $_W['uniacid'],
		':agentid' => $_W['agentid'],
        ':btype' => $_W['deliveryer']['type']
	);
	...
	//待抢
	$num1 = pdo_fetch('select  count(*) as num from ' . tablename('tiny_wmall_errander_order') . ' as a left join ' . tablename('tiny_wmall_errander_category') . " as b on a.order_cid = b.id where a.uniacid = :uniacid and b.type = :btype and a.agentid = :agentid and a.is_pay = 1 and a.status != 4 and a.delivery_status = 1 and {$_W['deliveryer']['work_status']} and {$can_collect_order}", array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid'], ':btype' => $_W["deliveryer"]['type']));
	//待取
	$num2 = pdo_fetch('select count(*) as num from ' . tablename('tiny_wmall_errander_order') . " WHERE uniacid = :uniacid and order_type = :btype and agentid = :agentid and ((deliveryer_id = :deliveryer_id and transfer_delivery_status = 0) or (delivery_collect_type = 3 and transfer_deliveryer_id = :transfer_deliveryer_id and transfer_delivery_status = 1)) and is_pay = 1 and delivery_status = 2 and status != 4", array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid'], ':deliveryer_id' => $_deliveryer['id'], ':transfer_deliveryer_id' => $_deliveryer['id'], ':btype' => $_W["deliveryer"]['type']));
	//配送中
	$num3 = pdo_fetch('select count(*) as num from ' . tablename('tiny_wmall_errander_order') . " WHERE uniacid = :uniacid and order_type = :btype and agentid = :agentid and ((deliveryer_id = :deliveryer_id and transfer_delivery_status = 0) or (delivery_collect_type = 3 and transfer_deliveryer_id = :transfer_deliveryer_id and transfer_delivery_status = 1)) and is_pay = 1 and delivery_status = 3 and status != 4", array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid'], ':deliveryer_id' => $_deliveryer['id'], ':transfer_deliveryer_id' => $_deliveryer['id'], ':btype' => $_W["deliveryer"]['type']));
...
if($ta == 'success') {
	$id = intval($_GPC['id']);
	$status = errander_order_status_update($id, 'delivery_success', array('deliveryer_id' => $_deliveryer['id'], 'code' => $_GPC['code']));
	if(is_error($status)) {
		imessage(error(-1, $status['message']), '', 'ajax');
	}
...
if($ta == 'direct_transfer') {//老版跑腿可以设置限定配送员
	$id = intval($_GPC['id']);
	$order = errander_order_fetch($id);
	if(empty($order)) {
