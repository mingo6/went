...
else
{
    ...
    else
    {
        ...
        {
            if( $_W["role"] == "agenter" && !in_array($_W["_controller"], $permissions["agenter"]["controller"]) ) 
            {
                $permission = false;
            }
...
{
    if ($_REQUEST['ctrl']=='store') $_W["role"] = 'is_store';
    if( !in_array($_W["role"], array( "manager", "operator", "founder", "merchanter", "agenter",'is_store' )) && empty($_W["we7_wmall"]["store"]) && (empty($_W["_action"]) || $_W["_op"] != "login") )
    {
        imessage("抱歉，您无权进行该操作，请先登录！", iurl("store/oauth/login"), "info");
    }
//    $session = json_decode(base64_decode($_GPC['__we7_wmall_store']), true);
//    if ( $_W["_op"] != "login" && empty($session) ){
//        imessage("请先登录！", iurl("store/oauth/login"), "info");
//    }
//    if(is_array($session)) {
