...
function member_getcash_transfers($id) {
    global $_W;
    $id = intval($id);
    ...
    $response = $pay->mktTransfers($params);
    if(is_error($response)) {
        pdo_update('tiny_wmall_member_getcash_log', array('status' => 2), array('id' => $id));
        return error(-1, "打款未成功，等待管理员审核。详细错误信息：{$response['message']}");
    }
    pdo_update('tiny_wmall_member_getcash_log', array('status' => 1, 'endtime' => TIMESTAMP), array('uniacid' => $_W['uniacid'], 'id' => $id));
    sys_notice_store_getcash($log['sid'], $id, 'success');
    return error(0, '打款成功');
}
