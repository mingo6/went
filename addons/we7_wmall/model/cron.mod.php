...
function cron_order() {
	...
	if(!check_cache_status($key, 60)) {
        ...
        //拼团订单自动退款
        $orders_group = pdo_fetchall('select id, sid, addtime, type, active_id from ' . tablename('tiny_wmall_order') . ' where uniacid = :uniacid and is_pay = 1 and status = 1 and order_type <= 2 and type = 2 limit 5', array(':uniacid' => $_W['uniacid']));
        ...
        if (!empty($orders_group)){
            foreach ($orders_group as $order) {
                $is_allgroupgoods_fail = $AllgroupgoodsOrder->orderFail($order['active_id']);
                if ($is_allgroupgoods_fail){
                    order_status_update($order['id'], 'cancel', $is_allgroupgoods_fail);
                }
            }
        }

		if($config_takeout['handle_time_limit'] > 0) {
			$orders = pdo_fetchall('select id, sid, addtime, type, active_id from ' . tablename('tiny_wmall_order') . ' where uniacid = :uniacid and is_pay = 1 and status = 1 and order_type <= 2 and type = 0 and paytime <= :paytime limit 5', array(':uniacid' => $_W['uniacid'], ':paytime' => (time() - $config_takeout['handle_time_limit'] * 60)));
			if(!empty($orders)) {
				$extra = array(
