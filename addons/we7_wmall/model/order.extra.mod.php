...
function order_insert_member_cart($sid, $ignore_bargain = false) {
	...
	if(!empty($_GPC['goods'])) {
		...
		foreach($_GPC['goods'] as $k => $v) {
			...
			if(!$goods['is_options']) {
				...
				foreach($v['options'] as $key => $val) {
					...
					//如果是砍价、拼团、秒杀的订单，修改订单价格
					if (!empty($_GPC['goods'][88888]['type'])){
                        $goods_info[$k]['price'] = $_GPC['goods'][88888]['price'];
                        $cat_type = $_GPC['goods'][88888]['type'];
                        $active_id = $_GPC['goods'][88888]['active_id'];
                    }
					$cart_item = array(
						'title' => $title,
						'option_title' => $option['name'],
		...
		if (!empty($cat_type)){
           ...
           /* if ($cat_type==1){
               //修改销量
               $cutPrice_goods_id = pdo_get('tiny_wmall_cutPrice_user',array('id'=>$active_id),array('kid'));
               pdo_query("update " . tablename("tiny_wmall_cutPrice_goods") .  " set sale_num = sale_num + 1 , number = number - 1 where id = :id", array( ":id" => $cutPrice_goods_id['kid'] ));
           }
           //秒杀订单
           elseif ($cat_type==3){
               pdo_query("update " . tablename("tiny_wmall_secondKill_goods") .  " set sale_num = sale_num + 1 , surplus = surplus - 1 where id = :id", array( ":id" => $active_id ));
               icheckauth();
               $user_data = array('sid'=>$active_id,'uid'=>$_W["member"]["uid"],'openid'=>$_W["member"]["openid"],'nickname'=>$_W["member"]["nickname"],'headimgurl'=>$_W["member"]["avatar"],'price'=>$_GPC['goods'][88888]['price'],'createtime'=>time(),'ip'=>$_W['clientip']);
               pdo_insert('tiny_wmall_secondKill_user',$user_data);
               $data['active_id'] = pdo_insertid();
           } */
        }
		if(empty($isexist)) {
			pdo_insert('tiny_wmall_order_cart', $data);
