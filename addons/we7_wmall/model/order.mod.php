...
function order_system_status_update($id, $type, $extra = array()) {
	...
	if($type == 'pay') {
		...
		if($order['order_type'] <= 2) {
			if($store['auto_handel_order'] == 1 && $order['type'] != 2) {
				$update['status'] = 2;
				...
				//拼团
				if ($order['type'] == 2){
                    order_insert_status_log($order['id'], 'group_pay');
                }else{
                    order_insert_status_log($order['id'], 'pay');
                }
			...
			if($order['type'] == 2){
				//拼团订单支付
				$allgroupgoodsOrder = new AllgroupgoodsOrder();
				$error = $allgroupgoodsOrder->orderPaySuccess($order['active_id'], 2, $extra);
				if(is_error($error)){
					//支付失败日志记录.
				}
			}
 ...
 * @return void
 */
function calculateOrderCommission($orderInfo){
    global $_W;

    if($orderInfo['is_commission'] != 0){
        return true;
    }
    ...
    //将分佣配置字段名 = 值 组成一个数组
    $arrCommScale['commission_sale_1'] = empty($_W['we7_wmall']['config']['member']['commission_sale_1'])?0:$_W['we7_wmall']['config']['member']['commission_sale_1'];
    $arrCommScale['commission_sale_2'] = empty($_W['we7_wmall']['config']['member']['commission_sale_2'])?0:$_W['we7_wmall']['config']['member']['commission_sale_2'];
    $arrCommScale['commission_sale_3'] = empty($_W['we7_wmall']['config']['member']['commission_sale_3'])?0:$_W['we7_wmall']['config']['member']['commission_sale_3'];

    $maxCommissionNum = 3;	//分佣数量
    $currLevel = 1;
    //上级id
    $pid = $orderInfo['pid'];
    //等级小于分佣数量的话，while循环
    while($currLevel < $maxCommissionNum){
        if($pid <= 0){
            pdo_update('tiny_wmall_order', array('is_commission' => 1), array('id' => $orderInfo['id']));
            return true;
        }
        //上级的id和pid
        $userInfo = pdo_get("tiny_wmall_members",array("uid"=>$pid),array('pid'));
        if(!$userInfo){
            pdo_update('tiny_wmall_order', array('is_commission' => 1), array('id' => $orderInfo['id']));
            return true;
        }
        if(isset($arrCommScale['commission_sale_' . $currLevel])){
            //分佣百分比
            $scale = $arrCommScale['commission_sale_' . $currLevel] / 100;
            $commissionMoney = round($orderInfo['price'] * $scale,2);
            ...
            if($commissionMoney > 0){
                ...
                pdo_insert("tiny_wmall_order_commission_log",$log_data);


                //添加用户余额
                $credit_log = array( $pid, "分佣:".$commissionMoney."元" );
                mload()->model('member');
                member_credit_update($pid, "credit2", "+" .$commissionMoney, $credit_log);

                //pdo_query("update " . tablename("mc_members") .  " set credit2 = credit2 + :commissionMoney where uid = :uid", array( ":uid" => $pid , ":commissionMoney" => $commissionMoney ));
            }
        }
