	...
	if (is_weixin()) {
		if (!empty($_W['openid'])) {
			...
			if (empty($member)) {
				...
				if (empty($mc)) {
					$member = array('uniacid' => $_W['uniacid'], 'pid' => $_SESSION['pid'], 'uid' => date('His') . random(3, true), 'openid' => $fansInfo['openid'], 'nickname' => $fansInfo['nickname'], 'realname' => $fansInfo['nickname'], 'sex' => $fansInfo['sex'] == 1 ? '男' : '女', 'avatar' => rtrim(rtrim($fansInfo['headimgurl'], '0'), 132) . 132, 'is_sys' => 2, 'status' => 1, 'token' => random(32), 'addtime' => TIMESTAMP);
					pdo_insert('tiny_wmall_members', $member);
					$member['credit1'] = 0;
					$member['credit2'] = 0;
				}
				else {
					$member = array('uniacid' => $_W['uniacid'], 'pid' => $_SESSION['pid'], 'uid' => $mc['uid'], 'openid' => $_W['openid'], 'nickname' => $fans['nickname'], 'realname' => $mc['realname'], 'mobile' => $mc['mobile'], 'sex' => $fansInfo['sex'] == 1 ? '男' : '女', 'avatar' => rtrim(rtrim($fansInfo['headimgurl'], '0'), 132) . 132, 'is_sys' => 1, 'status' => 1, 'token' => random(32), 'addtime' => TIMESTAMP);
					pdo_insert('tiny_wmall_members', $member);
...
function member_register($params)
{
	...
	if (empty($mc)) {
		$member = array('uniacid' => $_W['uniacid'], 'pid' => $_SESSION['pid'], 'uid' => date('His') . random(3, true), 'openid' => $params['openid'], 'mobile' => $params['mobile'], 'nickname' => $params['nickname'], 'realname' => $params['nickname'], 'sex' => $params['sex'] == 1 ? '男' : '女', 'avatar' => rtrim(rtrim($params['headimgurl'], '0'), 132) . 132, 'is_sys' => 2, 'status' => 1, 'token' => random(32), 'addtime' => TIMESTAMP);
		pdo_insert('tiny_wmall_members', $member);
	}
	else {
		$member = array('uniacid' => $_W['uniacid'], 'pid' => $_SESSION['pid'], 'uid' => $mc['uid'], 'openid' => $params['openid'], 'nickname' => $params['nickname'], 'realname' => $mc['realname'], 'mobile' => $params['mobile'] ? $params['mobile'] : $mc['mobile'], 'sex' => $params['sex'] == 1 ? '男' : '女', 'avatar' => rtrim(rtrim($params['headimgurl'], '0'), 132) . 132, 'is_sys' => 1, 'status' => 1, 'token' => random(32), 'addtime' => TIMESTAMP);
		pdo_insert('tiny_wmall_members', $member);
	}
