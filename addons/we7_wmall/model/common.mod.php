	...
	if(empty($order_type)) {
		...
		if($order_type == 'takeout') {
			...
			foreach($payment as $key => $row) {
				if(empty($store_payment) || !in_array($row, $store_payment)) {
		...
		} elseif($order_type == 'superredpacket_meal') {
			$index = array_search('delivery', $payment);
			if($index !== false) {
				unset($payment[$index]);
			}
