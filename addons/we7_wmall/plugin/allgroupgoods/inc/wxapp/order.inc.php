...
if( $op == "create" ) 
{
	...
	$good = allgroupgoods_goods_get($goods_id);
...
if( $op == "submit" ) 
{
	$goods_id = intval($_GPC["goods_id"]);
	$good = allgroupgoods_goods_get($goods_id);
	$can = allgroupgoods_can_exchange_goods($goods_id);
	if( $can["errno"] == -2 ) 
	{
	...
	pdo_insert("tiny_wmall_allgroupgoods_order_new", $order);
	...
	if( 0 < $good["use_credit1"] && $good["use_credit2"] <= 0 ) 
	{
		...
		allgroupgoods_order_update($order_info["order_id"], "pay");
	}
...
if( $op == "detail" ) 
{
	...
	$order = allgroupgoods_order_get($order_id);
	imessage(error(0, $order), "", "ajax");
}
if( $op == "list" ) 
{
	$orders = allgroupgoods_orderall_get();
	imessage(error(0, $orders), "", "ajax");
}
if( $op == "cancel" ) 
{
	$order_id = intval($_GPC["order_id"]);
	allgroupgoods_order_update($order_id, "cancel");
