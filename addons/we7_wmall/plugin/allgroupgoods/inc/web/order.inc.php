...
if($op == 'list'){
    ...
    //订单统计
    $condition = ' where uniacid = :uniacid and status = 5 and order_type < 3 and stat_day = :stat_day AND type = 2';
    ...
    $filter_type = trim($_GPC['filter_type']) ? trim($_GPC['filter_type']) : 'all';
    $condition = ' WHERE uniacid = :uniacid and order_type < 3 AND type = 2';
    $params = array(
        ':uniacid' => $_W['uniacid'],
    );
    ...
    if(!empty($orders)) {
        ...
        foreach($orders as &$da) {
            ...
            $group_info = $AllgroupgoodsOrder->getGroupInfo($da['active_id']);
            $da['active_state'] = $group_info['state'];
        }
    }
    ...
    include itemplate('allgroupgoodsList');
}
...
if($op == 'export') {
    ...
    $condition = ' WHERE uniacid = :uniacid and order_type < 3 AND type = 3';
    ...
    $order_fields = array(
        ...
        'delivery_assign_time' => array(
            'field' => 'delivery_assign_time',
            'title' => '最后抢单时间',
            'width' => '25'
        ),
        'endtime' => array(
