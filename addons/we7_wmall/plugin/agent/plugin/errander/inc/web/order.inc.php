...
if( $op == "list" ) 
{
    ...
    //订单类型
    $order_type = ((isset($_GPC['order_type']) ? $_GPC['order_type'] : ''));
    if (!empty($order_type))
    {
    ...
    $errander_type = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall_errander_type') . ' WHERE uniacid = :uniacid AND status = 1 ORDER BY id ASC', array(':uniacid'=>$_W['uniacid']));
