...
if( $op == "transfers" ) 
{
	...
	mload()->classs("wxpay");
	$pay = new WxPay();
	$params = array( "partner_trade_no" => $log["trade_no"], "openid" => (!empty($log["account"]["openid"]) ? $log["account"]["openid"] : $deliveryer["openid"]), "check_name" => "FORCE_CHECK", "re_user_name" => $deliveryer["title"], "amount" => $log["final_fee"] * 100, "desc" => (string) $deliveryer["title"] . date("Y-m-d H:i", $log["addtime"]) . "配送费提现申请" );
