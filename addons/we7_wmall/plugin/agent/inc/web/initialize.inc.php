...
if( $op == "index" ) 
{
	...
	if( $_W["ispost"] ) 
	{
		...
		$tables = array( "tiny_wmall_activity_bargain", "tiny_wmall_activity_bargain_goods", "tiny_wmall_cube", "tiny_wmall_errander_category",  "tiny_wmall_news", "tiny_wmall_news_category", "tiny_wmall_notice",  "tiny_wmall_order_comment",  "tiny_wmall_report", "tiny_wmall_slide",  "tiny_wmall_store_category",  "tiny_wmall_text" );
		...
		foreach( $tables as $table ) 
		{
			...
			{
			    if ($agentid!=0){
                    pdo_delete($table, array( "agentid" => $agentid ,"uniacid" => $_W["uniacid"]));
                    $info = pdo_fetchall("select * from ".tablename($table)." where uniacid = :uniacid and agentid = :agentid ", array( ":uniacid" => $_W["uniacid"], ":agentid" => 0 ));
                    foreach ($info as $k=>$v)
                    {
                        unset($v['id']);
                        $v['agentid'] = $agentid;
