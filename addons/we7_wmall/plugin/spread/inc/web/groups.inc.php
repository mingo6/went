...
if( $op == "post" ) 
{
	...
	if( $_W["ispost"] ) 
	{
		$data = array( "uniacid" => $_W["uniacid"], "title" => trim($_GPC["title"]), "commission1" => floatval($_GPC["commission1"]), "commission2" => floatval($_GPC["commission2"]), "group_condition" => floatval($_GPC["group_condition"]) );
		if( !empty($ad["id"]) ) 
		{
