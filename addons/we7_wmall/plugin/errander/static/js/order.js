define(['tiny', 'map', 'laytpl', 'jquery.range'], function(tiny, AMap, laytpl) {
	...
	trade.init = function(params) {
		...
		$(document).on('click', '.popup-select-buy-address .address-buy-item', function(){
			...
			$('.popup-save-address :text[name="name"]').val(address.address + address.name);
		...
		$(document).on('click', '#save-address', function(){
			...
			$.post(tiny.getUrl('errander/address/serve_address'), address, function(data){
				...
				if(result.message.errno != -1) {
					...
					var address_html = address.address + address.name;
					if(address.number) {
						address_html = address.address + address.name + address.number;
					}
		...
		$(document).on('click', '.popup-select-buy-address .available-address-item', function(){
			...
			var address_html = $this.data('name') + $this.data('address');
			if($this.data('number')) {
				address_html = $this.data('name') + $this.data('address') + $this.data('number');
			}
			$('#start-address .item-title').removeClass('color-gray').html(address_html);
			order.start_address = {
				id: start_address_id,
		...
		$(document).on('click', '#order-submit', function(){
            ...
            var data = orderData();
			if(params.rule.type == 'express'){
				data.buy_username = $('#buy_username').val();
				data.buy_mobile = $('#buy_mobile').val();
                ...
                if(!data.buy_username) {
                    $.toast('收件人姓名不能为空');
                    return false;
                }
                var phoneReg = /(^1[3|4|5|7|8]\d{9}$)|(^09\d{8}$)/;
                if(!phoneReg.test(data.buy_mobile) ) {
                    $.toast('收件人电话格式不正确');
                    return false;
                }
                if(!data.end_address_id) {
