...
if( $op == "post" ) 
{
	...
	if( $_W["ispost"] ) 
	{
		...
		$data = array( "uniacid" => $_W["uniacid"], "uid" => $_W["member"]["uid"], "realname" => trim($_GPC["realname"]), "sex" => trim($_GPC["sex"]), "mobile" => trim($_GPC["mobile"]), "name" => trim($_GPC["name"]),"address" => trim($_GPC["address"]), "number" => trim($_GPC["number"]), "location_x" => floatval($_GPC["location_x"]), "location_y" => floatval($_GPC["location_y"]), "type" => 1 );
