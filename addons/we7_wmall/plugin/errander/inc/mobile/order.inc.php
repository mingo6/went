...
//	$start_address_id = intval($_GPC["start_address_id"]);
	$start_address_id = intval($_GPC["end_address_id"]);
	if( $category["type"] == "buy" )
	{
		$start_address = member_fetch_serve_address($start_address_id);
	}
	elseif( $category["type"] == "errand" )
    {
        $start_address_id = intval($_GPC["start_address_id"]);
        $start_address = member_fetch_serve_address($start_address_id);
    }
    elseif( $category["type"] == "express" )
    {
        $start_address = member_fetch_serve_address(intval($_GPC["start_address_id"]));
    }
	else 
	{
	...
	$order = array(
        ...
        "buy_address" => $start_address["name"] . $start_address["address"] . $start_address["number"],
        ...
        "accept_address" => $end_address["name"] . $end_address["address"] . $end_address["number"],
        "accept_location_x" => $end_address["location_x"],
        "accept_location_y" => $end_address["location_y"],
        "distance" => $fee["distance"],
        "delivery_time" => (string) $delivery_day . " " . $delivery_time,
        "goods_name" => $goods_name,
        "goods_price" => (in_array($category["type"], array( "buy", "multiaddress","errand" )) ? trim($_GPC["goods_price"]) : trim($_GPC["goods_price_cn"])),
	...
	//快递订单
    if ($category["type"] == "express")
    {
        $order['buy_username'] = $_GPC['buy_username'];
        $order['buy_mobile'] = $_GPC['buy_mobile'];
    }
	...
	$order["plateform_serve"] = iserializer(array( "fee" => $order["plateform_serve_see"], "note" => "订单配送费 ￥" . $order["delivery_fee"] . " + 订单小费 ￥" . $order["delivery_tips"] ));
	if( $_W["is_agent"] ) 
	{
...
if( $op == "list" ) 
{
	...
	$type = pdo_getall('tiny_wmall_errander_type',array('status'=>1,'uniacid'=>$_W["uniacid"]));
	$type_name = array();
	foreach ($type as $k=>$v)
    {
        $type_name[$v['value']] = $v['errander_name'];
    }
