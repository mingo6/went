...
if ($op == 'list') 
{
	...
	$params = array(':uniacid' => $_W['uniacid'],':agentid' => $agent_id);
    ...
    {
        $categorys[$k]['type'] = pdo_get("tiny_wmall_errander_type",array('uniacid' => $_W['uniacid'], 'value' => $v['type']))['errander_name'];
    }
}
if ($op == 'post') 
{
	...
	if (0 < $id) 
	{
		...
		if (empty($item)) 
		{
			imessage(error(-1, '跑腿分类不存在或已删除'), referer(), 'ajax');
		}
		...
		if (!(empty($item['deliveryers']))) 
		{
			$item['deliveryers'] = explode(',', $item['deliveryers']);
			$deliverys = array();
			foreach ($item['deliveryers'] as $value ) 
			{
				$deliverys[] = pdo_fetch('select id, title, avatar, work_status from' . tablename('tiny_wmall_deliveryer') . ' where uniacid = :uniacid and id = :id', array(':uniacid' => $_W['uniacid'], ':id' => $value));
			}
		}
	}
	if ($_W['ispost']) 
...
//		}
		...
		$deliveryers = implode(',', $deliveryer_id);
		$data['deliveryers'] = $deliveryers;
