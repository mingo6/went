...
if( $op == "list" ) 
{
	...
	if( $_W["ispost"] ) 
	{
		if( !empty($_GPC["ids"]) ) 
		{
			foreach( $_GPC["ids"] as $k => $v ) 
			{
				...
				pdo_update("tiny_wmall_secondKill_goods", $data, array( "uniacid" => $_W["uniacid"], "id" => intval($v) ));
			}
		}
		imessage(error(0, "修改成功"), iurl("secondKill/goods/list"), "ajax");
	}
	...
	$lists = pdo_fetchall("select * from" . tablename("tiny_wmall_secondKill_goods") . $condition . " order by num desc", $params);

	//商品分类
    $category = pdo_getall("tiny_wmall_secondKill_category",array("uniacid" => $_W["uniacid"]));
...
if( $op == "post" ) 
{
	...
	if( 0 < $id ) 
	{
		$item = secondKill_goods_get($id);
		$item["redpacket"] = iunserializer($item["redpacket"]);
	}
	$categorys = secondKill_category_get();
	...
	if( $_W["ispost"] ) 
	{
        ...
        if ($data['s_price']<=0 || $data['s_price']>=$data['y_price']) imessage(error(-1, "售价必须大于0且不能大于原价"), "", "ajax");
        if ($data['number']<=0) imessage(error(-1, "商品数量必须大于0"), "", "ajax");
		...
		if( 0 < $id )
		{
			pdo_update("tiny_wmall_secondKill_goods", $data, array( "uniacid" => $_W["uniacid"], "id" => $id ));

            $gid = pdo_get("tiny_wmall_secondKill_goods", array( "uniacid" => $_W["uniacid"], "id" => $id ),array('gid'));
		...
		else 
		{
			...
			pdo_insert("tiny_wmall_secondKill_goods", $data);
		}
		imessage(error(0, "编辑商品成功"), iurl("secondKill/goods/list"), "ajax");
	}
}
if( $op == "status" ) 
{
	...
	$state = pdo_update("tiny_wmall_secondKill_goods", array( "status" => $status ), array( "uniacid" => $_W["uniacid"], "id" => $id ));
	if( $state === false ) 
	{
...
if( $op == "del" ) 
{
	...
	foreach( $ids as $v ) 
	{
		pdo_delete("tiny_wmall_secondKill_goods", array( "uniacid" => $_W["uniacid"], "id" => $v ));
	}
