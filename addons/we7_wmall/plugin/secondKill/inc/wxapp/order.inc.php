...
if( $op == "create" ) 
{
	...
	$good = secondKill_goods_get($goods_id);
...
if( $op == "submit" ) 
{
	$goods_id = intval($_GPC["goods_id"]);
	$good = secondKill_goods_get($goods_id);
	$can = secondKill_can_exchange_goods($goods_id);
	if( $can["errno"] == -2 ) 
	{
	...
	pdo_insert("tiny_wmall_secondKill_order_new", $order);
	...
	if( 0 < $good["use_credit1"] && $good["use_credit2"] <= 0 ) 
	{
		...
		secondKill_order_update($order_info["order_id"], "pay");
	}
...
if( $op == "detail" ) 
{
	...
	$order = secondKill_order_get($order_id);
	imessage(error(0, $order), "", "ajax");
}
if( $op == "list" ) 
{
	$orders = secondKill_orderall_get();
	imessage(error(0, $orders), "", "ajax");
}
if( $op == "cancel" ) 
{
	$order_id = intval($_GPC["order_id"]);
	secondKill_order_update($order_id, "cancel");
