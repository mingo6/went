...
$_W["siteroot"] = str_replace("/addons/we7_wmall/plugin/secondKill", "", $_W["siteroot"]);
$_W["_plugin"] = array( "name" => "secondKill" );
pload()->func("duiba");
pload()->model("secondKill");
$uid = intval($_GPC["uid"]);
...
{
	$order = pdo_get("tiny_wmall_secondKill_order", array( "uniacid" => $_W["uniacid"], "ordernum" => $_GPC["orderMum"] ));
	if( !empty($order) ) 
	{
	...
	pdo_insert("tiny_wmall_secondKill_order", $order);
	$return = array( "status" => "ok", "errorMessage" => "", "bizId" => $order["ordersn"], "credits" => $credit1 );
	exit( json_encode($return) );
}
$order = pdo_get("tiny_wmall_secondKill_order", array( "uniacid" => $_W["uniacid"], "ordernum" => $_GPC["orderNum"] ));
if( !empty($order) ) 
{
	$success = trim($_GET["success"]);
	if( $success == "true" ) 
	{
		pdo_update("tiny_wmall_secondKill_order", array( "status" => 1 ), array( "id" => $order["id"] ));
	}
	else 
	{
		pdo_update("tiny_wmall_secondKill_order", array( "status" => 0 ), array( "id" => $order["id"] ));
