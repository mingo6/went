			...
			<div class="form-group">
				...
				<div class="col-sm-9 col-xs-9 col-md-9">
					<div class="input-group">
						<input type="number" class="form-control" name="delivery_time" value="<?php  echo $store['delivery_time'];?>" digits="true" <?php  if($store['data']['delivery_time_type'] === 0) { ?>disabled<?php  } ?>>
						<span class="input-group-addon">分钟</span>
					</div>
					<?php  if($store['data']['delivery_time_type'] === 0) { ?>
						<span class="help-block">当前设置为平台根据近30天顾客支付成功到订单完成时间计算得出的平均值,如需手动填写,请修改预计送达时间计算方式</span>
					<?php  } ?>
				</div>
			</div>
			<div class="form-group">
			...
			<div class="form-group delivery-fee-mode delivery-fee-mode-2" <?php  if($store['delivery_fee_mode'] == 2) { ?>style="display: block"<?php  } ?>>
				...
				<div class="col-sm-9 col-xs-12">
					...
					<div class="input-group" style="width: 80%;">
						...
						<div class="col-sm-9 col-xs-12">
							<div class="radio radio-inline">
								<input type="radio" name="distance_type" value="0" id="distance_type-0" <?php  if($item['delivery_price_extra']['distance_type'] == 0) { ?>checked<?php  } ?>>
								<label for="distance_type-0">直线距离</label>
							</div>
							<div class="radio radio-inline">
								<input type="radio" name="distance_type" value="2" id="distance_type-2" <?php  if($item['delivery_price_extra']['distance_type'] == 2) { ?>checked<?php  } ?>>
								<label for="distance_type-2">骑行规划距离</label>
							</div>
							<div class="radio radio-inline">
								<input type="radio" name="distance_type" value="1" id="distance_type-1" <?php  if($item['delivery_price_extra']['distance_type'] == 1) { ?>checked<?php  } ?>>
								<label for="distance_type-1">驾车导航距离</label>
							</div>
							<div class="radio radio-inline">
								<input type="radio" name="distance_type" value="3" id="distance_type-3" <?php  if($item['delivery_price_extra']['distance_type'] == 3) { ?>checked<?php  } ?>>
					...
					<div class="input-group" style="width: 80%;">
						<label class="col-xs-12 col-sm-2 col-md-2 control-label">配送距离计算方式</label>
						<div class="col-sm-9 col-xs-12">
							<div class="radio radio-inline">
								<input type="radio" name="calculate_distance_type" id="calculate_distance_type-0" value="0" <?php  if($store['delivery_price_extra']['calculate_distance_type'] == 0 || !$store['delivery_price_extra']['calculate_distance_type']) { ?>checked<?php  } ?>>
								<label for="calculate_distance_type-0">默认</label>
							</div>
							<div class="radio radio-inline">
								<input type="radio" name="calculate_distance_type" id="calculate_distance_type-1" value="1" <?php  if($store['delivery_price_extra']['calculate_distance_type'] == 1) { ?>checked<?php  } ?>>
								<label for="calculate_distance_type-1">向上取整</label>
							</div>
							<div class="radio radio-inline">
								<input type="radio" name="calculate_distance_type" id="calculate_distance_type-2" value="2" <?php  if($store['delivery_price_extra']['calculate_distance_type'] == 2) { ?>checked<?php  } ?>>
...
此项设置的费用将进入平台的收益，与配送员的订单提成没有关系</span>
			...
			<div class="form-group">
				...
				<div class="col-sm-9 col-xs-12">
					<div class="input-group">
						<span class="input-group-addon">每单</span>
						<input type="number" name="plateform_bear_deliveryprice" class="form-control" value="<?php  echo $store['delivery_extra']['plateform_bear_deliveryprice'];?>">
						<span class="input-group-addon">元</span>
					</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-9 col-xs-12">
				<div class="radio radio-inline">
					<input type="radio" value="1" name="custom_goods_sailed_status" id="custom-goods-sailed-status-1" <?php  if($store['data']['custom_goods_sailed_status'] == 1) { ?>checked<?php  } ?>>
					<label for="custom-goods-sailed-status-1">允许</label>
				</div>
				<div class="radio radio-inline">
					<input type="radio" value="0" name="custom_goods_sailed_status" id="custom-goods-sailed-status-0" <?php  if($store['data']['custom_goods_sailed_status'] == 0 ||  !$store['data']['custom_goods_sailed_status']) { ?>checked<?php  } ?>>
		...
		<div class="form-group">
			...
			<div class="col-sm-8 col-xs-12">
				<div class="radio radio-inline">
					<input type="radio" value="1" name="self_audit_comment" id="self_audit_comment-1" required="true" onclick="$('.comment').show();" <?php  if($store['self_audit_comment'] == 1) { ?>checked<?php  } ?>>
					<label for="self_audit_comment-1">允许</label>
				</div>
				<div class="radio radio-inline">
					<input type="radio" value="0" name="self_audit_comment" id="self_audit_comment-0" onclick="$('.comment').hide();" <?php  if($store['self_audit_comment'] == 0 || !$store['self_audit_comment']) { ?>checked<?php  } ?>>
					<label for="self_audit_comment-0">不允许</label>
				</div>
				<span class="help-block">注意:设置为不允许后,顾客对于商家的评价将直接审核通过并显示到手机端</span>
			</div>
		</div>
		<div class="form-group comment" <?php  if($store['self_audit_comment'] == 0) { ?> style="display: none" <?php  } ?>>
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">顾客评价是否需要直接通过审核</label>
			<div class="col-sm-8 col-xs-12">
				<div class="radio radio-inline">
					<input type="radio" value="1" name="comment_status" id="comment_status-1" <?php  if($store['comment_status'] == 1) { ?>checked<?php  } ?>>
					<label for="comment_status-1">是</label>
				</div>
				<div class="radio radio-inline">
					<input type="radio" value="0" name="comment_status" id="comment_status-0" <?php  if($store['comment_status'] == 0 || !$store['comment_status']) { ?>checked<?php  } ?>>
		...
		<div class="form-group">
			...
			<div class="col-sm-9 col-xs-12">
				<div class="radio radio-inline">
					<input type="radio" value="1" name="delivery_time_type" id="delivery_time_type-1" <?php  if($store['data']['delivery_time_type'] == 1) { ?>checked<?php  } ?>>
					<label for="delivery_time_type-1">门店手动设置</label>
				</div>
				<div class="radio radio-inline">
					<input type="radio" value="0" name="delivery_time_type" id="delivery_time_type-0" <?php  if($store['data']['delivery_time_type'] == 0 ||  !$store['data']['delivery_time_type']) { ?>checked<?php  } ?>>
					...
					<div class="col-sm-9 col-xs-12">
						...
						<div class="col-sm-9 col-xs-12">
							<div class="input-group">
								...
								<input type="text" name="fee_takeout[fee_min]" value="<?php  echo $account['fee_takeout']['fee_min'];?>" class="form-control"/>
								<span class="input-group-addon">元</span>
							</div>
						</div>
					</div>
					...
					<div class="col-sm-9 col-xs-12">
						...
						<div class="col-sm-9 col-xs-12">
							<div class="input-group">
								...
								<span class="input-group-addon">%，最低抽成金额</span>
								<input type="text" name="fee_selfDelivery[fee_min]" value="<?php  echo $account['fee_selfDelivery']['fee_min'];?>" class="form-control"/>
								<span class="input-group-addon">元</span>
							</div>
						</div>
					</div>
					...
					<div class="col-sm-9 col-xs-12">
						...
						<div class="col-sm-9 col-xs-12">
							<div class="input-group">
								...
								<span class="input-group-addon">%，最低抽成金额</span>
								<input type="text" name="fee_instore[fee_min]" value="<?php  echo $account['fee_instore']['fee_min'];?>" class="form-control"/>
								<span class="input-group-addon">元</span>
							</div>
						</div>
					</div>
		...
		<div class="form-group">
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">特殊商品佣金计算方式（外卖单，自提单，店内单，佣金计算方式设置为按百分比抽成时才有效）</label>
			<div class="col-sm-9 col-xs-12 toggle-tabs">
				<div class="col-sm-12 col-xs-12 col-md-12" id="fee-goods-container">
					<?php  if(is_array($account['fee_goods'])) { foreach($account['fee_goods'] as $k => $v) { ?>
					<div class="fee-goods-item">
						<label class="col-xs-12 col-sm-3 col-md-2 control-label" id="goods-title-<?php  echo $k;?>"><?php  echo $v['title'];?></label>
						<div class="col-sm-9 col-xs-12 col-md-10 toggle-tabs" data-content=".fee-goods-type-<?php  echo $k;?>" id="goods-fee-<?php  echo $k;?>">
							<div class="form-group">
								<div class="radio radio-inline">
									<input type="radio" value="1" name="fee_goods[<?php  echo $k;?>][type]" id="fee_goods-type-<?php  echo $k;?>-1" <?php  if($v['type'] == 1 || empty($v['type'])) { ?>checked<?php  } ?>>
									<label for="fee_goods-type-<?php  echo $k;?>-1" class="toggle-role" data-target="fee_goods-<?php  echo $k;?>-1">订单百分比抽成</label>
								</div>
								<div class="radio radio-inline">
									<input type="radio" value="2" name="fee_goods[<?php  echo $k;?>][type]" id="fee_goods-type-<?php  echo $k;?>-2" <?php  if($v['type'] == 2) { ?>checked<?php  } ?>>
									<label for="fee_goods-type-<?php  echo $k;?>-2" class="toggle-role" data-target="fee_goods-<?php  echo $k;?>-2">固定抽成</label>
								</div>
							</div>
							<div class="toggle-content fee-goods-type-<?php  echo $k;?>">
								<div class="toggle-pane <?php  if($v['type'] == 1 || empty($v['type'])) { ?>active<?php  } ?>" id="fee_goods-<?php  echo $k;?>-1">
									<div class="form-group">
										<div class="col-sm-12 col-xs-12">
											<label class="col-xs-12 col-sm-3 col-md-2 control-label">抽成比例：</label>
											<div class="col-sm-9 col-xs-12">
												<div class="input-group">
													<input type="text" name="fee_goods[<?php  echo $k;?>][fee_rate]" value="<?php  echo $v['fee_rate'];?>" class="form-control"/>
								...
								<div class="toggle-pane <?php  if($v['type'] == 2) { ?>active<?php  } ?>" id="fee_goods-<?php  echo $k;?>-2">
									<div class="form-group">
										<div class="col-sm-12 col-xs-12">
											<label class="col-xs-12 col-sm-3 col-md-2 control-label">抽成金额：</label>
											<div class="col-sm-9 col-xs-12">
												<div class="input-group">
													<input type="text" name="fee_goods[<?php  echo $k;?>][fee]" value="<?php  echo $v['fee'];?>" class="form-control"/>
													<span class="input-group-addon">元</span>
												</div>
											</div>
										</div>
						...
						<div class="col-md-12">
							...
							<a class="btn btn-danger btn-sm goods-del-btn" href="javascript:;">删除</a>
						</div>
					</div>
					<?php  } } ?>
				</div>
				<a class="btn btn-primary btn-sm " id="btn-select-goods" href="javascript:;">添加商品</a>
			</div>
		</div>
		<div class="form-group">
					...
					<div class="col-sm-9 col-xs-12">
						...
						<div class="col-sm-9 col-xs-12">
							<div class="input-group">
								...
								<span class="input-group-addon">%，最低抽成金额</span>
								<input type="text" name="fee_paybill[fee_min]" value="<?php  echo $account['fee_paybill']['fee_min'];?>" class="form-control"/>
								<span class="input-group-addon">元</span>
							</div>
						</div>
					</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-9 col-xs-12">
				<div class="input-group">
					<input type="number" name="fee_period" value="<?php  echo $account['fee_period'];?>" class="form-control"/>
		...
		<div class="form-group">
			...
			<div class="col-sm-9 col-xs-12">
				<div id="extra-fee-container">
					<?php  if(!empty($store['data']['extra_fee'])) { ?>
						<?php  if(is_array($store['data']['extra_fee'])) { foreach($store['data']['extra_fee'] as $fee) { ?>
							<div class="attr-item">
								<div class="input-group">
					...
					<div class="col-sm-9 col-xs-12">
						...
						<div class="col-sm-9 col-xs-12">
							<div class="input-group">
								...
								<span class="input-group-addon">%，最低抽成金额</span>
								<input type="text" name="fee_eleme[fee_min]" value="<?php  echo $account['fee_eleme']['fee_min'];?>" class="form-control"/>
								<span class="input-group-addon">元</span>
							</div>
						</div>
					</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-9 col-xs-12">
				<div class="radio radio-inline">
					<input type="radio" value="0" name="eleme[delivery_mode]" id="eleme-delivery-mode-0" <?php  if(empty($store['data']['eleme']['delivery']['delivery_mode'])) { ?>checked<?php  } ?> onclick="$('.eleme-delivery-fee').addClass('hide');">
					<label for="eleme-delivery-mode-0">饿了么配送</label>
				</div>
				<div class="radio radio-inline">
					<input type="radio" value="1" name="eleme[delivery_mode]" id="eleme-delivery-mode-1" <?php  if($store['data']['eleme']['delivery']['delivery_mode'] == 1) { ?>checked<?php  } ?> onclick="$('.eleme-delivery-fee').addClass('hide');">
					<label for="eleme-delivery-mode-1">店内配送</label>
				</div>
				<div class="radio radio-inline">
					<input type="radio" value="2" name="eleme[delivery_mode]" id="eleme-delivery-mode-2" <?php  if($store['data']['eleme']['delivery']['delivery_mode'] == 2) { ?>checked<?php  } ?> onclick="$('.eleme-delivery-fee').removeClass('hide');">
					<label for="eleme-delivery-mode-2">平台配送</label>
				</div>
			</div>
		</div>
		<div class="eleme-delivery-fee <?php  if($store['data']['eleme']['delivery']['delivery_mode'] != 2) { ?>hide<?php  } ?>" >
			<div class="form-group">
				<label class="col-xs-12 col-sm-3 col-md-2 control-label">平台配送费</label>
				<div class="col-sm-9 col-xs-12">
					<div class="radio radio-inline">
						<input type="radio" name="eleme[delivery_fee_mode]" id="eleme-delivery-fee-mode-1" value="1" <?php  if($store['data']['eleme']['delivery']['delivery_fee_mode'] == 1 || !$store['data']['eleme']['delivery']['delivery_fee_mode']) { ?>checked<?php  } ?> onclick="$('.eleme-delivery-fee-mode').addClass('hide');$('.eleme-delivery-fee-mode-1').removeClass('hide');">
						<label for="eleme-delivery-fee-mode-1">固定金额</label>
					</div>
					<div class="radio radio-inline">
						<input type="radio" name="eleme[delivery_fee_mode]" id="eleme-delivery-fee-mode-2" value="2" <?php  if($store['data']['eleme']['delivery']['delivery_fee_mode'] == 2) { ?>checked<?php  } ?> onclick="$('.eleme-delivery-fee-mode').addClass('hide');$('.eleme-delivery-fee-mode-2').removeClass('hide');">
						<label for="eleme-delivery-fee-mode-2">按距离收取</label>
					</div>
				</div>
			</div>
			<div class="eleme-delivery-fee-mode eleme-delivery-fee-mode-1 <?php  if($store['data']['eleme']['delivery']['delivery_fee_mode'] == 2) { ?>hide<?php  } ?>">
				<div class="form-group">
					<label class="col-xs-12 col-sm-3 col-md-2 control-label"></label>
					<div class="col-sm-9 col-xs-12">
						<div class="input-group">
							<div class="input-group-addon">每单</div>
							<input type="number" name="eleme[delivery_price]" value="<?php  echo $store['data']['eleme']['delivery']['delivery_price'];?>" class="form-control"/>
							<div class="input-group-addon">元</div>
						</div>
					</div>
				</div>
			</div>
			<div class="form-group eleme-delivery-fee-mode eleme-delivery-fee-mode-2 <?php  if($store['data']['eleme']['delivery']['delivery_fee_mode'] == 1 || !$store['data']['eleme']['delivery']['delivery_fee_mode']) { ?>hide<?php  } ?>">
				<label class="col-xs-12 col-sm-3 col-md-2 control-label"></label>
				<div class="col-sm-9 col-xs-12">
					<div class="input-group">
						<span class="input-group-addon">起步价</span>
						<input type="number" class="form-control" name="eleme[start_fee]" value="<?php  echo $store['data']['eleme']['delivery']['delivery_price']['start_fee'];?>">
						<span class="input-group-addon">元包含</span>
						<input type="number" class="form-control" name="eleme[start_km]" value="<?php  echo $store['data']['eleme']['delivery']['delivery_price']['start_km'];?>">
						<span class="input-group-addon">公里</span>
					</div>
					<br>
					<div class="input-group">
						<span class="input-group-addon">每增加1公里加</span>
						<input type="number" class="form-control" name="eleme[pre_km_fee]" value="<?php  echo $store['data']['eleme']['delivery']['delivery_price']['pre_km_fee'];?>">
						<span class="input-group-addon">元</span>
					</div>
					<div class="help-block">
					...
					<div class="col-sm-9 col-xs-12">
						...
						<div class="col-sm-9 col-xs-12">
							<div class="input-group">
								...
								<span class="input-group-addon">%，最低抽成金额</span>
								<input type="text" name="fee_meituan[fee_min]" value="<?php  echo $account['fee_meituan']['fee_min'];?>" class="form-control"/>
								<span class="input-group-addon">元</span>
							</div>
						</div>
					</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-9 col-xs-12">
				<div class="radio radio-inline">
					<input type="radio" value="0" name="meituan[delivery_mode]" id="meituan-delivery-mode-0" <?php  if(empty($store['data']['meituan']['delivery']['delivery_mode'])) { ?>checked<?php  } ?> onclick="$('.meituan-delivery-fee').addClass('hide');">
					<label for="meituan-delivery-mode-0">美团配送</label>
				</div>
				<div class="radio radio-inline">
					<input type="radio" value="1" name="meituan[delivery_mode]" id="meituan-delivery-mode-1" <?php  if($store['data']['meituan']['delivery']['delivery_mode'] == 1) { ?>checked<?php  } ?> onclick="$('.meituan-delivery-fee').addClass('hide');">
					<label for="meituan-delivery-mode-1">店内配送</label>
				</div>
				<div class="radio radio-inline">
					<input type="radio" value="2" name="meituan[delivery_mode]" id="meituan-delivery-mode-2" <?php  if($store['data']['meituan']['delivery']['delivery_mode'] == 2) { ?>checked<?php  } ?> onclick="$('.meituan-delivery-fee').removeClass('hide');">
					<label for="meituan-delivery-mode-2">平台配送</label>
				</div>
			</div>
		</div>
		<div class="meituan-delivery-fee <?php  if($store['data']['meituan']['delivery']['delivery_mode'] != 2) { ?>hide<?php  } ?>" >
			<div class="form-group">
				<label class="col-xs-12 col-sm-3 col-md-2 control-label">平台配送费</label>
				<div class="col-sm-9 col-xs-12">
					<div class="radio radio-inline">
						<input type="radio" name="meituan[delivery_fee_mode]" id="meituan-delivery-fee-mode-1" value="1" <?php  if($store['data']['meituan']['delivery']['delivery_fee_mode'] == 1 || !$store['data']['meituan']['delivery']['delivery_fee_mode']) { ?>checked<?php  } ?> onclick="$('.meituan-delivery-fee-mode').addClass('hide');$('.meituan-delivery-fee-mode-1').removeClass('hide');">
						<label for="meituan-delivery-fee-mode-1">固定金额</label>
					</div>
					<div class="radio radio-inline">
						<input type="radio" name="meituan[delivery_fee_mode]" id="meituan-delivery-fee-mode-2" value="2" <?php  if($store['data']['meituan']['delivery']['delivery_fee_mode'] == 2) { ?>checked<?php  } ?> onclick="$('.meituan-delivery-fee-mode').addClass('hide');$('.meituan-delivery-fee-mode-2').removeClass('hide');">
						<label for="meituan-delivery-fee-mode-2">按距离收取</label>
					</div>
				</div>
			</div>
			<div class="meituan-delivery-fee-mode meituan-delivery-fee-mode-1 <?php  if($store['data']['meituan']['delivery']['delivery_fee_mode'] == 2) { ?>hide<?php  } ?>">
				<div class="form-group">
					<label class="col-xs-12 col-sm-3 col-md-2 control-label"></label>
					<div class="col-sm-9 col-xs-12">
						<div class="input-group">
							<div class="input-group-addon">每单</div>
							<input type="number" name="meituan[delivery_price]" value="<?php  echo $store['data']['meituan']['delivery']['delivery_price'];?>" class="form-control"/>
							<div class="input-group-addon">元</div>
						</div>
					</div>
				</div>
			</div>
			<div class="form-group meituan-delivery-fee-mode meituan-delivery-fee-mode-2 <?php  if($store['data']['meituan']['delivery']['delivery_fee_mode'] == 1 || !$store['data']['meituan']['delivery']['delivery_fee_mode']) { ?>hide<?php  } ?>">
				<label class="col-xs-12 col-sm-3 col-md-2 control-label"></label>
				<div class="col-sm-9 col-xs-12">
					<div class="input-group">
						<span class="input-group-addon">起步价</span>
						<input type="number" class="form-control" name="meituan[start_fee]" value="<?php  echo $store['data']['meituan']['delivery']['delivery_price']['start_fee'];?>">
						<span class="input-group-addon">元包含</span>
						<input type="number" class="form-control" name="meituan[start_km]" value="<?php  echo $store['data']['meituan']['delivery']['delivery_price']['start_km'];?>">
						<span class="input-group-addon">公里</span>
					</div>
					<br>
					<div class="input-group">
						<span class="input-group-addon">每增加1公里加</span>
						<input type="number" class="form-control" name="meituan[pre_km_fee]" value="<?php  echo $store['data']['meituan']['delivery']['delivery_price']['pre_km_fee'];?>">
						<span class="input-group-addon">元</span>
					</div>
					<div class="help-block">
		...
		<?php  } ?>
		<?php  if(check_plugin_perm('superCoupon')) { ?>
			<h3>超级优惠券</h3>
			<div class="form-group">
				<label class="col-xs-12 col-sm-3 col-md-2 control-label">是否允许商家使用</label>
				<div class="col-sm-9 col-xs-12">
					<div class="radio radio-inline">
						<input type="radio" value="1" name="superCoupon[status]" id="superCoupon_status-1" <?php  if($store['data']['superCoupon']['status'] == 1) { ?>checked<?php  } ?>>
						<label for="superCoupon_status-1">允许</label>
					</div>
					<div class="radio radio-inline">
						<input type="radio" value="0" name="superCoupon[status]" id="superCoupon_status-0" <?php  if(!$store['data']['superCoupon']['status']) { ?>checked<?php  } ?>>
						<label for="superCoupon_status-0">不允许</label>
					</div>
				</div>
			</div>
			<div class="form-group">
				<label class="col-xs-12 col-sm-3 col-md-2 control-label">发放数量限制</label>
				<div class="col-sm-9 col-xs-12">
					<div class="input-group">
						<div class="input-group-addon">每月最多发放</div>
						<input type="number" name="superCoupon[max_limit]" value="<?php  echo $store['data']['superCoupon']['max_limit'];?>" class="form-control"/>
...
<script type="text/html" id="tpl-goods-item">
	<{# for(var i = 0, len = d.length; i < len; i++){ }>
	<div class="fee-goods-item">
		<label class="col-xs-12 col-sm-3 col-md-2 control-label"><{d[i].title}></label>
		<div class="col-sm-9 col-xs-12 col-md-10 toggle-tabs" data-content=".fee-goods-type-<{d[i].id}>">
			<div class="form-group">
				<div class="radio radio-inline">
					<input type="radio" value="1" name="fee_goods[<{d[i].id}>][type]" id="fee_goods-type-<{d[i].id}>-1" checked>
					<label for="fee_goods-type-<{d[i].id}>-1" class="toggle-role" data-target="fee_goods-<{d[i].id}>-1">订单百分比抽成</label>
				</div>
				<div class="radio radio-inline">
					<input type="radio" value="2" name="fee_goods[<{d[i].id}>][type]" id="fee_goods-type-<{d[i].id}>-2">
					<label for="fee_goods-type-<{d[i].id}>-2" class="toggle-role" data-target="fee_goods-<{d[i].id}>-2">固定抽成</label>
				</div>
			</div>
			<div class="toggle-content fee-goods-type-<{d[i].id}>">
				<div class="toggle-pane active" id="fee_goods-<{d[i].id}>-1">
					<div class="form-group">
						<div class="col-sm-12 col-xs-12">
							<label class="col-xs-12 col-sm-3 col-md-2 control-label">抽成比例：</label>
							<div class="col-sm-9 col-xs-12">
								<div class="input-group">
									<input type="text" name="fee_goods[<{d[i].id}>][fee_rate]" value="" class="form-control"/>
									<span class="input-group-addon">%</span>
								</div>
							</div>
						</div>
				...
				<div class="toggle-pane" id="fee_goods-<{d[i].id}>-2">
					<div class="form-group">
						<div class="col-sm-12 col-xs-12">
							<label class="col-xs-12 col-sm-3 col-md-2 control-label">抽成金额：</label>
							<div class="col-sm-9 col-xs-12">
								<div class="input-group">
									<input type="text" name="fee_goods[<{d[i].id}>][fee]" value="" class="form-control"/>
									<span class="input-group-addon">元</span>
								</div>
							</div>
						</div>
...
irequire(['laytpl', 'tiny'], function(laytpl, tiny){
	$('#btn-select-goods').click(function(){
		tiny.selectgoods(function(goods){
			...
			for(var n in goods) {
				if(goods[n]['id']) {
					$('#goods-title-' + goods[n]['id']).remove();
					$('#goods-fee-' + goods[n]['id']).remove();
				}
			}
			var gettpl = $('#tpl-goods-item').html();
			laytpl(gettpl).render(goods, function(html){
				$('#fee-goods-container').append(html);
			});
		}, {mutil: 1, is_options: 0, store_id: "<?php  echo $store['id'];?>"});
	});
	// 特殊佣金删除商品
	$(document).on('click', '.goods-del-btn', function(){
