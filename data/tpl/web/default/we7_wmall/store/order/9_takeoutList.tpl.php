			...
			<div class="col-sm-9 col-xs-12">
				<div class="btn-group" style="margin-right: 10px;">
					<div class="btn-group">
						...
						<a href="<?php  echo ifilter_url('order_channel: h5app');?>" class="btn <?php  if($order_channel == 'h5app') { ?>btn-primary<?php  } else { ?>btn-default<?php  } ?>">app</a>
					</div>
				</div>
	...
	<div class="col-md-8">
		...
		<div class="panel-order <?php  echo $order['order_plateform'];?>">
			...
			<div class="user-info">
				...
				<a href="<?php  echo ifilter_url('filter_type:all,uid:' . $order['uid']);?>" class="greenest pull-right" <?php  if($order['label']['color']) { ?>style="color: <?php  echo $order['label']['color'];?>;"<?php  } ?>>查看用户历史订单</a>
			...
			<div class="product-info">
				<p class="product-title">
					...
					<span class="pull-right greenest toggle-product" <?php  if($order['label']['color']) { ?>style="color: <?php  echo $order['label']['color'];?>;"<?php  } ?>>收起 <i class="fa fa-angle-up"></i></span>
				</p>
			...
			<div class="btn-area">
				<?php  if($order['status'] < 5) { ?>
					...
					<?php  if($order['order_type'] == 1) { ?>
						<?php  if($order['status'] == 2 || $order['status'] == 3) { ?>
							...
							<?php  if($store['delivery_mode'] == 1) { ?>
								...
								<?php  if($store['data']['dada']['status'] == 1) { ?>
									<a href="<?php  echo iurl('store/order/takeout/push_dada', array('id' => $order['id']));?>" class="btn btn-primary btn-sm js-post" data-confirm="确定推送到达达配送?">推送到达达配送</a>
								<?php  } ?>
							<?php  } ?>
						<?php  } ?>
						<?php  if($order['status'] > 1 && $order['delivery_type'] == 1) { ?>
					...
					<?php  } else { ?>
						...
						<?php  } ?>
					<?php  } ?>
					<?php  if($order['status'] == 1 || ($order['status'] > 1 && $config_takeout['cancel_after_handle'] == 1)) { ?>
						<a href="<?php  echo iurl('store/order/takeout/cancel', array('id' => $order['id']));?>" class="btn btn-primary btn-sm js-modal" data-batch="modal">
