...
<!--	<?php  if(check_perm('deliveryer.account')) { ?>
	...
	<?php  } ?>-->
	<?php  if(check_perm('deliveryer.plateform') || check_perm('deliveryer.getcash' || 'deliveryer.current' || 'deliveryer.group')) { ?>
		...
		<ul class="menu-item">
			...
			<?php  } ?>
<!--
			<?php  if(check_perm('deliveryer.group')) { ?>
			<li <?php  if($_W['_action'] == 'group') { ?>class="active"<?php  } ?>>
			<a href="<?php  echo iurl('deliveryer/group/index');?>">配送员等级</a>
			</li>
			<?php  } ?>
-->
