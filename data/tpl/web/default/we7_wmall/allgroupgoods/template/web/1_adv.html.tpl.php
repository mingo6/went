...
<form action="" class="form-table form form-validate" method="post">
	<div class="panel panel-table">
		<div class="panel-heading">
			<a href="<?php  echo iurl('allgroupgoods/adv/post');?>" class="btn btn-primary btn-sm">添加幻灯片</a>
		</div>
		<div class="panel-body table-responsive js-table">
			...
			<?php  } else { ?>
				<table class="table table-hover">
					...
					<?php  if(is_array($slides)) { foreach($slides as $slide) { ?>
						<tr>
							...
							<td>
								<input type="checkbox" class="js-checkbox" data-on-text="开启" data-off-text="关闭" data-href="<?php  echo iurl('allgroupgoods/adv/status', array('id' => $slide['id']));?>" data-name="status" value="1" <?php  if($slide['status'] == 1) { ?>checked<?php  } ?>>
							</td>
							<td class="text-right">
								<a href="<?php  echo iurl('allgroupgoods/adv/post', array('id' => $slide['id']))?>" class="btn btn-default btn-sm" title="编辑" data-toggle="tooltip" data-placement="top" ><i class="fa fa-edit"> </i> 编辑</a>
								<a href="<?php  echo iurl('allgroupgoods/adv/del', array('id' => $slide['id']))?>" class="btn btn-default btn-sm js-remove" data-confirm="确定删除该幻灯片?"><i class="fa fa-times"> </i> 删除</a>
							</td>
						</tr>
					<?php  } } ?>
				</table>
				<div class="btn-region clearfix">
					...
					<a href="<?php  echo iurl('allgroupgoods/adv/del')?>" class="btn btn-danger btn-sm js-batch" data-batch="remove" data-confirm="删除后将不可恢复，确定删除吗">批量删除</a>
				</div>
			<?php  } ?>
		</div>
	</div>
