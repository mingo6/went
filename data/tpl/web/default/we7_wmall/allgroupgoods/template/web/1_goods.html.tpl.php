...
<div class="page clearfix">
	<form class="form-horizontal form form-validate" id="form1" action="" method="post" enctype="multipart/form-data">
		...
		<div class="form-group">
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">请选择商户</label>
			<div class="col-sm-9 col-xs-12">
				<select class="form-control" name="sid" >
					<?php  if(is_array($stores)) { foreach($stores as $store) { ?>
						<option value="<?php  echo $store['id'];?>" <?php  if($store['id'] == $item['sid']) { ?>selected<?php  } ?>><?php  echo $store['title'];?></option>
					<?php  } } ?>
				</select>
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-9 col-xs-12">
				<div class="input-group">
					<input type="text" class="form-control" name="pt_price" value="<?php  echo $item['pt_price'];?>" required="true">
					<div class="input-group-addon">元</div>
				</div>
			</div>
		</div>
		<div class="form-group">
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">单独购价格</label>
			<div class="col-sm-9 col-xs-12">
				<div class="input-group">
					<input type="text" class="form-control" name="dd_price" value="<?php  echo $item['dd_price'];?>" required="true">
					<div class="input-group-addon">元</div>
				</div>
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-sm-9 col-xs-12">
				<div class="input-group">
					<input type="number" class="form-control" name="people" value="<?php  echo $item['people'];?>" required="true">
		...
		<div class="form-group">
			...
			<div class="col-sm-9 col-xs-12">
				<input type="text" class="form-control" name="inventory" value="<?php  echo $item['inventory'];?>" required="true">
			</div>
		</div>
		<div class="form-group">
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">已成团数量</label>
			<div class="col-sm-9 col-xs-12">
				<input type="text" class="form-control" name="ycd_num" value="<?php  echo $item['ycd_num'];?>">
			</div>
		</div>
		<div class="form-group">
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">商品销量</label>
			<div class="col-sm-9 col-xs-12">
				<input type="text" class="form-control" name="ysc_num" value="<?php  echo $item['ysc_num'];?>">
			</div>
		</div>
...
<form action="./index.php" class="form-horizontal form-filter">
	<?php  echo tpl_form_filter_hidden('allgroupgoods/goods/list');?>
	<input type="hidden" name="ta" value="list"/>
	<div class="form-group">
		<label class="col-xs-12 col-sm-3 col-md-2 control-label">商品类型</label>
		<div class="col-sm-9 col-xs-12">
			<div class="btn-group">
				<a class="btn <?php  if(!$type) { ?>btn-primary<?php  } else { ?>btn-default<?php  } ?>" href="<?php  echo iurl('allgroupgoods/goods/list')?>">不限</a>
				<?php  if(is_array($category)) { foreach($category as $item) { ?>
				<a class="btn <?php  if($type == $item['id']) { ?>btn-primary<?php  } else { ?>btn-default<?php  } ?>" href="<?php  echo iurl('allgroupgoods/goods/list', array('type' => $item['id']))?>"><?php  echo $item['name'];?></a>
				<?php  } } ?>
			</div>
		</div>
	</div>
...
<form action="" class="form-table form form-validate" method="post">
	<div class="panel panel-table">
		<div class="panel-heading">
			<a href="<?php  echo iurl('allgroupgoods/goods/post');?>" class="btn btn-primary btn-sm">添加商品</a>
		</div>
		<?php  if(!empty($lists)) { ?>
		<div class="panel-body table-responsive js-table">
			<table class="table table-hover">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<!--<input type="text" name="pt_price[]" class="form-control width-100" value="<?php  echo $item['pt_price'];?>">-->
							<?php  echo $item['pt_price'];?>
						</td>
						<td>
							<!--<input type="text" name="dd_price[]" class="form-control width-100" value="<?php  echo $item['dd_price'];?>">-->
							<?php  echo $item['dd_price'];?>
						</td>
						<td>
							<?php  echo date('Y-m-d H:i:s',$item['create_time'])?>
						</td>
						<td>
							<!--<input type="text" name="people[]" class="form-control width-100" value="<?php  echo $item['people'];?>">-->
							<?php  echo $item['people'];?>
						</td>
						<td>
							<!--<input type="text" name="inventory[]" class="form-control width-100" value="<?php  echo $item['inventory'];?>">-->
							<?php  echo $item['inventory'];?>
						</td>
						<td>
							<!--<input type="text" name="inventory[]" class="form-control width-100" value="<?php  echo $item['sale_num'];?>">-->
						...
						<!--<td>-->
							<!--<input type="checkbox" class="js-checkbox" data-href="<?php  echo iurl('allgroupgoods/goods/status', array('id' => $item['id']));?>" data-name="status" value="<?php  echo $item['status'];?>" <?php  if($item['status'] == 1) { ?>checked<?php  } ?>>-->
						<!--</td>-->
						<td style="text-align:right;">
							<a href="<?php  echo iurl('allgroupgoods/goods/post', array('id' => $item['id']))?>" class="btn btn-default btn-sm" title="编辑" data-toggle="tooltip" data-placement="top"><i class="fa fa-edit"> </i></a>
							<a href="<?php  echo iurl('allgroupgoods/goods/del', array('id' => $item['id']))?>" class="btn btn-default btn-sm js-remove" title="删除" data-toggle="tooltip" data-placement="top" data-confirm="删除后将不可恢复，确定删除吗?"><i class="fa fa-times"> </i></a>
						</td>
					</tr>
					<?php  } } ?>
				</tbody>
			...
			<div class="btn-region clearfix">
				<div class="pull-left">
					...
					<a href="<?php  echo iurl('allgroupgoods/goods/del')?>" class="btn btn-danger btn-sm js-batch" data-batch="remove" data-confirm="确定删除选中的商品吗?">批量删除</a>
				</div>
