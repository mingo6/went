...
<div class="page clearfix">
	<form class="form-horizontal form form-validate" id="form1" action="" method="post" enctype="multipart/form-data">
		...
		<div class="form-group">
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">每次砍价最小金额</label>
			<div class="col-sm-9 col-xs-12">
				<input type="text" class="form-control" name="cut_min_price" value="<?php  echo $item['cut_min_price'];?>" required="true">
			</div>
		</div>
		<div class="form-group">
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">每次砍价最大金额</label>
			<div class="col-sm-9 col-xs-12">
				<input type="text" class="form-control" name="cut_max_price" value="<?php  echo $item['cut_max_price'];?>" required="true">
			</div>
		</div>
		<div class="form-group">
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">限制砍价人数</label>
			<div class="col-sm-9 col-xs-12">
				<div class="input-group">
					<input type="text" class="form-control" name="cut_people_num" value="<?php  echo $item['cut_people_num'];?>" required="true">
		...
		<div class="form-group">
			...
			<div class="col-sm-9 col-xs-12">
				<input type="text" class="form-control" name="sale_num" value="<?php  echo $item['sale_num'];?>">
			</div>
		</div>
...
<form action="./index.php" class="form-horizontal form-filter">
	<?php  echo tpl_form_filter_hidden('cutPrice/goods/list');?>
	<input type="hidden" name="ta" value="list"/>
	<div class="form-group">
		<label class="col-xs-12 col-sm-3 col-md-2 control-label">商品类型</label>
		<div class="col-sm-9 col-xs-12">
			<div class="btn-group">
				<a class="btn <?php  if(!$type) { ?>btn-primary<?php  } else { ?>btn-default<?php  } ?>" href="<?php  echo iurl('cutPrice/goods/list')?>">不限</a>
				<?php  if(is_array($category)) { foreach($category as $item) { ?>
				<a class="btn <?php  if($type == $item['id']) { ?>btn-primary<?php  } else { ?>btn-default<?php  } ?>" href="<?php  echo iurl('cutPrice/goods/list', array('type' => $item['id']))?>"><?php  echo $item['name'];?></a>
				<?php  } } ?>
			</div>
		</div>
	</div>
...
<form action="" class="form-table form form-validate" method="post">
	<div class="panel panel-table">
		<div class="panel-heading">
			<a href="<?php  echo iurl('cutPrice/goods/post');?>" class="btn btn-primary btn-sm">添加商品</a>
		</div>
		<?php  if(!empty($lists)) { ?>
		<div class="panel-body table-responsive js-table">
			<table class="table table-hover">
				<thead class="navbar-inner">
					<tr>
						...
						<td class="col-md-1">每次砍价金额</td>
						<td class="col-md-1">原价</td>
						<td class="col-md-1">最低价</td>
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<?php  echo $item['cut_min_price'];?> - <?php  echo $item['cut_max_price'];?>
						</td>
						<td>
							<?php  echo $item['y_price'];?>
						</td>
						<td>
						...
						<!--<td>-->
							<!--<input type="checkbox" class="js-checkbox" data-href="<?php  echo iurl('cutPrice/goods/status', array('id' => $item['id']));?>" data-name="status" value="<?php  echo $item['status'];?>" <?php  if($item['status'] == 1) { ?>checked<?php  } ?>>-->
						<!--</td>-->
						<td style="text-align:right;">
							<a href="<?php  echo iurl('cutPrice/goods/post', array('id' => $item['id']))?>" class="btn btn-default btn-sm" title="编辑" data-toggle="tooltip" data-placement="top"><i class="fa fa-edit"> </i></a>
							<a href="<?php  echo iurl('cutPrice/goods/del', array('id' => $item['id']))?>" class="btn btn-default btn-sm js-remove" title="删除" data-toggle="tooltip" data-placement="top" data-confirm="删除后将不可恢复，确定删除吗?"><i class="fa fa-times"> </i></a>
						</td>
					</tr>
					<?php  } } ?>
				</tbody>
			...
			<div class="btn-region clearfix">
				<div class="pull-left">
					...
					<a href="<?php  echo iurl('cutPrice/goods/del')?>" class="btn btn-danger btn-sm js-batch" data-batch="remove" data-confirm="确定删除选中的商品吗?">批量删除</a>
				</div>
