...
<form action="./index.php?" class="form-horizontal form-filter" id="form-takeout">
    <?php  echo tpl_form_filter_hidden('cutPrice/order/list');?>
    ...
    <div class="col-md-8">
        ...
        <div class="panel-order <?php  echo $order['order_plateform'];?>">
            ...
            <div class="btn-area">
                ...
                <a href="<?php  echo iurl('cutPrice/order/status', array('type' => 'handle', 'id' => $order['id']));?>" class="btn btn-primary btn-sm js-post" data-confirm="确定接单吗">接受订单</a>
                <a href="<?php  echo iurl('cutPrice/order/status', array('type' => 'notify_clerk_handle', 'id' => $order['id']));?>" class="btn btn-primary btn-sm js-post" data-confirm="确定通知商户接单吗">通知商户接单</a>
                <?php  } ?>
                <?php  if($order['order_type'] == 1) { ?>
                <?php  if($order['status'] == 2 || $order['status'] == 3) { ?>
                <a href="<?php  echo iurl('cutPrice/order/status', array('type' => 'notify_deliveryer_collect', 'id' => $order['id']));?>" class="btn btn-primary btn-sm js-post" data-confirm="确定通知配送员抢单吗？">通知配送员抢单</a>
                ...
                <a href="<?php  echo iurl('cutPrice/order/status', array('type' => 're_notify_deliveryer_collect', 'id' => $order['id']));?>" class="btn btn-primary btn-sm js-post" data-confirm="该订单配送员已经接单,确定将订单重新放入待抢订单列？">将订单重新放入待抢订单列</a>
                <a href="javascript:;" class="btn btn-primary btn-sm btn-dispatch" data-id="<?php  echo $order['id'];?>">重新调度</a>
                <?php  } ?>
                <?php  if($order['status'] > 1) { ?>
                <a href="<?php  echo iurl('cutPrice/order/status', array('type' => 'end', 'id' => $order['id']));?>" class="btn btn-primary btn-sm js-post" data-confirm="确定完成订单吗?">完成订单</a>
                <?php  } ?>
                ...
                <a href="<?php  echo iurl('cutPrice/order/cancel', array('id' => $order['id']));?>" class="btn btn-primary btn-sm js-modal" data-batch="modal">取消订单并退款</a>
                <?php  } else { ?>
                <a href="<?php  echo iurl('cutPrice/order/cancel', array('id' => $order['id']));?>" class="btn btn-primary btn-sm js-modal" data-batch="modal">取消订单</a>
                <?php  } ?>

                <?php  if($order['is_remind'] == 1) { ?>
                <a href="<?php  echo iurl('cutPrice/order/remind', array('id' => $order['id']));?>" class="btn btn-primary btn-sm js-modal">回复催单</a>
                <?php  } ?>
                <?php  } ?>
                <?php  if($order['refund_status'] == 1) { ?>
                <a href="<?php  echo iurl('cutPrice/order/refund_handle', array('id' => $order['id']));?>" class="btn btn-primary btn-sm js-post" data-confirm="确定发起退款吗?">发起退款</a>
                <a href="<?php  echo iurl('cutPrice/order/refund_status', array('id' => $order['id']));?>" class="btn btn-primary btn-sm js-post" data-confirm="确定设置为已退款吗?">已退款</a>
                <?php  } else if($order['refund_status'] == 2) { ?>
                <a href="<?php  echo iurl('cutPrice/order/refund_query', array('id' => $order['id']));?>" class="btn btn-primary btn-sm js-post">查询退款进度</a>
                <a href="<?php  echo iurl('cutPrice/order/refund_status', array('id' => $order['id']));?>" class="btn btn-primary btn-sm js-post" data-confirm="确定设置为已退款吗?">已退款</a>
                <?php  } ?>
                <a href="<?php  echo iurl('cutPrice/order/print', array('id' => $order['id']));?>" class="btn btn-primary btn-sm js-post" data-confirm="确定打印该订单吗?"><i class="fa fa-print"></i> ( <?php  echo $order['print_nums'];?> )</a>
                <a href="<?php  echo iurl('cutPrice/order/changeOrder', array('id' => $order['id']));?>" class="btn btn-primary btn-sm js-modal" data-batch="modal">修改订单</a>
                <a href="<?php  echo iurl('cutPrice/order/detail', array('id' => $order['id']));?>" target="_blank" class="btn btn-primary btn-sm">详情</a>
            </div>
        </div>
        <?php  } } ?>
    </div>
    ...
    irequire(['laytpl', 'tiny'], function(laytpl, tiny){
        ...
        <?php  } ?>
            ...
            $(document).on('click', '.btn-dispatch', function(){
                var id = $(this).data('id');
                $.post("<?php  echo iurl('cutPrice/order/analyse')?>", {id: id}, function(data){
            ...
            $(document).on('click', '.btn-dispatch-submit', function(){
                ...
                util.loading();
                $.post("<?php  echo iurl('cutPrice/order/dispatch')?>", {order_id: order_id, deliveryer_id: deliveryer_id}, function(data){
                    var result = $.parseJSON(data);
                    util.loaded();
                    if(result.message.errno != 0) {
                        Notify.error(result.message.message);
