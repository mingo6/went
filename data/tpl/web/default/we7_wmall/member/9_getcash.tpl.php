...
<form action="./index.php?" class="form-horizontal form-filter" id="form1">
	<?php  echo tpl_form_filter_hidden('member/getcash/list');?>
				...
				<tbody>
					<?php  if(is_array($records)) { foreach($records as $record) { ?>
						<tr>
							...
							<?php  } ?>
							<td><?php  echo $record['wx_account'];?></td>
							<td>
							...
							<td align="right">
								<?php  if($record['status'] == 2) { ?>
									<a href="<?php  echo iurl('member/getcash/transfers', array('id' => $record['id']));?>" data-confirm="确定微信打款吗" class="btn btn-primary btn-sm js-post">微信打款</a>
									<a href="<?php  echo iurl('member/getcash/status', array('id' => $record['id'], 'status' => 1));?>" data-confirm="确定变更提现状态吗" class="btn btn-default btn-sm js-post">设为已处理</a>
									<a href="<?php  echo iurl('member/getcash/cancel', array('id' => $record['id']));?>" class="btn btn-danger btn-sm js-modal">撤销</a>
								<?php  } ?>
							</td>
						</tr>
					<?php  } } ?>
