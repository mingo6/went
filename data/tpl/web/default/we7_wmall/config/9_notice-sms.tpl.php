...
<div class="page clearfix">
	...
	<form class="form-horizontal form form-validate" id="form1" action="" method="post" enctype="multipart/form-data">
		...
		<?php  if(check_plugin_perm('errander')) { ?>
			...
			<div class="form-group">
				<label class="col-xs-12 col-sm-3 col-md-2 control-label">从阿里云上申请语音模板成功时间</label>
				<div class="col-sm-9 col-xs-12">
					<div class="radio radio-inline">
						<input type="radio" value="0" name="errander_deliveryer[version]" id="errander-deliveryer-version-0" <?php  if(!$sms['errander_deliveryer']['version']) { ?>checked<?php  } ?>>
						<label for="errander-deliveryer-version-0">2018年1月22日之前</label>
					</div>
					<div class="radio radio-inline">
						<input type="radio" value="1" name="errander_deliveryer[version]" id="errander-deliveryer-version-1" <?php  if($sms['errander_deliveryer']['version'] == 1) { ?>checked<?php  } ?>>
						<label for="errander-deliveryer-version-1">2018年1月22日之后</label>
					</div>
					<div class="help-block">
						<span class="text-danger">注意：申请时间不同,系统调用的接口不一样。请正确选择申请时间</span>
					</div>
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-9 col-xs-12">
					...
					<span class="help-block">
						...
						<br>
						<strong class="text-danger">模板内容: 您好${name},平台有新的配送订单,本单可收益${deliveryerfee}元,快去抢单吧</strong>
					</span>
				</div>
			</div>
			<div class="form-group">
		...
		<?php  } ?>
		<h3>平台管理员变更配送员工作状态语音通知</h3>
		<div class="form-group">
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">开启电话通知配送员功能</label>
			<div class="col-sm-9 col-xs-12">
				<div class="radio radio-inline">
					<input type="radio" value="1" name="work_status_change[status]" id="work_status_change-1" <?php  if($sms['work_status_change']['status'] == 1) { ?>checked<?php  } ?>>
					<label for="work_status_change-1">开启</label>
				</div>
				<div class="radio radio-inline">
					<input type="radio" value="0" name="work_status_change[status]" id="work_status_change-0" <?php  if(!$sms['work_status_change']['status']) { ?>checked<?php  } ?>>
					<label for="work_status_change-0">关闭</label>
				</div>
				<div class="help-block">
					<span class="text-danger">开启此功能需要先开启短信功能并正确配置AppKey和AppSecret</span>
				</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-9 col-xs-12">
				<div class="radio radio-inline">
					<input type="radio" value="0" name="work_status_change[version]" id="work_status_change_version-0" <?php  if(!$sms['work_status_change']['version']) { ?>checked<?php  } ?>>
					<label for="work_status_change_version-0">2018年1月22日之前</label>
				</div>
				<div class="radio radio-inline">
					<input type="radio" value="1" name="work_status_change[version]" id="work_status_change_version-1" <?php  if($sms['work_status_change']['version'] == 1) { ?>checked<?php  } ?>>
					<label for="work_status_change_version-1">2018年1月22日之后</label>
				</div>
				<div class="help-block">
					<span class="text-danger">注意：申请时间不同,系统调用的接口不一样。请正确选择申请时间</span>
				</div>
		...
		<div class="form-group">
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">语音通知模板</label>
			<div class="col-sm-9 col-xs-12">
				<input type="text" class="form-control" name="work_status_change[tts_code]" value="<?php  echo $sms['work_status_change']['tts_code'];?>">
				<span class="help-block">语音通知模板,请到大鱼平台申请. </span>
					<span class="help-block">
						<strong class="text-danger">模板标题: 配送员工作状态变更通知</strong>
						<br>
						<strong class="text-danger">模板内容: 您好${name},您的工作状态已变更为${work_status_cn},如有疑问,请联系平台负责人</strong>
					</span>
			</div>
		</div>
		<div class="form-group">
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">来电号码</label>
			<div class="col-sm-9 col-xs-12">
				<input type="text" class="form-control" name="work_status_change[called_show_num]" value="<?php  echo $sms['work_status_change']['called_show_num'];?>">
				<span class="help-block">传入的显示号码必须是阿里大鱼“管理中心-号码管理”中申请或购买的号码. </span>
			</div>
		</div>
		<div class="form-group">
