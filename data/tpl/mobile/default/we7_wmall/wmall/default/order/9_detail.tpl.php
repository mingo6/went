...
<style>
.order .order-inner .order-status:before, #order-detail .order-state-con:before{
	display:none;
}
.row.no-gutter.flex{
	display: flex;
			...
			<div id="order-detail" class="tab active">
				<div class="order-state border-1px-tb">
					...
					<div class="table border-1px-t">
						...
						<?php  } else if(in_array($order['status'], array(5))) { ?>
							...
							<?php  } ?>
							<?php  if($config_order['customer_delete_order'] == 1) { ?>
								<a href="<?php  echo imurl('wmall/order/index/deleteOrder', array('id' => $order['id']));?>" class="table-cell border-1px-l js-post" data-confirm="确定删除该订单么?">删除订单</a>
							<?php  } ?>
				...
				<div class="swiper-container slide" data-space-between='10' data-pagination='.swiper-slide-pagination' data-autoplay="5000" style="overflow: hidden">
					<div class="swiper-wrapper" >
						...
						<div class="swiper-slide js-url" data-link="<?php  echo $slide['link'];?>" style="width: 100%;">
					...
					<div class="order-details-con border-1px-t">
						...
						<div class="inner-con border-1px-b">
							...
							<?php  if($order['data']['extra_fee']) { ?>
								<?php  if(is_array($order['data']['extra_fee'])) { foreach($order['data']['extra_fee'] as $extra_fee) { ?>
									<div class="row no-gutter">
										<div class="col-60">
											<?php  echo $extra_fee['name'];?>
										</div>
										<div class="col-40 text-right">￥<?php  echo $extra_fee['fee'];?></div>
									</div>
								<?php  } } ?>
							<?php  } ?>
						</div>
						...
						<div class="inner-con">
							<div class="row no-gutter flex">
								<div class="color-muted">订单 <span class="color-black">￥<?php  echo $order['total_fee'];?></span> - 优惠<span class="color-black">￥<?php  echo $order['discount_fee'];?></span></div>
								<div class="color-muted">总计 <span class="color-black">￥<?php  echo $order['final_fee'];?></span></div>
							</div>
						</div>
					</div>
					<?php  if($order['order_type'] < 3) { ?>
				...
				<div class="list-block other-info">
					<ul class="border-1px-tb">
						...
						<?php  if($order['order_type'] == 4) { ?>
							...
							<li class="item-content">
								<div class="item-inner border-1px-b">
									...
									<div class="item-after"><?php  echo $order['table_category']['title'];?></div>
								</div>
							</li>
							<li class="item-content">
								<div class="item-inner border-1px-b">
						...
						<?php  if(!empty($order['invoice']['title'])) { ?>
							<li class="item-content">
								<div class="item-inner">
									<div class="item-title">发票抬头</div>
									<div class="item-after"><?php  if(empty($order['invoice'])) { ?>无<?php  } else { ?><?php  echo $order['invoice']['title'];?><?php  } ?></div>
								</div>
							</li>
						<?php  } ?>
						<?php  if(!empty($order['invoice']['recognition'])) { ?>
							<li class="item-content">
								<div class="item-inner">
									<div class="item-title">纳税人识别号</div>
									<div class="item-after"><?php  if(empty($order['invoice'])) { ?>无<?php  } else { ?><?php  echo $order['invoice']['recognition'];?><?php  } ?></div>
								</div>
							</li>
						<?php  } ?>
					</ul>
			...
			<div id="order-status" class="tab">
				<?php  if(is_array($logs)) { foreach($logs as $key => $log) { ?>
					<div class="order-status-item">
						...
						<?php  if($order['delivery_handle_type'] == 'app' && in_array($log['type'], array('delivery_assign'))) { ?>
