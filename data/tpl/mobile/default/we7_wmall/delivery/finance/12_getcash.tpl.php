		...
		<ul class="takeout-list border-1px-tb">
			<li>
				...
				<div class="takeout-item-right">
					...
					<p class="takeout-rule">最低提现金额为<?php  echo $config_getcash['get_cash_fee_limit'];?>元</p>
					<p class="takeout-rule">提现费率为<?php  echo $config_getcash['get_cash_fee_rate'];?>%,最低收取<?php  echo $config_getcash['get_cash_fee_min'];?>元<?php  if($config_getcash['get_cash_fee_max'] > 0) { ?>,最高收取<?php  echo $config_getcash['get_cash_fee_max'];?>元<?php  } ?></p>
					<?php  if($deliveryer['credit2'] < $config_getcash['get_cash_fee_limit']) { ?>
						<a href="#" class="button button-big button-fill button-success disabled">不足<?php  echo $config_getcash['get_cash_fee_limit'];?>元</a>
					<?php  } else { ?>
