			...
			<div class="category-container">
				...
				<div class="children-category" id="delivery-time-children">
					<div class="children-category-wrapper">
						<ul id="category1">
							<?php  if($time_flag == 1) { ?>
								<li data-value="尽快到达" data-id="<?php  echo $predict_index;?>" data-extra-price="<?php  echo $predict_extra_price;?>" class="time-flag active"><a href="javascript:;">立刻 <span><!--(<?php  echo $predict_delivery_price;?>)--></span></a></li>
								<li class="delivery-tips time-flag">约<?php  echo $predict_time;?>到达</li>
							<?php  } ?>
...
<div class="popup popup-select-end-address">
	<div class="page select-address">
		...
		<div class="content">
			<?php  if(!empty($addresses['available'])) { ?>
				<div class="content-block-title">可选地址</div>
				<div class="list-block media-list">
					<ul class="border-1px-tb">
						<?php  if(is_array($addresses['available'])) { foreach($addresses['available'] as $address) { ?>
						<li>
...
<div class="modal modal-no-buttons modal-notice modal-category-notice">
	<div class="modal-inner">
		...
		<div class="modal-text">
			<div class="notice">
				<?php  echo $category['notice']['text'];?>
			</div>
			<a <?php  if(empty($category['notice']['url'])) { ?> href="javascript:;" onclick="$.closeModal('.modal-notice');" <?php  } else { ?> href="<?php  echo $category['notice']['url'];?>" <?php  } ?> class="button button-big button-fill button-danger close-modal"><?php  echo $category['notice']['btn_text'];?></a>
		</div>
	</div>
</div>
...
<script>
	<?php  if(!empty($category['notice']['text'])) { ?>
		require(['tiny'], function(tiny){
			var erranderCategoryNotice = 'erranderCategoryNotice' + <?php  echo $category['id'];?>;
			var betweentime = <?php  echo $category['notice']['between_time'];?> * 60;
