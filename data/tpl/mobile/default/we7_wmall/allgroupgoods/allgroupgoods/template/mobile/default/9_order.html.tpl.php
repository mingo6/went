		...
		<form method="post" id="order-form" action="<?php  echo imurl('wmall/order/create/submit', array('sid' => $sid));?>">
			...
			<?php  if($store['delivery_type'] == 3) { ?>
				...
				<div class="list-block media-list pay-method">
					...
					<!--<div class="btn">-->
						<!--<button type="button" name="order_type" class="order_type btn1" value="1" checked id="order-type-1">商家配送</button>-->
						<!--<button type="button" name="order_type" class="order_type btn2" value="2" id="order-type-2">到店自提</button>-->
					<!--</div>-->
				</div>
			<?php  } ?>
			...
			<div class="list-block address" id="address-container">
				<ul>
					<li>
						<a class="item-link item-content external border-1px-b" href="<?php  echo imurl('wmall/member/address', array('goods_id' => $_GPC['goods_id'], 'redirect_type' => 'group_order', 'recordid' => $_GPC['recordid'], 'type' => $_GPC['type']));?>">
							<div class="item-inner">
					...
					<li id="delivery-time-1" class="border-1px-b" style="display:none">
			...
			<div class="list-block detail-info">
				<ul class="border-1px-tb">
					<li>
						<div class="item-content">
							<div class="item-inner border-1px-b">
								...
								<div class="item-after">本单由 <span class="color-black"><?php  if($store['delivery_mode'] == 1) { ?>商家<?php  } else { ?><?php  echo $_config_mall['delivery_title'];?><?php  } ?> 配送</span> </div>
							</div>
						</div>
					</li>
					<li class="order-list">
					...
					<li class="order-list">
						<div class="inner-con border-1px-b">
							<?php  if($cart['box_price'] > 0) { ?>
								<div class="row no-gutter">
									<div class="col-50">餐盒费</div>
									<div class="col-50 text-right color-black">￥<?php  echo $cart['box_price'];?></div>
								</div>
							<?php  } ?>
							<div class="row no-gutter">
								<div class="col-50">包装费</div>
								<div class="col-50 text-right color-black">￥<?php  echo $store['pack_price'];?></div>
							</div>
							<?php  if($distance > 0) { ?>
								<div class="row no-gutter row-distance">
									<div class="col-50">配送距离</div>
									<div class="col-50 text-right color-black"><?php  echo $distance;?>公里</div>
								</div>
							<?php  } ?>
							<div class="row no-gutter row-delivery-fee">
								<?php  if($store['delivery_fee_mode'] == 2) { ?>
									<div class="col-50 delivery-fee-modal">配送费 <i class="icon icon-question-circle"></i></div>
								<?php  } else { ?>
									<div class="col-50">配送费</div>
								<?php  } ?>
								<div class="col-50 text-right color-black">￥<span id="delivery-price"><?php  echo $delivery_price;?></span></div>
							</div>
							<?php  if($extra_fee > 0) { ?>
								<?php  if(is_array($store['data']['extra_fee'])) { foreach($store['data']['extra_fee'] as $item) { ?>
									<div class="row no-gutter extra-fee">
										<div class="col-50"><?php  echo $item['name'];?></div>
										<div class="col-50 text-right color-black">￥<?php  echo $item['fee'];?></div>
									</div>
								<?php  } } ?>
							<?php  } ?>
						</div>
							...
							<div class="inner-con border-1px-b">
								<?php  if(is_array($activityed['list'])) { foreach($activityed['list'] as $key => $row) { ?>
									<div class="row no-gutter activity-row activity-<?php  echo $key;?> <?php  if($key == 'selfDelivery' && $store['delivery_type'] != 2) { ?>hide<?php  } ?>">
										...
										<div class="col-50 text-right color-danger <?php  if($row['type'] == 'delivery') { ?>delivery-activity-price-text<?php  } ?>"><?php  echo $row['text'];?></div>
									</div>
								<?php  } } ?>
							</div>
						</li>
					...
					<li class="order-list">
						<div class="inner-con">
							<div class="row no-gutter">
								<div class="col-60 color-muted">订单 <span class="color-black total-price">￥<?php  echo $cart['price'] + $cart['box_price'] + $store['pack_price'] + $delivery_price + $extra_fee?></span> - 优惠 <span class="color-black activity-price">￥<?php  echo $activity_price;?></span></div>
					...
					<div class="children-category-wrapper">
						<ul id="category1">
							<?php  if($time_flag == 1) { ?>
								...
								<li class="delivery-tips time-flag">约<?php  echo $predict_time_cn;?>送达</li>
							<?php  } ?>
	...
	<div class="content-block">
		...
		<div class="popup-body">
			...
			<div class="content-block" style="margin-top: 10px;">
				<a href="javascript:;" class="button button-danger button-fill button-big btn-submit close-popup" id="note-submit">确定</a>
			</div>
		</div>
	</div>
</div>
	...
	<div class="modal-inner">
		...
		<div class="modal-text">
			<div class="notice">
				...
				<div id="delivery-predict-price" class="<?php  if(empty($delivery_time['times'][$predict_index]['fee'])) { ?>hide<?php  } ?>">
					特殊时间段配送附加费<?php  echo $delivery_time['times'][$predict_index]['fee'];?>元
				</div>
			</div>
...
$(function(){
	...
	$(document).on('click', '#order-submit', function(){
		...
		$.post("<?php  echo imurl('allgroupgoods/order/submit', array('goods_id' => $goods['id']));?>", params, function(data){
	...
	var totalprice = <?php  echo $cart['price'] + $cart['box_price'] + $store['pack_price'] + $delivery_price + $extra_fee?>;
	...
	$(document).on('click', '#order-type-1', function(){
		...
		$('#order-type-1').removeClass("shangjia");
		$('#order-type-2').removeClass("ziqu");
	});
	...
	$(document).on('click', '#order-type-2', function(){
		...
		$('#order-type-1').addClass("shangjia");
		$('#order-type-2').addClass("ziqu");
	...
	$(document).on('click', '#delivery-time-children li:not(.delivery-tips)', function(){
		...
		if(delivery_extra_price > 0) {
			$('#delivery-predict-price').html('特殊时间段配送附加费' + delivery_extra_price + '元').removeClass('hide');
		}
